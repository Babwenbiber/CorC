/*
 * generated by Xtext 2.25.0
 */
grammar InternalDsl;

options {
	superClass=AbstractInternalContentAssistParser;
	backtrack=true;
}

@lexer::header {
package de.tu_bs.cs.isf.cbc.textual.tool.ide.contentassist.antlr.internal;

// Hack: Use our own Lexer superclass by means of import. 
// Currently there is no other way to specify the superclass for the lexer.
import org.eclipse.xtext.ide.editor.contentassist.antlr.internal.Lexer;
}

@parser::header {
package de.tu_bs.cs.isf.cbc.textual.tool.ide.contentassist.antlr.internal;

import java.io.InputStream;
import org.eclipse.xtext.*;
import org.eclipse.xtext.parser.*;
import org.eclipse.xtext.parser.impl.*;
import org.eclipse.emf.ecore.util.EcoreUtil;
import org.eclipse.emf.ecore.EObject;
import org.eclipse.xtext.parser.antlr.XtextTokenStream;
import org.eclipse.xtext.parser.antlr.XtextTokenStream.HiddenTokens;
import org.eclipse.xtext.ide.editor.contentassist.antlr.internal.AbstractInternalContentAssistParser;
import org.eclipse.xtext.ide.editor.contentassist.antlr.internal.DFA;
import de.tu_bs.cs.isf.cbc.textual.tool.services.DslGrammarAccess;

}
@parser::members {
	private DslGrammarAccess grammarAccess;

	public void setGrammarAccess(DslGrammarAccess grammarAccess) {
		this.grammarAccess = grammarAccess;
	}

	@Override
	protected Grammar getGrammar() {
		return grammarAccess.getGrammar();
	}

	@Override
	protected String getValueForTokenName(String tokenName) {
		return tokenName;
	}
}

// Entry rule entryRuleCbCProblem
entryRuleCbCProblem
:
{ before(grammarAccess.getCbCProblemRule()); }
	 ruleCbCProblem
{ after(grammarAccess.getCbCProblemRule()); } 
	 EOF 
;

// Rule CbCProblem
ruleCbCProblem 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getCbCProblemAccess().getAlternatives()); }
		(rule__CbCProblem__Alternatives)
		{ after(grammarAccess.getCbCProblemAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleCbCFormula
entryRuleCbCFormula
:
{ before(grammarAccess.getCbCFormulaRule()); }
	 ruleCbCFormula
{ after(grammarAccess.getCbCFormulaRule()); } 
	 EOF 
;

// Rule CbCFormula
ruleCbCFormula 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getCbCFormulaAccess().getGroup()); }
		(rule__CbCFormula__Group__0)
		{ after(grammarAccess.getCbCFormulaAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleAbstractStatement
entryRuleAbstractStatement
:
{ before(grammarAccess.getAbstractStatementRule()); }
	 ruleAbstractStatement
{ after(grammarAccess.getAbstractStatementRule()); } 
	 EOF 
;

// Rule AbstractStatement
ruleAbstractStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getAbstractStatementAccess().getAlternatives()); }
		(rule__AbstractStatement__Alternatives)
		{ after(grammarAccess.getAbstractStatementAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleEString
entryRuleEString
:
{ before(grammarAccess.getEStringRule()); }
	 ruleEString
{ after(grammarAccess.getEStringRule()); } 
	 EOF 
;

// Rule EString
ruleEString 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getEStringAccess().getAlternatives()); }
		(rule__EString__Alternatives)
		{ after(grammarAccess.getEStringAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleCodeString
entryRuleCodeString
:
{ before(grammarAccess.getCodeStringRule()); }
	 ruleCodeString
{ after(grammarAccess.getCodeStringRule()); } 
	 EOF 
;

// Rule CodeString
ruleCodeString 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		(
			{ before(grammarAccess.getCodeStringAccess().getGroup()); }
			(rule__CodeString__Group__0)
			{ after(grammarAccess.getCodeStringAccess().getGroup()); }
		)
		(
			{ before(grammarAccess.getCodeStringAccess().getGroup()); }
			(rule__CodeString__Group__0)*
			{ after(grammarAccess.getCodeStringAccess().getGroup()); }
		)
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleVariableString
entryRuleVariableString
:
{ before(grammarAccess.getVariableStringRule()); }
	 ruleVariableString
{ after(grammarAccess.getVariableStringRule()); } 
	 EOF 
;

// Rule VariableString
ruleVariableString 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getVariableStringAccess().getGroup()); }
		(rule__VariableString__Group__0)
		{ after(grammarAccess.getVariableStringAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleBoolean
entryRuleBoolean
:
{ before(grammarAccess.getBooleanRule()); }
	 ruleBoolean
{ after(grammarAccess.getBooleanRule()); } 
	 EOF 
;

// Rule Boolean
ruleBoolean 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getBooleanAccess().getAlternatives()); }
		(rule__Boolean__Alternatives)
		{ after(grammarAccess.getBooleanAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOperation
entryRuleOperation
:
{ before(grammarAccess.getOperationRule()); }
	 ruleOperation
{ after(grammarAccess.getOperationRule()); } 
	 EOF 
;

// Rule Operation
ruleOperation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOperationAccess().getAlternatives()); }
		(rule__Operation__Alternatives)
		{ after(grammarAccess.getOperationAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleAbstractStatement_Impl
entryRuleAbstractStatement_Impl
:
{ before(grammarAccess.getAbstractStatement_ImplRule()); }
	 ruleAbstractStatement_Impl
{ after(grammarAccess.getAbstractStatement_ImplRule()); } 
	 EOF 
;

// Rule AbstractStatement_Impl
ruleAbstractStatement_Impl 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getAbstractStatement_ImplAccess().getGroup()); }
		(rule__AbstractStatement_Impl__Group__0)
		{ after(grammarAccess.getAbstractStatement_ImplAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleMethodStatement
entryRuleMethodStatement
:
{ before(grammarAccess.getMethodStatementRule()); }
	 ruleMethodStatement
{ after(grammarAccess.getMethodStatementRule()); } 
	 EOF 
;

// Rule MethodStatement
ruleMethodStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getMethodStatementAccess().getGroup()); }
		(rule__MethodStatement__Group__0)
		{ after(grammarAccess.getMethodStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleReturnStatement
entryRuleReturnStatement
:
{ before(grammarAccess.getReturnStatementRule()); }
	 ruleReturnStatement
{ after(grammarAccess.getReturnStatementRule()); } 
	 EOF 
;

// Rule ReturnStatement
ruleReturnStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getReturnStatementAccess().getGroup()); }
		(rule__ReturnStatement__Group__0)
		{ after(grammarAccess.getReturnStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleStrengthWeakStatement
entryRuleStrengthWeakStatement
:
{ before(grammarAccess.getStrengthWeakStatementRule()); }
	 ruleStrengthWeakStatement
{ after(grammarAccess.getStrengthWeakStatementRule()); } 
	 EOF 
;

// Rule StrengthWeakStatement
ruleStrengthWeakStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getStrengthWeakStatementAccess().getGroup()); }
		(rule__StrengthWeakStatement__Group__0)
		{ after(grammarAccess.getStrengthWeakStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleCondition
entryRuleCondition
:
{ before(grammarAccess.getConditionRule()); }
	 ruleCondition
{ after(grammarAccess.getConditionRule()); } 
	 EOF 
;

// Rule Condition
ruleCondition 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getConditionAccess().getGroup()); }
		(rule__Condition__Group__0)
		{ after(grammarAccess.getConditionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleSkipStatement
entryRuleSkipStatement
:
{ before(grammarAccess.getSkipStatementRule()); }
	 ruleSkipStatement
{ after(grammarAccess.getSkipStatementRule()); } 
	 EOF 
;

// Rule SkipStatement
ruleSkipStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getSkipStatementAccess().getNameAssignment()); }
		(rule__SkipStatement__NameAssignment)
		{ after(grammarAccess.getSkipStatementAccess().getNameAssignment()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleCompositionStatement
entryRuleCompositionStatement
:
{ before(grammarAccess.getCompositionStatementRule()); }
	 ruleCompositionStatement
{ after(grammarAccess.getCompositionStatementRule()); } 
	 EOF 
;

// Rule CompositionStatement
ruleCompositionStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getCompositionStatementAccess().getGroup()); }
		(rule__CompositionStatement__Group__0)
		{ after(grammarAccess.getCompositionStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJavaStatement
entryRuleJavaStatement
:
{ before(grammarAccess.getJavaStatementRule()); }
	 ruleJavaStatement
{ after(grammarAccess.getJavaStatementRule()); } 
	 EOF 
;

// Rule JavaStatement
ruleJavaStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJavaStatementAccess().getGroup()); }
		(rule__JavaStatement__Group__0)
		{ after(grammarAccess.getJavaStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLWhileExpression
entryRuleJMLWhileExpression
:
{ before(grammarAccess.getJMLWhileExpressionRule()); }
	 ruleJMLWhileExpression
{ after(grammarAccess.getJMLWhileExpressionRule()); } 
	 EOF 
;

// Rule JMLWhileExpression
ruleJMLWhileExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLWhileExpressionAccess().getGroup()); }
		(rule__JMLWhileExpression__Group__0)
		{ after(grammarAccess.getJMLWhileExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJSingleStatement
entryRuleXJSingleStatement
:
{ before(grammarAccess.getXJSingleStatementRule()); }
	 ruleXJSingleStatement
{ after(grammarAccess.getXJSingleStatementRule()); } 
	 EOF 
;

// Rule XJSingleStatement
ruleXJSingleStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getAlternatives()); }
		(rule__XJSingleStatement__Alternatives)
		{ after(grammarAccess.getXJSingleStatementAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLAnnotation
entryRuleJMLAnnotation
:
{ before(grammarAccess.getJMLAnnotationRule()); }
	 ruleJMLAnnotation
{ after(grammarAccess.getJMLAnnotationRule()); } 
	 EOF 
;

// Rule JMLAnnotation
ruleJMLAnnotation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLAnnotationAccess().getGroup()); }
		(rule__JMLAnnotation__Group__0)
		{ after(grammarAccess.getJMLAnnotationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleLoopInvariantAnnotation
entryRuleLoopInvariantAnnotation
:
{ before(grammarAccess.getLoopInvariantAnnotationRule()); }
	 ruleLoopInvariantAnnotation
{ after(grammarAccess.getLoopInvariantAnnotationRule()); } 
	 EOF 
;

// Rule LoopInvariantAnnotation
ruleLoopInvariantAnnotation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getGroup()); }
		(rule__LoopInvariantAnnotation__Group__0)
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleLoopInvariantAnnotationStatement
entryRuleLoopInvariantAnnotationStatement
:
{ before(grammarAccess.getLoopInvariantAnnotationStatementRule()); }
	 ruleLoopInvariantAnnotationStatement
{ after(grammarAccess.getLoopInvariantAnnotationStatementRule()); } 
	 EOF 
;

// Rule LoopInvariantAnnotationStatement
ruleLoopInvariantAnnotationStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getLoopInvariantAnnotationStatementAccess().getGroup()); }
		(rule__LoopInvariantAnnotationStatement__Group__0)
		{ after(grammarAccess.getLoopInvariantAnnotationStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleInlineBlockStatement
entryRuleInlineBlockStatement
:
{ before(grammarAccess.getInlineBlockStatementRule()); }
	 ruleInlineBlockStatement
{ after(grammarAccess.getInlineBlockStatementRule()); } 
	 EOF 
;

// Rule InlineBlockStatement
ruleInlineBlockStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getInlineBlockStatementAccess().getGroup()); }
		(rule__InlineBlockStatement__Group__0)
		{ after(grammarAccess.getInlineBlockStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleBlockStatement
entryRuleBlockStatement
:
{ before(grammarAccess.getBlockStatementRule()); }
	 ruleBlockStatement
{ after(grammarAccess.getBlockStatementRule()); } 
	 EOF 
;

// Rule BlockStatement
ruleBlockStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getBlockStatementAccess().getGroup()); }
		(rule__BlockStatement__Group__0)
		{ after(grammarAccess.getBlockStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleInlineJavaBlockStatement
entryRuleInlineJavaBlockStatement
:
{ before(grammarAccess.getInlineJavaBlockStatementRule()); }
	 ruleInlineJavaBlockStatement
{ after(grammarAccess.getInlineJavaBlockStatementRule()); } 
	 EOF 
;

// Rule InlineJavaBlockStatement
ruleInlineJavaBlockStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getInlineJavaBlockStatementAccess().getGroup()); }
		(rule__InlineJavaBlockStatement__Group__0)
		{ after(grammarAccess.getInlineJavaBlockStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleSelectionStatement
entryRuleSelectionStatement
:
{ before(grammarAccess.getSelectionStatementRule()); }
	 ruleSelectionStatement
{ after(grammarAccess.getSelectionStatementRule()); } 
	 EOF 
;

// Rule SelectionStatement
ruleSelectionStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getSelectionStatementAccess().getGroup()); }
		(rule__SelectionStatement__Group__0)
		{ after(grammarAccess.getSelectionStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleSmallRepetitionStatement
entryRuleSmallRepetitionStatement
:
{ before(grammarAccess.getSmallRepetitionStatementRule()); }
	 ruleSmallRepetitionStatement
{ after(grammarAccess.getSmallRepetitionStatementRule()); } 
	 EOF 
;

// Rule SmallRepetitionStatement
ruleSmallRepetitionStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getSmallRepetitionStatementAccess().getGroup()); }
		(rule__SmallRepetitionStatement__Group__0)
		{ after(grammarAccess.getSmallRepetitionStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleVariant
entryRuleVariant
:
{ before(grammarAccess.getVariantRule()); }
	 ruleVariant
{ after(grammarAccess.getVariantRule()); } 
	 EOF 
;

// Rule Variant
ruleVariant 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getVariantAccess().getGroup()); }
		(rule__Variant__Group__0)
		{ after(grammarAccess.getVariantAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJavaVariables
entryRuleJavaVariables
:
{ before(grammarAccess.getJavaVariablesRule()); }
	 ruleJavaVariables
{ after(grammarAccess.getJavaVariablesRule()); } 
	 EOF 
;

// Rule JavaVariables
ruleJavaVariables 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJavaVariablesAccess().getGroup()); }
		(rule__JavaVariables__Group__0)
		{ after(grammarAccess.getJavaVariablesAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJavaVariable
entryRuleJavaVariable
:
{ before(grammarAccess.getJavaVariableRule()); }
	 ruleJavaVariable
{ after(grammarAccess.getJavaVariableRule()); } 
	 EOF 
;

// Rule JavaVariable
ruleJavaVariable 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJavaVariableAccess().getGroup()); }
		(rule__JavaVariable__Group__0)
		{ after(grammarAccess.getJavaVariableAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleGlobalConditions
entryRuleGlobalConditions
:
{ before(grammarAccess.getGlobalConditionsRule()); }
	 ruleGlobalConditions
{ after(grammarAccess.getGlobalConditionsRule()); } 
	 EOF 
;

// Rule GlobalConditions
ruleGlobalConditions 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getGlobalConditionsAccess().getGroup()); }
		(rule__GlobalConditions__Group__0)
		{ after(grammarAccess.getGlobalConditionsAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleRenaming
entryRuleRenaming
:
{ before(grammarAccess.getRenamingRule()); }
	 ruleRenaming
{ after(grammarAccess.getRenamingRule()); } 
	 EOF 
;

// Rule Renaming
ruleRenaming 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getRenamingAccess().getGroup()); }
		(rule__Renaming__Group__0)
		{ after(grammarAccess.getRenamingAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleRename
entryRuleRename
:
{ before(grammarAccess.getRenameRule()); }
	 ruleRename
{ after(grammarAccess.getRenameRule()); } 
	 EOF 
;

// Rule Rename
ruleRename 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getRenameAccess().getGroup()); }
		(rule__Rename__Group__0)
		{ after(grammarAccess.getRenameAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleExpression
entryRuleExpression
:
{ before(grammarAccess.getExpressionRule()); }
	 ruleExpression
{ after(grammarAccess.getExpressionRule()); } 
	 EOF 
;

// Rule Expression
ruleExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getExpressionAccess().getImplicationParserRuleCall()); }
		ruleImplication
		{ after(grammarAccess.getExpressionAccess().getImplicationParserRuleCall()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleVariableOrMethodName
entryRuleVariableOrMethodName
:
{ before(grammarAccess.getVariableOrMethodNameRule()); }
	 ruleVariableOrMethodName
{ after(grammarAccess.getVariableOrMethodNameRule()); } 
	 EOF 
;

// Rule VariableOrMethodName
ruleVariableOrMethodName 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getVariableOrMethodNameAccess().getAlternatives()); }
		(rule__VariableOrMethodName__Alternatives)
		{ after(grammarAccess.getVariableOrMethodNameAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleImplication
entryRuleImplication
:
{ before(grammarAccess.getImplicationRule()); }
	 ruleImplication
{ after(grammarAccess.getImplicationRule()); } 
	 EOF 
;

// Rule Implication
ruleImplication 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getImplicationAccess().getGroup()); }
		(rule__Implication__Group__0)
		{ after(grammarAccess.getImplicationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleConcat
entryRuleConcat
:
{ before(grammarAccess.getConcatRule()); }
	 ruleConcat
{ after(grammarAccess.getConcatRule()); } 
	 EOF 
;

// Rule Concat
ruleConcat 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getConcatAccess().getGroup()); }
		(rule__Concat__Group__0)
		{ after(grammarAccess.getConcatAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleForeach
entryRuleForeach
:
{ before(grammarAccess.getForeachRule()); }
	 ruleForeach
{ after(grammarAccess.getForeachRule()); } 
	 EOF 
;

// Rule Foreach
ruleForeach 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getForeachAccess().getAlternatives()); }
		(rule__Foreach__Alternatives)
		{ after(grammarAccess.getForeachAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleExists
entryRuleExists
:
{ before(grammarAccess.getExistsRule()); }
	 ruleExists
{ after(grammarAccess.getExistsRule()); } 
	 EOF 
;

// Rule Exists
ruleExists 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getExistsAccess().getAlternatives()); }
		(rule__Exists__Alternatives)
		{ after(grammarAccess.getExistsAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleRelation
entryRuleRelation
:
{ before(grammarAccess.getRelationRule()); }
	 ruleRelation
{ after(grammarAccess.getRelationRule()); } 
	 EOF 
;

// Rule Relation
ruleRelation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getRelationAccess().getGroup()); }
		(rule__Relation__Group__0)
		{ after(grammarAccess.getRelationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleMultiMathOperation
entryRuleMultiMathOperation
:
{ before(grammarAccess.getMultiMathOperationRule()); }
	 ruleMultiMathOperation
{ after(grammarAccess.getMultiMathOperationRule()); } 
	 EOF 
;

// Rule MultiMathOperation
ruleMultiMathOperation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getMultiMathOperationAccess().getGroup()); }
		(rule__MultiMathOperation__Group__0)
		{ after(grammarAccess.getMultiMathOperationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleAddMathOperation
entryRuleAddMathOperation
:
{ before(grammarAccess.getAddMathOperationRule()); }
	 ruleAddMathOperation
{ after(grammarAccess.getAddMathOperationRule()); } 
	 EOF 
;

// Rule AddMathOperation
ruleAddMathOperation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getAddMathOperationAccess().getGroup()); }
		(rule__AddMathOperation__Group__0)
		{ after(grammarAccess.getAddMathOperationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleQualifiedExpression
entryRuleQualifiedExpression
:
{ before(grammarAccess.getQualifiedExpressionRule()); }
	 ruleQualifiedExpression
{ after(grammarAccess.getQualifiedExpressionRule()); } 
	 EOF 
;

// Rule QualifiedExpression
ruleQualifiedExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getQualifiedExpressionAccess().getGroup()); }
		(rule__QualifiedExpression__Group__0)
		{ after(grammarAccess.getQualifiedExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRulePrimaryExpression
entryRulePrimaryExpression
:
{ before(grammarAccess.getPrimaryExpressionRule()); }
	 rulePrimaryExpression
{ after(grammarAccess.getPrimaryExpressionRule()); } 
	 EOF 
;

// Rule PrimaryExpression
rulePrimaryExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getAlternatives()); }
		(rule__PrimaryExpression__Alternatives)
		{ after(grammarAccess.getPrimaryExpressionAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLExpression
entryRuleJMLExpression
:
{ before(grammarAccess.getJMLExpressionRule()); }
	 ruleJMLExpression
{ after(grammarAccess.getJMLExpressionRule()); } 
	 EOF 
;

// Rule JMLExpression
ruleJMLExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLExpressionAccess().getJMLImplicationParserRuleCall()); }
		ruleJMLImplication
		{ after(grammarAccess.getJMLExpressionAccess().getJMLImplicationParserRuleCall()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLImplication
entryRuleJMLImplication
:
{ before(grammarAccess.getJMLImplicationRule()); }
	 ruleJMLImplication
{ after(grammarAccess.getJMLImplicationRule()); } 
	 EOF 
;

// Rule JMLImplication
ruleJMLImplication 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLImplicationAccess().getGroup()); }
		(rule__JMLImplication__Group__0)
		{ after(grammarAccess.getJMLImplicationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLConcat
entryRuleJMLConcat
:
{ before(grammarAccess.getJMLConcatRule()); }
	 ruleJMLConcat
{ after(grammarAccess.getJMLConcatRule()); } 
	 EOF 
;

// Rule JMLConcat
ruleJMLConcat 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLConcatAccess().getGroup()); }
		(rule__JMLConcat__Group__0)
		{ after(grammarAccess.getJMLConcatAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLForeach
entryRuleJMLForeach
:
{ before(grammarAccess.getJMLForeachRule()); }
	 ruleJMLForeach
{ after(grammarAccess.getJMLForeachRule()); } 
	 EOF 
;

// Rule JMLForeach
ruleJMLForeach 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLForeachAccess().getAlternatives()); }
		(rule__JMLForeach__Alternatives)
		{ after(grammarAccess.getJMLForeachAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLExists
entryRuleJMLExists
:
{ before(grammarAccess.getJMLExistsRule()); }
	 ruleJMLExists
{ after(grammarAccess.getJMLExistsRule()); } 
	 EOF 
;

// Rule JMLExists
ruleJMLExists 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLExistsAccess().getAlternatives()); }
		(rule__JMLExists__Alternatives)
		{ after(grammarAccess.getJMLExistsAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLRelation
entryRuleJMLRelation
:
{ before(grammarAccess.getJMLRelationRule()); }
	 ruleJMLRelation
{ after(grammarAccess.getJMLRelationRule()); } 
	 EOF 
;

// Rule JMLRelation
ruleJMLRelation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLRelationAccess().getGroup()); }
		(rule__JMLRelation__Group__0)
		{ after(grammarAccess.getJMLRelationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLMultiMathOperation
entryRuleJMLMultiMathOperation
:
{ before(grammarAccess.getJMLMultiMathOperationRule()); }
	 ruleJMLMultiMathOperation
{ after(grammarAccess.getJMLMultiMathOperationRule()); } 
	 EOF 
;

// Rule JMLMultiMathOperation
ruleJMLMultiMathOperation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLMultiMathOperationAccess().getGroup()); }
		(rule__JMLMultiMathOperation__Group__0)
		{ after(grammarAccess.getJMLMultiMathOperationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLAddMathOperation
entryRuleJMLAddMathOperation
:
{ before(grammarAccess.getJMLAddMathOperationRule()); }
	 ruleJMLAddMathOperation
{ after(grammarAccess.getJMLAddMathOperationRule()); } 
	 EOF 
;

// Rule JMLAddMathOperation
ruleJMLAddMathOperation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLAddMathOperationAccess().getGroup()); }
		(rule__JMLAddMathOperation__Group__0)
		{ after(grammarAccess.getJMLAddMathOperationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLQualifiedExpression
entryRuleJMLQualifiedExpression
:
{ before(grammarAccess.getJMLQualifiedExpressionRule()); }
	 ruleJMLQualifiedExpression
{ after(grammarAccess.getJMLQualifiedExpressionRule()); } 
	 EOF 
;

// Rule JMLQualifiedExpression
ruleJMLQualifiedExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLQualifiedExpressionAccess().getGroup()); }
		(rule__JMLQualifiedExpression__Group__0)
		{ after(grammarAccess.getJMLQualifiedExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJMLPrimaryExpression
entryRuleJMLPrimaryExpression
:
{ before(grammarAccess.getJMLPrimaryExpressionRule()); }
	 ruleJMLPrimaryExpression
{ after(grammarAccess.getJMLPrimaryExpressionRule()); } 
	 EOF 
;

// Rule JMLPrimaryExpression
ruleJMLPrimaryExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getAlternatives()); }
		(rule__JMLPrimaryExpression__Alternatives)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJStatementOrBlock
entryRuleXJStatementOrBlock
:
{ before(grammarAccess.getXJStatementOrBlockRule()); }
	 ruleXJStatementOrBlock
{ after(grammarAccess.getXJStatementOrBlockRule()); } 
	 EOF 
;

// Rule XJStatementOrBlock
ruleXJStatementOrBlock 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJStatementOrBlockAccess().getAlternatives()); }
		(rule__XJStatementOrBlock__Alternatives)
		{ after(grammarAccess.getXJStatementOrBlockAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXImportDeclaration
entryRuleXImportDeclaration
:
{ before(grammarAccess.getXImportDeclarationRule()); }
	 ruleXImportDeclaration
{ after(grammarAccess.getXImportDeclarationRule()); } 
	 EOF 
;

// Rule XImportDeclaration
ruleXImportDeclaration 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getGroup()); }
		(rule__XImportDeclaration__Group__0)
		{ after(grammarAccess.getXImportDeclarationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmTypeReference
entryRuleJvmTypeReference
:
{ before(grammarAccess.getJvmTypeReferenceRule()); }
	 ruleJvmTypeReference
{ after(grammarAccess.getJvmTypeReferenceRule()); } 
	 EOF 
;

// Rule JvmTypeReference
ruleJvmTypeReference 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmTypeReferenceAccess().getGroup()); }
		(rule__JvmTypeReference__Group__0)
		{ after(grammarAccess.getJvmTypeReferenceAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleFullJvmFormalParameter
entryRuleFullJvmFormalParameter
:
{ before(grammarAccess.getFullJvmFormalParameterRule()); }
	 ruleFullJvmFormalParameter
{ after(grammarAccess.getFullJvmFormalParameterRule()); } 
	 EOF 
;

// Rule FullJvmFormalParameter
ruleFullJvmFormalParameter 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getFullJvmFormalParameterAccess().getGroup()); }
		(rule__FullJvmFormalParameter__Group__0)
		{ after(grammarAccess.getFullJvmFormalParameterAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJSemicolonStatement
entryRuleXJSemicolonStatement
:
{ before(grammarAccess.getXJSemicolonStatementRule()); }
	 ruleXJSemicolonStatement
{ after(grammarAccess.getXJSemicolonStatementRule()); } 
	 EOF 
;

// Rule XJSemicolonStatement
ruleXJSemicolonStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJSemicolonStatementAccess().getGroup()); }
		(rule__XJSemicolonStatement__Group__0)
		{ after(grammarAccess.getXJSemicolonStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJEmptyStatement
entryRuleXJEmptyStatement
:
{ before(grammarAccess.getXJEmptyStatementRule()); }
	 ruleXJEmptyStatement
{ after(grammarAccess.getXJEmptyStatementRule()); } 
	 EOF 
;

// Rule XJEmptyStatement
ruleXJEmptyStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJEmptyStatementAccess().getGroup()); }
		(rule__XJEmptyStatement__Group__0)
		{ after(grammarAccess.getXJEmptyStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJBranchingStatement
entryRuleXJBranchingStatement
:
{ before(grammarAccess.getXJBranchingStatementRule()); }
	 ruleXJBranchingStatement
{ after(grammarAccess.getXJBranchingStatementRule()); } 
	 EOF 
;

// Rule XJBranchingStatement
ruleXJBranchingStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJBranchingStatementAccess().getAlternatives()); }
		(rule__XJBranchingStatement__Alternatives)
		{ after(grammarAccess.getXJBranchingStatementAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXBlockExpression
entryRuleXBlockExpression
:
{ before(grammarAccess.getXBlockExpressionRule()); }
	 ruleXBlockExpression
{ after(grammarAccess.getXBlockExpressionRule()); } 
	 EOF 
;

// Rule XBlockExpression
ruleXBlockExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXBlockExpressionAccess().getGroup()); }
		(rule__XBlockExpression__Group__0)
		{ after(grammarAccess.getXBlockExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXVariableDeclaration
entryRuleXVariableDeclaration
:
{ before(grammarAccess.getXVariableDeclarationRule()); }
	 ruleXVariableDeclaration
{ after(grammarAccess.getXVariableDeclarationRule()); } 
	 EOF 
;

// Rule XVariableDeclaration
ruleXVariableDeclaration 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXVariableDeclarationAccess().getGroup()); }
		(rule__XVariableDeclaration__Group__0)
		{ after(grammarAccess.getXVariableDeclarationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJAdditionalXVariableDeclaration
entryRuleXJAdditionalXVariableDeclaration
:
{ before(grammarAccess.getXJAdditionalXVariableDeclarationRule()); }
	 ruleXJAdditionalXVariableDeclaration
{ after(grammarAccess.getXJAdditionalXVariableDeclarationRule()); } 
	 EOF 
;

// Rule XJAdditionalXVariableDeclaration
ruleXJAdditionalXVariableDeclaration 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getGroup()); }
		(rule__XJAdditionalXVariableDeclaration__Group__0)
		{ after(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXAssignment
entryRuleXAssignment
:
{ before(grammarAccess.getXAssignmentRule()); }
	 ruleXAssignment
{ after(grammarAccess.getXAssignmentRule()); } 
	 EOF 
;

// Rule XAssignment
ruleXAssignment 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXAssignmentAccess().getAlternatives()); }
		(rule__XAssignment__Alternatives)
		{ after(grammarAccess.getXAssignmentAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpMultiAssign
entryRuleOpMultiAssign
:
{ before(grammarAccess.getOpMultiAssignRule()); }
	 ruleOpMultiAssign
{ after(grammarAccess.getOpMultiAssignRule()); } 
	 EOF 
;

// Rule OpMultiAssign
ruleOpMultiAssign 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getAlternatives()); }
		(rule__OpMultiAssign__Alternatives)
		{ after(grammarAccess.getOpMultiAssignAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJFeatureCallWithArrayAccess
entryRuleXJFeatureCallWithArrayAccess
:
{ before(grammarAccess.getXJFeatureCallWithArrayAccessRule()); }
	 ruleXJFeatureCallWithArrayAccess
{ after(grammarAccess.getXJFeatureCallWithArrayAccessRule()); } 
	 EOF 
;

// Rule XJFeatureCallWithArrayAccess
ruleXJFeatureCallWithArrayAccess 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getGroup()); }
		(rule__XJFeatureCallWithArrayAccess__Group__0)
		{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXFeatureCall
entryRuleXFeatureCall
:
{ before(grammarAccess.getXFeatureCallRule()); }
	 ruleXFeatureCall
{ after(grammarAccess.getXFeatureCallRule()); } 
	 EOF 
;

// Rule XFeatureCall
ruleXFeatureCall 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXFeatureCallAccess().getGroup()); }
		(rule__XFeatureCall__Group__0)
		{ after(grammarAccess.getXFeatureCallAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXConstructorCall
entryRuleXConstructorCall
:
{ before(grammarAccess.getXConstructorCallRule()); }
	 ruleXConstructorCall
{ after(grammarAccess.getXConstructorCallRule()); } 
	 EOF 
;

// Rule XConstructorCall
ruleXConstructorCall 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXConstructorCallAccess().getGroup()); }
		(rule__XConstructorCall__Group__0)
		{ after(grammarAccess.getXConstructorCallAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJArrayConstructorCall
entryRuleXJArrayConstructorCall
:
{ before(grammarAccess.getXJArrayConstructorCallRule()); }
	 ruleXJArrayConstructorCall
{ after(grammarAccess.getXJArrayConstructorCallRule()); } 
	 EOF 
;

// Rule XJArrayConstructorCall
ruleXJArrayConstructorCall 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJArrayConstructorCallAccess().getGroup()); }
		(rule__XJArrayConstructorCall__Group__0)
		{ after(grammarAccess.getXJArrayConstructorCallAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJArrayDimension
entryRuleXJArrayDimension
:
{ before(grammarAccess.getXJArrayDimensionRule()); }
	 ruleXJArrayDimension
{ after(grammarAccess.getXJArrayDimensionRule()); } 
	 EOF 
;

// Rule XJArrayDimension
ruleXJArrayDimension 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJArrayDimensionAccess().getGroup()); }
		(rule__XJArrayDimension__Group__0)
		{ after(grammarAccess.getXJArrayDimensionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXCastedExpression
entryRuleXCastedExpression
:
{ before(grammarAccess.getXCastedExpressionRule()); }
	 ruleXCastedExpression
{ after(grammarAccess.getXCastedExpressionRule()); } 
	 EOF 
;

// Rule XCastedExpression
ruleXCastedExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXCastedExpressionAccess().getAlternatives()); }
		(rule__XCastedExpression__Alternatives)
		{ after(grammarAccess.getXCastedExpressionAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXPostfixOperation
entryRuleXPostfixOperation
:
{ before(grammarAccess.getXPostfixOperationRule()); }
	 ruleXPostfixOperation
{ after(grammarAccess.getXPostfixOperationRule()); } 
	 EOF 
;

// Rule XPostfixOperation
ruleXPostfixOperation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getAlternatives()); }
		(rule__XPostfixOperation__Alternatives)
		{ after(grammarAccess.getXPostfixOperationAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXMemberFeatureCall
entryRuleXMemberFeatureCall
:
{ before(grammarAccess.getXMemberFeatureCallRule()); }
	 ruleXMemberFeatureCall
{ after(grammarAccess.getXMemberFeatureCallRule()); } 
	 EOF 
;

// Rule XMemberFeatureCall
ruleXMemberFeatureCall 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup()); }
		(rule__XMemberFeatureCall__Group__0)
		{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXForLoopExpression
entryRuleXForLoopExpression
:
{ before(grammarAccess.getXForLoopExpressionRule()); }
	 ruleXForLoopExpression
{ after(grammarAccess.getXForLoopExpressionRule()); } 
	 EOF 
;

// Rule XForLoopExpression
ruleXForLoopExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXForLoopExpressionAccess().getGroup()); }
		(rule__XForLoopExpression__Group__0)
		{ after(grammarAccess.getXForLoopExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXBasicForLoopExpression
entryRuleXBasicForLoopExpression
:
{ before(grammarAccess.getXBasicForLoopExpressionRule()); }
	 ruleXBasicForLoopExpression
{ after(grammarAccess.getXBasicForLoopExpressionRule()); } 
	 EOF 
;

// Rule XBasicForLoopExpression
ruleXBasicForLoopExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXBasicForLoopExpressionAccess().getGroup()); }
		(rule__XBasicForLoopExpression__Group__0)
		{ after(grammarAccess.getXBasicForLoopExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXIfExpression
entryRuleXIfExpression
:
{ before(grammarAccess.getXIfExpressionRule()); }
	 ruleXIfExpression
{ after(grammarAccess.getXIfExpressionRule()); } 
	 EOF 
;

// Rule XIfExpression
ruleXIfExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXIfExpressionAccess().getGroup()); }
		(rule__XIfExpression__Group__0)
		{ after(grammarAccess.getXIfExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXWhileExpression
entryRuleXWhileExpression
:
{ before(grammarAccess.getXWhileExpressionRule()); }
	 ruleXWhileExpression
{ after(grammarAccess.getXWhileExpressionRule()); } 
	 EOF 
;

// Rule XWhileExpression
ruleXWhileExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXWhileExpressionAccess().getGroup()); }
		(rule__XWhileExpression__Group__0)
		{ after(grammarAccess.getXWhileExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXDoWhileExpression
entryRuleXDoWhileExpression
:
{ before(grammarAccess.getXDoWhileExpressionRule()); }
	 ruleXDoWhileExpression
{ after(grammarAccess.getXDoWhileExpressionRule()); } 
	 EOF 
;

// Rule XDoWhileExpression
ruleXDoWhileExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXDoWhileExpressionAccess().getGroup()); }
		(rule__XDoWhileExpression__Group__0)
		{ after(grammarAccess.getXDoWhileExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXSwitchExpression
entryRuleXSwitchExpression
:
{ before(grammarAccess.getXSwitchExpressionRule()); }
	 ruleXSwitchExpression
{ after(grammarAccess.getXSwitchExpressionRule()); } 
	 EOF 
;

// Rule XSwitchExpression
ruleXSwitchExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXSwitchExpressionAccess().getGroup()); }
		(rule__XSwitchExpression__Group__0)
		{ after(grammarAccess.getXSwitchExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXCasePart
entryRuleXCasePart
:
{ before(grammarAccess.getXCasePartRule()); }
	 ruleXCasePart
{ after(grammarAccess.getXCasePartRule()); } 
	 EOF 
;

// Rule XCasePart
ruleXCasePart 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXCasePartAccess().getGroup()); }
		(rule__XCasePart__Group__0)
		{ after(grammarAccess.getXCasePartAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJSwitchStatements
entryRuleXJSwitchStatements
:
{ before(grammarAccess.getXJSwitchStatementsRule()); }
	 ruleXJSwitchStatements
{ after(grammarAccess.getXJSwitchStatementsRule()); } 
	 EOF 
;

// Rule XJSwitchStatements
ruleXJSwitchStatements 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJSwitchStatementsAccess().getGroup()); }
		(rule__XJSwitchStatements__Group__0)
		{ after(grammarAccess.getXJSwitchStatementsAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJTryWithResourcesStatement
entryRuleXJTryWithResourcesStatement
:
{ before(grammarAccess.getXJTryWithResourcesStatementRule()); }
	 ruleXJTryWithResourcesStatement
{ after(grammarAccess.getXJTryWithResourcesStatementRule()); } 
	 EOF 
;

// Rule XJTryWithResourcesStatement
ruleXJTryWithResourcesStatement 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getGroup()); }
		(rule__XJTryWithResourcesStatement__Group__0)
		{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJTryWithResourcesVariableDeclaration
entryRuleXJTryWithResourcesVariableDeclaration
:
{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationRule()); }
	 ruleXJTryWithResourcesVariableDeclaration
{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationRule()); } 
	 EOF 
;

// Rule XJTryWithResourcesVariableDeclaration
ruleXJTryWithResourcesVariableDeclaration 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getGroup()); }
		(rule__XJTryWithResourcesVariableDeclaration__Group__0)
		{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXTryCatchFinallyExpression
entryRuleXTryCatchFinallyExpression
:
{ before(grammarAccess.getXTryCatchFinallyExpressionRule()); }
	 ruleXTryCatchFinallyExpression
{ after(grammarAccess.getXTryCatchFinallyExpressionRule()); } 
	 EOF 
;

// Rule XTryCatchFinallyExpression
ruleXTryCatchFinallyExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup()); }
		(rule__XTryCatchFinallyExpression__Group__0)
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXCatchClause
entryRuleXCatchClause
:
{ before(grammarAccess.getXCatchClauseRule()); }
	 ruleXCatchClause
{ after(grammarAccess.getXCatchClauseRule()); } 
	 EOF 
;

// Rule XCatchClause
ruleXCatchClause 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXCatchClauseAccess().getGroup()); }
		(rule__XCatchClause__Group__0)
		{ after(grammarAccess.getXCatchClauseAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXSynchronizedExpression
entryRuleXSynchronizedExpression
:
{ before(grammarAccess.getXSynchronizedExpressionRule()); }
	 ruleXSynchronizedExpression
{ after(grammarAccess.getXSynchronizedExpressionRule()); } 
	 EOF 
;

// Rule XSynchronizedExpression
ruleXSynchronizedExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXSynchronizedExpressionAccess().getGroup()); }
		(rule__XSynchronizedExpression__Group__0)
		{ after(grammarAccess.getXSynchronizedExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXPrimaryExpression
entryRuleXPrimaryExpression
:
{ before(grammarAccess.getXPrimaryExpressionRule()); }
	 ruleXPrimaryExpression
{ after(grammarAccess.getXPrimaryExpressionRule()); } 
	 EOF 
;

// Rule XPrimaryExpression
ruleXPrimaryExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXPrimaryExpressionAccess().getAlternatives()); }
		(rule__XPrimaryExpression__Alternatives)
		{ after(grammarAccess.getXPrimaryExpressionAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXLiteral
entryRuleXLiteral
:
{ before(grammarAccess.getXLiteralRule()); }
	 ruleXLiteral
{ after(grammarAccess.getXLiteralRule()); } 
	 EOF 
;

// Rule XLiteral
ruleXLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXLiteralAccess().getAlternatives()); }
		(rule__XLiteral__Alternatives)
		{ after(grammarAccess.getXLiteralAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXJArrayLiteral
entryRuleXJArrayLiteral
:
{ before(grammarAccess.getXJArrayLiteralRule()); }
	 ruleXJArrayLiteral
{ after(grammarAccess.getXJArrayLiteralRule()); } 
	 EOF 
;

// Rule XJArrayLiteral
ruleXJArrayLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXJArrayLiteralAccess().getGroup()); }
		(rule__XJArrayLiteral__Group__0)
		{ after(grammarAccess.getXJArrayLiteralAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXCharLiteral
entryRuleXCharLiteral
:
{ before(grammarAccess.getXCharLiteralRule()); }
	 ruleXCharLiteral
{ after(grammarAccess.getXCharLiteralRule()); } 
	 EOF 
;

// Rule XCharLiteral
ruleXCharLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXCharLiteralAccess().getGroup()); }
		(rule__XCharLiteral__Group__0)
		{ after(grammarAccess.getXCharLiteralAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXStringLiteral
entryRuleXStringLiteral
:
{ before(grammarAccess.getXStringLiteralRule()); }
	 ruleXStringLiteral
{ after(grammarAccess.getXStringLiteralRule()); } 
	 EOF 
;

// Rule XStringLiteral
ruleXStringLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXStringLiteralAccess().getGroup()); }
		(rule__XStringLiteral__Group__0)
		{ after(grammarAccess.getXStringLiteralAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpEquality
entryRuleOpEquality
:
{ before(grammarAccess.getOpEqualityRule()); }
	 ruleOpEquality
{ after(grammarAccess.getOpEqualityRule()); } 
	 EOF 
;

// Rule OpEquality
ruleOpEquality 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpEqualityAccess().getAlternatives()); }
		(rule__OpEquality__Alternatives)
		{ after(grammarAccess.getOpEqualityAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpMulti
entryRuleOpMulti
:
{ before(grammarAccess.getOpMultiRule()); }
	 ruleOpMulti
{ after(grammarAccess.getOpMultiRule()); } 
	 EOF 
;

// Rule OpMulti
ruleOpMulti 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpMultiAccess().getAlternatives()); }
		(rule__OpMulti__Alternatives)
		{ after(grammarAccess.getOpMultiAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpOther
entryRuleOpOther
:
{ before(grammarAccess.getOpOtherRule()); }
	 ruleOpOther
{ after(grammarAccess.getOpOtherRule()); } 
	 EOF 
;

// Rule OpOther
ruleOpOther 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpOtherAccess().getAlternatives()); }
		(rule__OpOther__Alternatives)
		{ after(grammarAccess.getOpOtherAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXAndExpression
entryRuleXAndExpression
:
{ before(grammarAccess.getXAndExpressionRule()); }
	 ruleXAndExpression
{ after(grammarAccess.getXAndExpressionRule()); } 
	 EOF 
;

// Rule XAndExpression
ruleXAndExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXAndExpressionAccess().getGroup()); }
		(rule__XAndExpression__Group__0)
		{ after(grammarAccess.getXAndExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXBitwiseInclusiveOrExpression
entryRuleXBitwiseInclusiveOrExpression
:
{ before(grammarAccess.getXBitwiseInclusiveOrExpressionRule()); }
	 ruleXBitwiseInclusiveOrExpression
{ after(grammarAccess.getXBitwiseInclusiveOrExpressionRule()); } 
	 EOF 
;

// Rule XBitwiseInclusiveOrExpression
ruleXBitwiseInclusiveOrExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getGroup()); }
		(rule__XBitwiseInclusiveOrExpression__Group__0)
		{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpInclusiveOr
entryRuleOpInclusiveOr
:
{ before(grammarAccess.getOpInclusiveOrRule()); }
	 ruleOpInclusiveOr
{ after(grammarAccess.getOpInclusiveOrRule()); } 
	 EOF 
;

// Rule OpInclusiveOr
ruleOpInclusiveOr 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpInclusiveOrAccess().getVerticalLineKeyword()); }
		'|'
		{ after(grammarAccess.getOpInclusiveOrAccess().getVerticalLineKeyword()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXBitwiseExclusiveOrExpression
entryRuleXBitwiseExclusiveOrExpression
:
{ before(grammarAccess.getXBitwiseExclusiveOrExpressionRule()); }
	 ruleXBitwiseExclusiveOrExpression
{ after(grammarAccess.getXBitwiseExclusiveOrExpressionRule()); } 
	 EOF 
;

// Rule XBitwiseExclusiveOrExpression
ruleXBitwiseExclusiveOrExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getGroup()); }
		(rule__XBitwiseExclusiveOrExpression__Group__0)
		{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpExclusiveOr
entryRuleOpExclusiveOr
:
{ before(grammarAccess.getOpExclusiveOrRule()); }
	 ruleOpExclusiveOr
{ after(grammarAccess.getOpExclusiveOrRule()); } 
	 EOF 
;

// Rule OpExclusiveOr
ruleOpExclusiveOr 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpExclusiveOrAccess().getCircumflexAccentKeyword()); }
		'^'
		{ after(grammarAccess.getOpExclusiveOrAccess().getCircumflexAccentKeyword()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXBitwiseAndExpression
entryRuleXBitwiseAndExpression
:
{ before(grammarAccess.getXBitwiseAndExpressionRule()); }
	 ruleXBitwiseAndExpression
{ after(grammarAccess.getXBitwiseAndExpressionRule()); } 
	 EOF 
;

// Rule XBitwiseAndExpression
ruleXBitwiseAndExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXBitwiseAndExpressionAccess().getGroup()); }
		(rule__XBitwiseAndExpression__Group__0)
		{ after(grammarAccess.getXBitwiseAndExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpBitwiseAnd
entryRuleOpBitwiseAnd
:
{ before(grammarAccess.getOpBitwiseAndRule()); }
	 ruleOpBitwiseAnd
{ after(grammarAccess.getOpBitwiseAndRule()); } 
	 EOF 
;

// Rule OpBitwiseAnd
ruleOpBitwiseAnd 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpBitwiseAndAccess().getAmpersandKeyword()); }
		'&'
		{ after(grammarAccess.getOpBitwiseAndAccess().getAmpersandKeyword()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpUnary
entryRuleOpUnary
:
{ before(grammarAccess.getOpUnaryRule()); }
	 ruleOpUnary
{ after(grammarAccess.getOpUnaryRule()); } 
	 EOF 
;

// Rule OpUnary
ruleOpUnary 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpUnaryAccess().getAlternatives()); }
		(rule__OpUnary__Alternatives)
		{ after(grammarAccess.getOpUnaryAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXAnnotationElementValueOrCommaList
entryRuleXAnnotationElementValueOrCommaList
:
{ before(grammarAccess.getXAnnotationElementValueOrCommaListRule()); }
	 ruleXAnnotationElementValueOrCommaList
{ after(grammarAccess.getXAnnotationElementValueOrCommaListRule()); } 
	 EOF 
;

// Rule XAnnotationElementValueOrCommaList
ruleXAnnotationElementValueOrCommaList 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getAlternatives()); }
		(rule__XAnnotationElementValueOrCommaList__Alternatives)
		{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXAnnotationElementValue
entryRuleXAnnotationElementValue
:
{ before(grammarAccess.getXAnnotationElementValueRule()); }
	 ruleXAnnotationElementValue
{ after(grammarAccess.getXAnnotationElementValueRule()); } 
	 EOF 
;

// Rule XAnnotationElementValue
ruleXAnnotationElementValue 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXAnnotationElementValueAccess().getAlternatives()); }
		(rule__XAnnotationElementValue__Alternatives)
		{ after(grammarAccess.getXAnnotationElementValueAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXAnnotation
entryRuleXAnnotation
:
{ before(grammarAccess.getXAnnotationRule()); }
	 ruleXAnnotation
{ after(grammarAccess.getXAnnotationRule()); } 
	 EOF 
;

// Rule XAnnotation
ruleXAnnotation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXAnnotationAccess().getGroup()); }
		(rule__XAnnotation__Group__0)
		{ after(grammarAccess.getXAnnotationAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXAnnotationElementValuePair
entryRuleXAnnotationElementValuePair
:
{ before(grammarAccess.getXAnnotationElementValuePairRule()); }
	 ruleXAnnotationElementValuePair
{ after(grammarAccess.getXAnnotationElementValuePairRule()); } 
	 EOF 
;

// Rule XAnnotationElementValuePair
ruleXAnnotationElementValuePair 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXAnnotationElementValuePairAccess().getGroup()); }
		(rule__XAnnotationElementValuePair__Group__0)
		{ after(grammarAccess.getXAnnotationElementValuePairAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXAnnotationOrExpression
entryRuleXAnnotationOrExpression
:
{ before(grammarAccess.getXAnnotationOrExpressionRule()); }
	 ruleXAnnotationOrExpression
{ after(grammarAccess.getXAnnotationOrExpressionRule()); } 
	 EOF 
;

// Rule XAnnotationOrExpression
ruleXAnnotationOrExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXAnnotationOrExpressionAccess().getAlternatives()); }
		(rule__XAnnotationOrExpression__Alternatives)
		{ after(grammarAccess.getXAnnotationOrExpressionAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXExpression
entryRuleXExpression
:
{ before(grammarAccess.getXExpressionRule()); }
	 ruleXExpression
{ after(grammarAccess.getXExpressionRule()); } 
	 EOF 
;

// Rule XExpression
ruleXExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXExpressionAccess().getXAssignmentParserRuleCall()); }
		ruleXAssignment
		{ after(grammarAccess.getXExpressionAccess().getXAssignmentParserRuleCall()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpSingleAssign
entryRuleOpSingleAssign
:
{ before(grammarAccess.getOpSingleAssignRule()); }
	 ruleOpSingleAssign
{ after(grammarAccess.getOpSingleAssignRule()); } 
	 EOF 
;

// Rule OpSingleAssign
ruleOpSingleAssign 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpSingleAssignAccess().getEqualsSignKeyword()); }
		'='
		{ after(grammarAccess.getOpSingleAssignAccess().getEqualsSignKeyword()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXOrExpression
entryRuleXOrExpression
:
{ before(grammarAccess.getXOrExpressionRule()); }
	 ruleXOrExpression
{ after(grammarAccess.getXOrExpressionRule()); } 
	 EOF 
;

// Rule XOrExpression
ruleXOrExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXOrExpressionAccess().getGroup()); }
		(rule__XOrExpression__Group__0)
		{ after(grammarAccess.getXOrExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpOr
entryRuleOpOr
:
{ before(grammarAccess.getOpOrRule()); }
	 ruleOpOr
{ after(grammarAccess.getOpOrRule()); } 
	 EOF 
;

// Rule OpOr
ruleOpOr 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpOrAccess().getVerticalLineVerticalLineKeyword()); }
		'||'
		{ after(grammarAccess.getOpOrAccess().getVerticalLineVerticalLineKeyword()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpAnd
entryRuleOpAnd
:
{ before(grammarAccess.getOpAndRule()); }
	 ruleOpAnd
{ after(grammarAccess.getOpAndRule()); } 
	 EOF 
;

// Rule OpAnd
ruleOpAnd 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpAndAccess().getAmpersandAmpersandKeyword()); }
		'&&'
		{ after(grammarAccess.getOpAndAccess().getAmpersandAmpersandKeyword()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXEqualityExpression
entryRuleXEqualityExpression
:
{ before(grammarAccess.getXEqualityExpressionRule()); }
	 ruleXEqualityExpression
{ after(grammarAccess.getXEqualityExpressionRule()); } 
	 EOF 
;

// Rule XEqualityExpression
ruleXEqualityExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXEqualityExpressionAccess().getGroup()); }
		(rule__XEqualityExpression__Group__0)
		{ after(grammarAccess.getXEqualityExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXRelationalExpression
entryRuleXRelationalExpression
:
{ before(grammarAccess.getXRelationalExpressionRule()); }
	 ruleXRelationalExpression
{ after(grammarAccess.getXRelationalExpressionRule()); } 
	 EOF 
;

// Rule XRelationalExpression
ruleXRelationalExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXRelationalExpressionAccess().getGroup()); }
		(rule__XRelationalExpression__Group__0)
		{ after(grammarAccess.getXRelationalExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpCompare
entryRuleOpCompare
:
{ before(grammarAccess.getOpCompareRule()); }
	 ruleOpCompare
{ after(grammarAccess.getOpCompareRule()); } 
	 EOF 
;

// Rule OpCompare
ruleOpCompare 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpCompareAccess().getAlternatives()); }
		(rule__OpCompare__Alternatives)
		{ after(grammarAccess.getOpCompareAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXOtherOperatorExpression
entryRuleXOtherOperatorExpression
:
{ before(grammarAccess.getXOtherOperatorExpressionRule()); }
	 ruleXOtherOperatorExpression
{ after(grammarAccess.getXOtherOperatorExpressionRule()); } 
	 EOF 
;

// Rule XOtherOperatorExpression
ruleXOtherOperatorExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXOtherOperatorExpressionAccess().getGroup()); }
		(rule__XOtherOperatorExpression__Group__0)
		{ after(grammarAccess.getXOtherOperatorExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXAdditiveExpression
entryRuleXAdditiveExpression
:
{ before(grammarAccess.getXAdditiveExpressionRule()); }
	 ruleXAdditiveExpression
{ after(grammarAccess.getXAdditiveExpressionRule()); } 
	 EOF 
;

// Rule XAdditiveExpression
ruleXAdditiveExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXAdditiveExpressionAccess().getGroup()); }
		(rule__XAdditiveExpression__Group__0)
		{ after(grammarAccess.getXAdditiveExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpAdd
entryRuleOpAdd
:
{ before(grammarAccess.getOpAddRule()); }
	 ruleOpAdd
{ after(grammarAccess.getOpAddRule()); } 
	 EOF 
;

// Rule OpAdd
ruleOpAdd 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpAddAccess().getAlternatives()); }
		(rule__OpAdd__Alternatives)
		{ after(grammarAccess.getOpAddAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXMultiplicativeExpression
entryRuleXMultiplicativeExpression
:
{ before(grammarAccess.getXMultiplicativeExpressionRule()); }
	 ruleXMultiplicativeExpression
{ after(grammarAccess.getXMultiplicativeExpressionRule()); } 
	 EOF 
;

// Rule XMultiplicativeExpression
ruleXMultiplicativeExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXMultiplicativeExpressionAccess().getGroup()); }
		(rule__XMultiplicativeExpression__Group__0)
		{ after(grammarAccess.getXMultiplicativeExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXUnaryOperation
entryRuleXUnaryOperation
:
{ before(grammarAccess.getXUnaryOperationRule()); }
	 ruleXUnaryOperation
{ after(grammarAccess.getXUnaryOperationRule()); } 
	 EOF 
;

// Rule XUnaryOperation
ruleXUnaryOperation 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXUnaryOperationAccess().getAlternatives()); }
		(rule__XUnaryOperation__Alternatives)
		{ after(grammarAccess.getXUnaryOperationAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleOpPostfix
entryRuleOpPostfix
:
{ before(grammarAccess.getOpPostfixRule()); }
	 ruleOpPostfix
{ after(grammarAccess.getOpPostfixRule()); } 
	 EOF 
;

// Rule OpPostfix
ruleOpPostfix 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getOpPostfixAccess().getAlternatives()); }
		(rule__OpPostfix__Alternatives)
		{ after(grammarAccess.getOpPostfixAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXSetLiteral
entryRuleXSetLiteral
:
{ before(grammarAccess.getXSetLiteralRule()); }
	 ruleXSetLiteral
{ after(grammarAccess.getXSetLiteralRule()); } 
	 EOF 
;

// Rule XSetLiteral
ruleXSetLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXSetLiteralAccess().getGroup()); }
		(rule__XSetLiteral__Group__0)
		{ after(grammarAccess.getXSetLiteralAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXListLiteral
entryRuleXListLiteral
:
{ before(grammarAccess.getXListLiteralRule()); }
	 ruleXListLiteral
{ after(grammarAccess.getXListLiteralRule()); } 
	 EOF 
;

// Rule XListLiteral
ruleXListLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXListLiteralAccess().getGroup()); }
		(rule__XListLiteral__Group__0)
		{ after(grammarAccess.getXListLiteralAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXExpressionInClosure
entryRuleXExpressionInClosure
:
{ before(grammarAccess.getXExpressionInClosureRule()); }
	 ruleXExpressionInClosure
{ after(grammarAccess.getXExpressionInClosureRule()); } 
	 EOF 
;

// Rule XExpressionInClosure
ruleXExpressionInClosure 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXExpressionInClosureAccess().getGroup()); }
		(rule__XExpressionInClosure__Group__0)
		{ after(grammarAccess.getXExpressionInClosureAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXParenthesizedExpression
entryRuleXParenthesizedExpression
:
{ before(grammarAccess.getXParenthesizedExpressionRule()); }
	 ruleXParenthesizedExpression
{ after(grammarAccess.getXParenthesizedExpressionRule()); } 
	 EOF 
;

// Rule XParenthesizedExpression
ruleXParenthesizedExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXParenthesizedExpressionAccess().getGroup()); }
		(rule__XParenthesizedExpression__Group__0)
		{ after(grammarAccess.getXParenthesizedExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXExpressionOrVarDeclaration
entryRuleXExpressionOrVarDeclaration
:
{ before(grammarAccess.getXExpressionOrVarDeclarationRule()); }
	 ruleXExpressionOrVarDeclaration
{ after(grammarAccess.getXExpressionOrVarDeclarationRule()); } 
	 EOF 
;

// Rule XExpressionOrVarDeclaration
ruleXExpressionOrVarDeclaration 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXExpressionOrVarDeclarationAccess().getAlternatives()); }
		(rule__XExpressionOrVarDeclaration__Alternatives)
		{ after(grammarAccess.getXExpressionOrVarDeclarationAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmFormalParameter
entryRuleJvmFormalParameter
:
{ before(grammarAccess.getJvmFormalParameterRule()); }
	 ruleJvmFormalParameter
{ after(grammarAccess.getJvmFormalParameterRule()); } 
	 EOF 
;

// Rule JvmFormalParameter
ruleJvmFormalParameter 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmFormalParameterAccess().getGroup()); }
		(rule__JvmFormalParameter__Group__0)
		{ after(grammarAccess.getJvmFormalParameterAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleFeatureCallID
entryRuleFeatureCallID
:
{ before(grammarAccess.getFeatureCallIDRule()); }
	 ruleFeatureCallID
{ after(grammarAccess.getFeatureCallIDRule()); } 
	 EOF 
;

// Rule FeatureCallID
ruleFeatureCallID 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getFeatureCallIDAccess().getAlternatives()); }
		(rule__FeatureCallID__Alternatives)
		{ after(grammarAccess.getFeatureCallIDAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleIdOrSuper
entryRuleIdOrSuper
:
{ before(grammarAccess.getIdOrSuperRule()); }
	 ruleIdOrSuper
{ after(grammarAccess.getIdOrSuperRule()); } 
	 EOF 
;

// Rule IdOrSuper
ruleIdOrSuper 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getIdOrSuperAccess().getAlternatives()); }
		(rule__IdOrSuper__Alternatives)
		{ after(grammarAccess.getIdOrSuperAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXBooleanLiteral
entryRuleXBooleanLiteral
:
{ before(grammarAccess.getXBooleanLiteralRule()); }
	 ruleXBooleanLiteral
{ after(grammarAccess.getXBooleanLiteralRule()); } 
	 EOF 
;

// Rule XBooleanLiteral
ruleXBooleanLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXBooleanLiteralAccess().getGroup()); }
		(rule__XBooleanLiteral__Group__0)
		{ after(grammarAccess.getXBooleanLiteralAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXNullLiteral
entryRuleXNullLiteral
:
{ before(grammarAccess.getXNullLiteralRule()); }
	 ruleXNullLiteral
{ after(grammarAccess.getXNullLiteralRule()); } 
	 EOF 
;

// Rule XNullLiteral
ruleXNullLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXNullLiteralAccess().getGroup()); }
		(rule__XNullLiteral__Group__0)
		{ after(grammarAccess.getXNullLiteralAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXNumberLiteral
entryRuleXNumberLiteral
:
{ before(grammarAccess.getXNumberLiteralRule()); }
	 ruleXNumberLiteral
{ after(grammarAccess.getXNumberLiteralRule()); } 
	 EOF 
;

// Rule XNumberLiteral
ruleXNumberLiteral 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXNumberLiteralAccess().getGroup()); }
		(rule__XNumberLiteral__Group__0)
		{ after(grammarAccess.getXNumberLiteralAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXThrowExpression
entryRuleXThrowExpression
:
{ before(grammarAccess.getXThrowExpressionRule()); }
	 ruleXThrowExpression
{ after(grammarAccess.getXThrowExpressionRule()); } 
	 EOF 
;

// Rule XThrowExpression
ruleXThrowExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXThrowExpressionAccess().getGroup()); }
		(rule__XThrowExpression__Group__0)
		{ after(grammarAccess.getXThrowExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleXReturnExpression
entryRuleXReturnExpression
:
{ before(grammarAccess.getXReturnExpressionRule()); }
	 ruleXReturnExpression
{ after(grammarAccess.getXReturnExpressionRule()); } 
	 EOF 
;

// Rule XReturnExpression
ruleXReturnExpression 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getXReturnExpressionAccess().getGroup()); }
		(rule__XReturnExpression__Group__0)
		{ after(grammarAccess.getXReturnExpressionAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleQualifiedName
entryRuleQualifiedName
:
{ before(grammarAccess.getQualifiedNameRule()); }
	 ruleQualifiedName
{ after(grammarAccess.getQualifiedNameRule()); } 
	 EOF 
;

// Rule QualifiedName
ruleQualifiedName 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getQualifiedNameAccess().getGroup()); }
		(rule__QualifiedName__Group__0)
		{ after(grammarAccess.getQualifiedNameAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleNumber
entryRuleNumber
@init { 
	HiddenTokens myHiddenTokenState = ((XtextTokenStream)input).setHiddenTokens();
}
:
{ before(grammarAccess.getNumberRule()); }
	 ruleNumber
{ after(grammarAccess.getNumberRule()); } 
	 EOF 
;
finally {
	myHiddenTokenState.restore();
}

// Rule Number
ruleNumber 
	@init {
		HiddenTokens myHiddenTokenState = ((XtextTokenStream)input).setHiddenTokens();
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getNumberAccess().getAlternatives()); }
		(rule__Number__Alternatives)
		{ after(grammarAccess.getNumberAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
	myHiddenTokenState.restore();
}

// Entry rule entryRuleArrayBrackets
entryRuleArrayBrackets
:
{ before(grammarAccess.getArrayBracketsRule()); }
	 ruleArrayBrackets
{ after(grammarAccess.getArrayBracketsRule()); } 
	 EOF 
;

// Rule ArrayBrackets
ruleArrayBrackets 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getArrayBracketsAccess().getGroup()); }
		(rule__ArrayBrackets__Group__0)
		{ after(grammarAccess.getArrayBracketsAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmParameterizedTypeReference
entryRuleJvmParameterizedTypeReference
:
{ before(grammarAccess.getJvmParameterizedTypeReferenceRule()); }
	 ruleJvmParameterizedTypeReference
{ after(grammarAccess.getJvmParameterizedTypeReferenceRule()); } 
	 EOF 
;

// Rule JvmParameterizedTypeReference
ruleJvmParameterizedTypeReference 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup()); }
		(rule__JvmParameterizedTypeReference__Group__0)
		{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmArgumentTypeReference
entryRuleJvmArgumentTypeReference
:
{ before(grammarAccess.getJvmArgumentTypeReferenceRule()); }
	 ruleJvmArgumentTypeReference
{ after(grammarAccess.getJvmArgumentTypeReferenceRule()); } 
	 EOF 
;

// Rule JvmArgumentTypeReference
ruleJvmArgumentTypeReference 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmArgumentTypeReferenceAccess().getAlternatives()); }
		(rule__JvmArgumentTypeReference__Alternatives)
		{ after(grammarAccess.getJvmArgumentTypeReferenceAccess().getAlternatives()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmWildcardTypeReference
entryRuleJvmWildcardTypeReference
:
{ before(grammarAccess.getJvmWildcardTypeReferenceRule()); }
	 ruleJvmWildcardTypeReference
{ after(grammarAccess.getJvmWildcardTypeReferenceRule()); } 
	 EOF 
;

// Rule JvmWildcardTypeReference
ruleJvmWildcardTypeReference 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getGroup()); }
		(rule__JvmWildcardTypeReference__Group__0)
		{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmUpperBound
entryRuleJvmUpperBound
:
{ before(grammarAccess.getJvmUpperBoundRule()); }
	 ruleJvmUpperBound
{ after(grammarAccess.getJvmUpperBoundRule()); } 
	 EOF 
;

// Rule JvmUpperBound
ruleJvmUpperBound 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmUpperBoundAccess().getGroup()); }
		(rule__JvmUpperBound__Group__0)
		{ after(grammarAccess.getJvmUpperBoundAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmUpperBoundAnded
entryRuleJvmUpperBoundAnded
:
{ before(grammarAccess.getJvmUpperBoundAndedRule()); }
	 ruleJvmUpperBoundAnded
{ after(grammarAccess.getJvmUpperBoundAndedRule()); } 
	 EOF 
;

// Rule JvmUpperBoundAnded
ruleJvmUpperBoundAnded 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmUpperBoundAndedAccess().getGroup()); }
		(rule__JvmUpperBoundAnded__Group__0)
		{ after(grammarAccess.getJvmUpperBoundAndedAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmLowerBound
entryRuleJvmLowerBound
:
{ before(grammarAccess.getJvmLowerBoundRule()); }
	 ruleJvmLowerBound
{ after(grammarAccess.getJvmLowerBoundRule()); } 
	 EOF 
;

// Rule JvmLowerBound
ruleJvmLowerBound 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmLowerBoundAccess().getGroup()); }
		(rule__JvmLowerBound__Group__0)
		{ after(grammarAccess.getJvmLowerBoundAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleJvmLowerBoundAnded
entryRuleJvmLowerBoundAnded
:
{ before(grammarAccess.getJvmLowerBoundAndedRule()); }
	 ruleJvmLowerBoundAnded
{ after(grammarAccess.getJvmLowerBoundAndedRule()); } 
	 EOF 
;

// Rule JvmLowerBoundAnded
ruleJvmLowerBoundAnded 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getJvmLowerBoundAndedAccess().getGroup()); }
		(rule__JvmLowerBoundAnded__Group__0)
		{ after(grammarAccess.getJvmLowerBoundAndedAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleQualifiedNameWithWildcard
entryRuleQualifiedNameWithWildcard
:
{ before(grammarAccess.getQualifiedNameWithWildcardRule()); }
	 ruleQualifiedNameWithWildcard
{ after(grammarAccess.getQualifiedNameWithWildcardRule()); } 
	 EOF 
;

// Rule QualifiedNameWithWildcard
ruleQualifiedNameWithWildcard 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getQualifiedNameWithWildcardAccess().getGroup()); }
		(rule__QualifiedNameWithWildcard__Group__0)
		{ after(grammarAccess.getQualifiedNameWithWildcardAccess().getGroup()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleValidID
entryRuleValidID
:
{ before(grammarAccess.getValidIDRule()); }
	 ruleValidID
{ after(grammarAccess.getValidIDRule()); } 
	 EOF 
;

// Rule ValidID
ruleValidID 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		{ before(grammarAccess.getValidIDAccess().getIDTerminalRuleCall()); }
		RULE_ID
		{ after(grammarAccess.getValidIDAccess().getIDTerminalRuleCall()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

// Entry rule entryRuleQualifiedNameInStaticImport
entryRuleQualifiedNameInStaticImport
:
{ before(grammarAccess.getQualifiedNameInStaticImportRule()); }
	 ruleQualifiedNameInStaticImport
{ after(grammarAccess.getQualifiedNameInStaticImportRule()); } 
	 EOF 
;

// Rule QualifiedNameInStaticImport
ruleQualifiedNameInStaticImport 
	@init {
		int stackSize = keepStackSize();
	}
	:
	(
		(
			{ before(grammarAccess.getQualifiedNameInStaticImportAccess().getGroup()); }
			(rule__QualifiedNameInStaticImport__Group__0)
			{ after(grammarAccess.getQualifiedNameInStaticImportAccess().getGroup()); }
		)
		(
			{ before(grammarAccess.getQualifiedNameInStaticImportAccess().getGroup()); }
			(rule__QualifiedNameInStaticImport__Group__0)*
			{ after(grammarAccess.getQualifiedNameInStaticImportAccess().getGroup()); }
		)
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCProblem__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0()); }
		(rule__CbCProblem__UnorderedGroup_0)
		{ after(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getCbCProblemAccess().getBlockStatementParserRuleCall_1()); }
		ruleBlockStatement
		{ after(grammarAccess.getCbCProblemAccess().getBlockStatementParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__AbstractStatement__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getAbstractStatementAccess().getAbstractStatement_ImplParserRuleCall_0()); }
		ruleAbstractStatement_Impl
		{ after(grammarAccess.getAbstractStatementAccess().getAbstractStatement_ImplParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getAbstractStatementAccess().getInlineBlockStatementParserRuleCall_1()); }
		ruleInlineBlockStatement
		{ after(grammarAccess.getAbstractStatementAccess().getInlineBlockStatementParserRuleCall_1()); }
	)
	|
	(
		{ before(grammarAccess.getAbstractStatementAccess().getCompositionStatementParserRuleCall_2()); }
		ruleCompositionStatement
		{ after(grammarAccess.getAbstractStatementAccess().getCompositionStatementParserRuleCall_2()); }
	)
	|
	(
		{ before(grammarAccess.getAbstractStatementAccess().getSkipStatementParserRuleCall_3()); }
		ruleSkipStatement
		{ after(grammarAccess.getAbstractStatementAccess().getSkipStatementParserRuleCall_3()); }
	)
	|
	(
		{ before(grammarAccess.getAbstractStatementAccess().getSelectionStatementParserRuleCall_4()); }
		ruleSelectionStatement
		{ after(grammarAccess.getAbstractStatementAccess().getSelectionStatementParserRuleCall_4()); }
	)
	|
	(
		{ before(grammarAccess.getAbstractStatementAccess().getSmallRepetitionStatementParserRuleCall_5()); }
		ruleSmallRepetitionStatement
		{ after(grammarAccess.getAbstractStatementAccess().getSmallRepetitionStatementParserRuleCall_5()); }
	)
	|
	(
		{ before(grammarAccess.getAbstractStatementAccess().getMethodStatementParserRuleCall_6()); }
		ruleMethodStatement
		{ after(grammarAccess.getAbstractStatementAccess().getMethodStatementParserRuleCall_6()); }
	)
	|
	(
		{ before(grammarAccess.getAbstractStatementAccess().getReturnStatementParserRuleCall_7()); }
		ruleReturnStatement
		{ after(grammarAccess.getAbstractStatementAccess().getReturnStatementParserRuleCall_7()); }
	)
	|
	(
		{ before(grammarAccess.getAbstractStatementAccess().getStrengthWeakStatementParserRuleCall_8()); }
		ruleStrengthWeakStatement
		{ after(grammarAccess.getAbstractStatementAccess().getStrengthWeakStatementParserRuleCall_8()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__EString__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getEStringAccess().getSTRINGTerminalRuleCall_0()); }
		RULE_STRING
		{ after(grammarAccess.getEStringAccess().getSTRINGTerminalRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getEStringAccess().getIDTerminalRuleCall_1()); }
		RULE_ID
		{ after(grammarAccess.getEStringAccess().getIDTerminalRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Alternatives_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCodeStringAccess().getGroup_3_0()); }
		(rule__CodeString__Group_3_0__0)
		{ after(grammarAccess.getCodeStringAccess().getGroup_3_0()); }
	)
	|
	(
		{ before(grammarAccess.getCodeStringAccess().getBooleanParserRuleCall_3_1()); }
		(ruleBoolean)
		{ after(grammarAccess.getCodeStringAccess().getBooleanParserRuleCall_3_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Alternatives_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_0()); }
		RULE_INT
		{ after(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getVariableStringAccess().getGroup_1_1()); }
		(rule__VariableString__Group_1_1__0)
		{ after(grammarAccess.getVariableStringAccess().getGroup_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Alternatives_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableStringAccess().getGroup_1_1_1_0()); }
		(rule__VariableString__Group_1_1_1_0__0)
		{ after(grammarAccess.getVariableStringAccess().getGroup_1_1_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getVariableStringAccess().getGroup_1_1_1_1()); }
		(rule__VariableString__Group_1_1_1_1__0)
		{ after(grammarAccess.getVariableStringAccess().getGroup_1_1_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Alternatives_1_1_1_0_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_1_0_1_1_0()); }
		ruleQualifiedName
		{ after(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_1_0_1_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_1_1_0_1_1_1()); }
		RULE_INT
		{ after(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_1_1_0_1_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Alternatives_1_1_1_0_1_2_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_1_0_1_2_2_0()); }
		ruleQualifiedName
		{ after(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_1_0_1_2_2_0()); }
	)
	|
	(
		{ before(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_1_1_0_1_2_2_1()); }
		RULE_INT
		{ after(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_1_1_0_1_2_2_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Alternatives_1_1_1_1_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_1_1_2_0()); }
		ruleQualifiedName
		{ after(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_1_1_2_0()); }
	)
	|
	(
		{ before(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_1_1_1_2_1()); }
		RULE_INT
		{ after(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_1_1_1_2_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Alternatives_1_1_1_1_3_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_1_1_3_2_0()); }
		ruleQualifiedName
		{ after(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_1_1_3_2_0()); }
	)
	|
	(
		{ before(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_1_1_1_3_2_1()); }
		RULE_INT
		{ after(grammarAccess.getVariableStringAccess().getINTTerminalRuleCall_1_1_1_1_3_2_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Boolean__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getBooleanAccess().getTrueKeyword_0()); }
		'true'
		{ after(grammarAccess.getBooleanAccess().getTrueKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getBooleanAccess().getFalseKeyword_1()); }
		'false'
		{ after(grammarAccess.getBooleanAccess().getFalseKeyword_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Operation__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOperationAccess().getPlusSignKeyword_0()); }
		'+'
		{ after(grammarAccess.getOperationAccess().getPlusSignKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getOperationAccess().getHyphenMinusKeyword_1()); }
		'-'
		{ after(grammarAccess.getOperationAccess().getHyphenMinusKeyword_1()); }
	)
	|
	(
		{ before(grammarAccess.getOperationAccess().getAsteriskKeyword_2()); }
		'*'
		{ after(grammarAccess.getOperationAccess().getAsteriskKeyword_2()); }
	)
	|
	(
		{ before(grammarAccess.getOperationAccess().getSolidusKeyword_3()); }
		'/'
		{ after(grammarAccess.getOperationAccess().getSolidusKeyword_3()); }
	)
	|
	(
		{ before(grammarAccess.getOperationAccess().getPercentSignKeyword_4()); }
		'%'
		{ after(grammarAccess.getOperationAccess().getPercentSignKeyword_4()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSingleStatement__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXJSemicolonStatementParserRuleCall_0()); }
		ruleXJSemicolonStatement
		{ after(grammarAccess.getXJSingleStatementAccess().getXJSemicolonStatementParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXSwitchExpressionParserRuleCall_1()); }
		ruleXSwitchExpression
		{ after(grammarAccess.getXJSingleStatementAccess().getXSwitchExpressionParserRuleCall_1()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXIfExpressionParserRuleCall_2()); }
		ruleXIfExpression
		{ after(grammarAccess.getXJSingleStatementAccess().getXIfExpressionParserRuleCall_2()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXForLoopExpressionParserRuleCall_3()); }
		(ruleXForLoopExpression)
		{ after(grammarAccess.getXJSingleStatementAccess().getXForLoopExpressionParserRuleCall_3()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXBasicForLoopExpressionParserRuleCall_4()); }
		ruleXBasicForLoopExpression
		{ after(grammarAccess.getXJSingleStatementAccess().getXBasicForLoopExpressionParserRuleCall_4()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getJMLWhileExpressionParserRuleCall_5()); }
		ruleJMLWhileExpression
		{ after(grammarAccess.getXJSingleStatementAccess().getJMLWhileExpressionParserRuleCall_5()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXJTryWithResourcesStatementParserRuleCall_6()); }
		(ruleXJTryWithResourcesStatement)
		{ after(grammarAccess.getXJSingleStatementAccess().getXJTryWithResourcesStatementParserRuleCall_6()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXTryCatchFinallyExpressionParserRuleCall_7()); }
		ruleXTryCatchFinallyExpression
		{ after(grammarAccess.getXJSingleStatementAccess().getXTryCatchFinallyExpressionParserRuleCall_7()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXSynchronizedExpressionParserRuleCall_8()); }
		(ruleXSynchronizedExpression)
		{ after(grammarAccess.getXJSingleStatementAccess().getXSynchronizedExpressionParserRuleCall_8()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getXJEmptyStatementParserRuleCall_9()); }
		ruleXJEmptyStatement
		{ after(grammarAccess.getXJSingleStatementAccess().getXJEmptyStatementParserRuleCall_9()); }
	)
	|
	(
		{ before(grammarAccess.getXJSingleStatementAccess().getInlineJavaBlockStatementParserRuleCall_10()); }
		(ruleInlineJavaBlockStatement)
		{ after(grammarAccess.getXJSingleStatementAccess().getInlineJavaBlockStatementParserRuleCall_10()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Alternatives_5_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getBlockStatementAccess().getJavaStatementAssignment_5_1_0()); }
		(rule__BlockStatement__JavaStatementAssignment_5_1_0)
		{ after(grammarAccess.getBlockStatementAccess().getJavaStatementAssignment_5_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getBlockStatementAccess().getGroup_5_1_1()); }
		(rule__BlockStatement__Group_5_1_1__0)
		{ after(grammarAccess.getBlockStatementAccess().getGroup_5_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableOrMethodNameAccess().getGroup_0()); }
		(rule__VariableOrMethodName__Group_0__0)
		{ after(grammarAccess.getVariableOrMethodNameAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getVariableOrMethodNameAccess().getGroup_1()); }
		(rule__VariableOrMethodName__Group_1__0)
		{ after(grammarAccess.getVariableOrMethodNameAccess().getGroup_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getConcatAccess().getGroup_1_0_0()); }
		(rule__Concat__Group_1_0_0__0)
		{ after(grammarAccess.getConcatAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getConcatAccess().getGroup_1_0_1()); }
		(rule__Concat__Group_1_0_1__0)
		{ after(grammarAccess.getConcatAccess().getGroup_1_0_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getForeachAccess().getExistsParserRuleCall_0()); }
		ruleExists
		{ after(grammarAccess.getForeachAccess().getExistsParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getForeachAccess().getGroup_1()); }
		(rule__Foreach__Group_1__0)
		{ after(grammarAccess.getForeachAccess().getGroup_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getExistsAccess().getRelationParserRuleCall_0()); }
		ruleRelation
		{ after(grammarAccess.getExistsAccess().getRelationParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getExistsAccess().getGroup_1()); }
		(rule__Exists__Group_1__0)
		{ after(grammarAccess.getExistsAccess().getGroup_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getRelationAccess().getGroup_1_0_0()); }
		(rule__Relation__Group_1_0_0__0)
		{ after(grammarAccess.getRelationAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getRelationAccess().getGroup_1_0_1()); }
		(rule__Relation__Group_1_0_1__0)
		{ after(grammarAccess.getRelationAccess().getGroup_1_0_1()); }
	)
	|
	(
		{ before(grammarAccess.getRelationAccess().getGroup_1_0_2()); }
		(rule__Relation__Group_1_0_2__0)
		{ after(grammarAccess.getRelationAccess().getGroup_1_0_2()); }
	)
	|
	(
		{ before(grammarAccess.getRelationAccess().getGroup_1_0_3()); }
		(rule__Relation__Group_1_0_3__0)
		{ after(grammarAccess.getRelationAccess().getGroup_1_0_3()); }
	)
	|
	(
		{ before(grammarAccess.getRelationAccess().getGroup_1_0_4()); }
		(rule__Relation__Group_1_0_4__0)
		{ after(grammarAccess.getRelationAccess().getGroup_1_0_4()); }
	)
	|
	(
		{ before(grammarAccess.getRelationAccess().getGroup_1_0_5()); }
		(rule__Relation__Group_1_0_5__0)
		{ after(grammarAccess.getRelationAccess().getGroup_1_0_5()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getMultiMathOperationAccess().getGroup_1_0_0()); }
		(rule__MultiMathOperation__Group_1_0_0__0)
		{ after(grammarAccess.getMultiMathOperationAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getMultiMathOperationAccess().getGroup_1_0_1()); }
		(rule__MultiMathOperation__Group_1_0_1__0)
		{ after(grammarAccess.getMultiMathOperationAccess().getGroup_1_0_1()); }
	)
	|
	(
		{ before(grammarAccess.getMultiMathOperationAccess().getGroup_1_0_2()); }
		(rule__MultiMathOperation__Group_1_0_2__0)
		{ after(grammarAccess.getMultiMathOperationAccess().getGroup_1_0_2()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getAddMathOperationAccess().getGroup_1_0_0()); }
		(rule__AddMathOperation__Group_1_0_0__0)
		{ after(grammarAccess.getAddMathOperationAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getAddMathOperationAccess().getGroup_1_0_1()); }
		(rule__AddMathOperation__Group_1_0_1__0)
		{ after(grammarAccess.getAddMathOperationAccess().getGroup_1_0_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_0()); }
		(rule__PrimaryExpression__Group_0__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_1()); }
		(rule__PrimaryExpression__Group_1__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_1()); }
	)
	|
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_2()); }
		(rule__PrimaryExpression__Group_2__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_2()); }
	)
	|
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_3()); }
		(rule__PrimaryExpression__Group_3__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_3()); }
	)
	|
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_4()); }
		(rule__PrimaryExpression__Group_4__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_4()); }
	)
	|
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_5()); }
		(rule__PrimaryExpression__Group_5__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_5()); }
	)
	|
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_6()); }
		(rule__PrimaryExpression__Group_6__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_6()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Alternatives_0_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getFalseKeyword_0_1_0_0()); }
		'false'
		{ after(grammarAccess.getPrimaryExpressionAccess().getFalseKeyword_0_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getIsTrueAssignment_0_1_0_1()); }
		(rule__PrimaryExpression__IsTrueAssignment_0_1_0_1)
		{ after(grammarAccess.getPrimaryExpressionAccess().getIsTrueAssignment_0_1_0_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Alternatives_2_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_2_3_0()); }
		(rule__PrimaryExpression__Group_2_3_0__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_2_3_0()); }
	)
	|
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_2_3_1()); }
		(rule__PrimaryExpression__Group_2_3_1__0)
		{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_2_3_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLImplicationAccess().getGroup_1_0_0()); }
		(rule__JMLImplication__Group_1_0_0__0)
		{ after(grammarAccess.getJMLImplicationAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLImplicationAccess().getGroup_1_0_1()); }
		(rule__JMLImplication__Group_1_0_1__0)
		{ after(grammarAccess.getJMLImplicationAccess().getGroup_1_0_1()); }
	)
	|
	(
		{ before(grammarAccess.getJMLImplicationAccess().getGroup_1_0_2()); }
		(rule__JMLImplication__Group_1_0_2__0)
		{ after(grammarAccess.getJMLImplicationAccess().getGroup_1_0_2()); }
	)
	|
	(
		{ before(grammarAccess.getJMLImplicationAccess().getGroup_1_0_3()); }
		(rule__JMLImplication__Group_1_0_3__0)
		{ after(grammarAccess.getJMLImplicationAccess().getGroup_1_0_3()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLConcatAccess().getGroup_1_0_0()); }
		(rule__JMLConcat__Group_1_0_0__0)
		{ after(grammarAccess.getJMLConcatAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLConcatAccess().getGroup_1_0_1()); }
		(rule__JMLConcat__Group_1_0_1__0)
		{ after(grammarAccess.getJMLConcatAccess().getGroup_1_0_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLForeachAccess().getJMLExistsParserRuleCall_0()); }
		ruleJMLExists
		{ after(grammarAccess.getJMLForeachAccess().getJMLExistsParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLForeachAccess().getGroup_1()); }
		(rule__JMLForeach__Group_1__0)
		{ after(grammarAccess.getJMLForeachAccess().getGroup_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLExistsAccess().getJMLRelationParserRuleCall_0()); }
		ruleJMLRelation
		{ after(grammarAccess.getJMLExistsAccess().getJMLRelationParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLExistsAccess().getGroup_1()); }
		(rule__JMLExists__Group_1__0)
		{ after(grammarAccess.getJMLExistsAccess().getGroup_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLRelationAccess().getGroup_1_0_0()); }
		(rule__JMLRelation__Group_1_0_0__0)
		{ after(grammarAccess.getJMLRelationAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLRelationAccess().getGroup_1_0_1()); }
		(rule__JMLRelation__Group_1_0_1__0)
		{ after(grammarAccess.getJMLRelationAccess().getGroup_1_0_1()); }
	)
	|
	(
		{ before(grammarAccess.getJMLRelationAccess().getGroup_1_0_2()); }
		(rule__JMLRelation__Group_1_0_2__0)
		{ after(grammarAccess.getJMLRelationAccess().getGroup_1_0_2()); }
	)
	|
	(
		{ before(grammarAccess.getJMLRelationAccess().getGroup_1_0_3()); }
		(rule__JMLRelation__Group_1_0_3__0)
		{ after(grammarAccess.getJMLRelationAccess().getGroup_1_0_3()); }
	)
	|
	(
		{ before(grammarAccess.getJMLRelationAccess().getGroup_1_0_4()); }
		(rule__JMLRelation__Group_1_0_4__0)
		{ after(grammarAccess.getJMLRelationAccess().getGroup_1_0_4()); }
	)
	|
	(
		{ before(grammarAccess.getJMLRelationAccess().getGroup_1_0_5()); }
		(rule__JMLRelation__Group_1_0_5__0)
		{ after(grammarAccess.getJMLRelationAccess().getGroup_1_0_5()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLMultiMathOperationAccess().getGroup_1_0_0()); }
		(rule__JMLMultiMathOperation__Group_1_0_0__0)
		{ after(grammarAccess.getJMLMultiMathOperationAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLMultiMathOperationAccess().getGroup_1_0_1()); }
		(rule__JMLMultiMathOperation__Group_1_0_1__0)
		{ after(grammarAccess.getJMLMultiMathOperationAccess().getGroup_1_0_1()); }
	)
	|
	(
		{ before(grammarAccess.getJMLMultiMathOperationAccess().getGroup_1_0_2()); }
		(rule__JMLMultiMathOperation__Group_1_0_2__0)
		{ after(grammarAccess.getJMLMultiMathOperationAccess().getGroup_1_0_2()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLAddMathOperationAccess().getGroup_1_0_0()); }
		(rule__JMLAddMathOperation__Group_1_0_0__0)
		{ after(grammarAccess.getJMLAddMathOperationAccess().getGroup_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLAddMathOperationAccess().getGroup_1_0_1()); }
		(rule__JMLAddMathOperation__Group_1_0_1__0)
		{ after(grammarAccess.getJMLAddMathOperationAccess().getGroup_1_0_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_0()); }
		(rule__JMLPrimaryExpression__Group_0__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_1()); }
		(rule__JMLPrimaryExpression__Group_1__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_1()); }
	)
	|
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2()); }
		(rule__JMLPrimaryExpression__Group_2__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2()); }
	)
	|
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_3()); }
		(rule__JMLPrimaryExpression__Group_3__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_3()); }
	)
	|
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_4()); }
		(rule__JMLPrimaryExpression__Group_4__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_4()); }
	)
	|
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_5()); }
		(rule__JMLPrimaryExpression__Group_5__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_5()); }
	)
	|
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_6()); }
		(rule__JMLPrimaryExpression__Group_6__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_6()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Alternatives_0_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getFalseKeyword_0_1_0_0()); }
		'false'
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getFalseKeyword_0_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getIsTrueAssignment_0_1_0_1()); }
		(rule__JMLPrimaryExpression__IsTrueAssignment_0_1_0_1)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getIsTrueAssignment_0_1_0_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Alternatives_2_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2_3_0()); }
		(rule__JMLPrimaryExpression__Group_2_3_0__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2_3_0()); }
	)
	|
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2_3_1()); }
		(rule__JMLPrimaryExpression__Group_2_3_1__0)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2_3_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJStatementOrBlock__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJStatementOrBlockAccess().getXBlockExpressionParserRuleCall_0()); }
		(ruleXBlockExpression)
		{ after(grammarAccess.getXJStatementOrBlockAccess().getXBlockExpressionParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getXJStatementOrBlockAccess().getXJSingleStatementParserRuleCall_1()); }
		ruleXJSingleStatement
		{ after(grammarAccess.getXJStatementOrBlockAccess().getXJSingleStatementParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Alternatives_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getGroup_1_0()); }
		(rule__XImportDeclaration__Group_1_0__0)
		{ after(grammarAccess.getXImportDeclarationAccess().getGroup_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getImportedTypeAssignment_1_1()); }
		(rule__XImportDeclaration__ImportedTypeAssignment_1_1)
		{ after(grammarAccess.getXImportDeclarationAccess().getImportedTypeAssignment_1_1()); }
	)
	|
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getImportedNamespaceAssignment_1_2()); }
		(rule__XImportDeclaration__ImportedNamespaceAssignment_1_2)
		{ after(grammarAccess.getXImportDeclarationAccess().getImportedNamespaceAssignment_1_2()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Alternatives_1_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getWildcardAssignment_1_0_2_0()); }
		(rule__XImportDeclaration__WildcardAssignment_1_0_2_0)
		{ after(grammarAccess.getXImportDeclarationAccess().getWildcardAssignment_1_0_2_0()); }
	)
	|
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getMemberNameAssignment_1_0_2_1()); }
		(rule__XImportDeclaration__MemberNameAssignment_1_0_2_1)
		{ after(grammarAccess.getXImportDeclarationAccess().getMemberNameAssignment_1_0_2_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSemicolonStatement__ExpressionAlternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJSemicolonStatementAccess().getExpressionXJBranchingStatementParserRuleCall_1_0_0()); }
		ruleXJBranchingStatement
		{ after(grammarAccess.getXJSemicolonStatementAccess().getExpressionXJBranchingStatementParserRuleCall_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getXJSemicolonStatementAccess().getExpressionXExpressionOrVarDeclarationParserRuleCall_1_0_1()); }
		ruleXExpressionOrVarDeclaration
		{ after(grammarAccess.getXJSemicolonStatementAccess().getExpressionXExpressionOrVarDeclarationParserRuleCall_1_0_1()); }
	)
	|
	(
		{ before(grammarAccess.getXJSemicolonStatementAccess().getExpressionXDoWhileExpressionParserRuleCall_1_0_2()); }
		ruleXDoWhileExpression
		{ after(grammarAccess.getXJSemicolonStatementAccess().getExpressionXDoWhileExpressionParserRuleCall_1_0_2()); }
	)
	|
	(
		{ before(grammarAccess.getXJSemicolonStatementAccess().getExpressionXReturnExpressionParserRuleCall_1_0_3()); }
		ruleXReturnExpression
		{ after(grammarAccess.getXJSemicolonStatementAccess().getExpressionXReturnExpressionParserRuleCall_1_0_3()); }
	)
	|
	(
		{ before(grammarAccess.getXJSemicolonStatementAccess().getExpressionXThrowExpressionParserRuleCall_1_0_4()); }
		ruleXThrowExpression
		{ after(grammarAccess.getXJSemicolonStatementAccess().getExpressionXThrowExpressionParserRuleCall_1_0_4()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJBranchingStatement__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJBranchingStatementAccess().getGroup_0()); }
		(rule__XJBranchingStatement__Group_0__0)
		{ after(grammarAccess.getXJBranchingStatementAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getXJBranchingStatementAccess().getGroup_1()); }
		(rule__XJBranchingStatement__Group_1__0)
		{ after(grammarAccess.getXJBranchingStatementAccess().getGroup_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getGroup_0()); }
		(rule__XAssignment__Group_0__0)
		{ after(grammarAccess.getXAssignmentAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getXAssignmentAccess().getGroup_1()); }
		(rule__XAssignment__Group_1__0)
		{ after(grammarAccess.getXAssignmentAccess().getGroup_1()); }
	)
	|
	(
		{ before(grammarAccess.getXAssignmentAccess().getGroup_2()); }
		(rule__XAssignment__Group_2__0)
		{ after(grammarAccess.getXAssignmentAccess().getGroup_2()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Alternatives_2_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getGroup_2_1_0()); }
		(rule__XAssignment__Group_2_1_0__0)
		{ after(grammarAccess.getXAssignmentAccess().getGroup_2_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getXAssignmentAccess().getGroup_2_1_1()); }
		(rule__XAssignment__Group_2_1_1__0)
		{ after(grammarAccess.getXAssignmentAccess().getGroup_2_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getPlusSignEqualsSignKeyword_0()); }
		'+='
		{ after(grammarAccess.getOpMultiAssignAccess().getPlusSignEqualsSignKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getHyphenMinusEqualsSignKeyword_1()); }
		'-='
		{ after(grammarAccess.getOpMultiAssignAccess().getHyphenMinusEqualsSignKeyword_1()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getAsteriskEqualsSignKeyword_2()); }
		'*='
		{ after(grammarAccess.getOpMultiAssignAccess().getAsteriskEqualsSignKeyword_2()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getSolidusEqualsSignKeyword_3()); }
		'/='
		{ after(grammarAccess.getOpMultiAssignAccess().getSolidusEqualsSignKeyword_3()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getPercentSignEqualsSignKeyword_4()); }
		'%='
		{ after(grammarAccess.getOpMultiAssignAccess().getPercentSignEqualsSignKeyword_4()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getAmpersandEqualsSignKeyword_5()); }
		'&='
		{ after(grammarAccess.getOpMultiAssignAccess().getAmpersandEqualsSignKeyword_5()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getVerticalLineEqualsSignKeyword_6()); }
		'|='
		{ after(grammarAccess.getOpMultiAssignAccess().getVerticalLineEqualsSignKeyword_6()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getCircumflexAccentEqualsSignKeyword_7()); }
		'^='
		{ after(grammarAccess.getOpMultiAssignAccess().getCircumflexAccentEqualsSignKeyword_7()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getGroup_8()); }
		(rule__OpMultiAssign__Group_8__0)
		{ after(grammarAccess.getOpMultiAssignAccess().getGroup_8()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAssignAccess().getGroup_9()); }
		(rule__OpMultiAssign__Group_9__0)
		{ after(grammarAccess.getOpMultiAssignAccess().getGroup_9()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXCastedExpressionAccess().getGroup_0()); }
		(rule__XCastedExpression__Group_0__0)
		{ after(grammarAccess.getXCastedExpressionAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getXCastedExpressionAccess().getXPostfixOperationParserRuleCall_1()); }
		ruleXPostfixOperation
		{ after(grammarAccess.getXCastedExpressionAccess().getXPostfixOperationParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getGroup_0()); }
		(rule__XPostfixOperation__Group_0__0)
		{ after(grammarAccess.getXPostfixOperationAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getGroup_1()); }
		(rule__XPostfixOperation__Group_1__0)
		{ after(grammarAccess.getXPostfixOperationAccess().getGroup_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Alternatives_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_0()); }
		(rule__XPostfixOperation__Group_1_1_0__0)
		{ after(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_1()); }
		(rule__XPostfixOperation__Group_1_1_1__0)
		{ after(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Alternatives_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_0()); }
		(rule__XMemberFeatureCall__Group_1_0__0)
		{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1()); }
		(rule__XMemberFeatureCall__Group_1_1__0)
		{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1()); }
	)
	|
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2()); }
		(rule__XMemberFeatureCall__Group_1_2__0)
		{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Alternatives_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_0()); }
		(rule__XTryCatchFinallyExpression__Group_3_0__0)
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_0()); }
	)
	|
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_1()); }
		(rule__XTryCatchFinallyExpression__Group_3_1__0)
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPrimaryExpression__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXPrimaryExpressionAccess().getXJArrayConstructorCallParserRuleCall_0()); }
		(ruleXJArrayConstructorCall)
		{ after(grammarAccess.getXPrimaryExpressionAccess().getXJArrayConstructorCallParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getXPrimaryExpressionAccess().getXConstructorCallParserRuleCall_1()); }
		ruleXConstructorCall
		{ after(grammarAccess.getXPrimaryExpressionAccess().getXConstructorCallParserRuleCall_1()); }
	)
	|
	(
		{ before(grammarAccess.getXPrimaryExpressionAccess().getXJFeatureCallWithArrayAccessParserRuleCall_2()); }
		ruleXJFeatureCallWithArrayAccess
		{ after(grammarAccess.getXPrimaryExpressionAccess().getXJFeatureCallWithArrayAccessParserRuleCall_2()); }
	)
	|
	(
		{ before(grammarAccess.getXPrimaryExpressionAccess().getXLiteralParserRuleCall_3()); }
		ruleXLiteral
		{ after(grammarAccess.getXPrimaryExpressionAccess().getXLiteralParserRuleCall_3()); }
	)
	|
	(
		{ before(grammarAccess.getXPrimaryExpressionAccess().getXParenthesizedExpressionParserRuleCall_4()); }
		ruleXParenthesizedExpression
		{ after(grammarAccess.getXPrimaryExpressionAccess().getXParenthesizedExpressionParserRuleCall_4()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XLiteral__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXLiteralAccess().getXBooleanLiteralParserRuleCall_0()); }
		ruleXBooleanLiteral
		{ after(grammarAccess.getXLiteralAccess().getXBooleanLiteralParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getXLiteralAccess().getXNumberLiteralParserRuleCall_1()); }
		ruleXNumberLiteral
		{ after(grammarAccess.getXLiteralAccess().getXNumberLiteralParserRuleCall_1()); }
	)
	|
	(
		{ before(grammarAccess.getXLiteralAccess().getXNullLiteralParserRuleCall_2()); }
		ruleXNullLiteral
		{ after(grammarAccess.getXLiteralAccess().getXNullLiteralParserRuleCall_2()); }
	)
	|
	(
		{ before(grammarAccess.getXLiteralAccess().getXCharLiteralParserRuleCall_3()); }
		ruleXCharLiteral
		{ after(grammarAccess.getXLiteralAccess().getXCharLiteralParserRuleCall_3()); }
	)
	|
	(
		{ before(grammarAccess.getXLiteralAccess().getXStringLiteralParserRuleCall_4()); }
		ruleXStringLiteral
		{ after(grammarAccess.getXLiteralAccess().getXStringLiteralParserRuleCall_4()); }
	)
	|
	(
		{ before(grammarAccess.getXLiteralAccess().getXJArrayLiteralParserRuleCall_5()); }
		ruleXJArrayLiteral
		{ after(grammarAccess.getXLiteralAccess().getXJArrayLiteralParserRuleCall_5()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpEquality__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpEqualityAccess().getEqualsSignEqualsSignKeyword_0()); }
		'=='
		{ after(grammarAccess.getOpEqualityAccess().getEqualsSignEqualsSignKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpEqualityAccess().getExclamationMarkEqualsSignKeyword_1()); }
		'!='
		{ after(grammarAccess.getOpEqualityAccess().getExclamationMarkEqualsSignKeyword_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMulti__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpMultiAccess().getAsteriskKeyword_0()); }
		'*'
		{ after(grammarAccess.getOpMultiAccess().getAsteriskKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAccess().getSolidusKeyword_1()); }
		'/'
		{ after(grammarAccess.getOpMultiAccess().getSolidusKeyword_1()); }
	)
	|
	(
		{ before(grammarAccess.getOpMultiAccess().getPercentSignKeyword_2()); }
		'%'
		{ after(grammarAccess.getOpMultiAccess().getPercentSignKeyword_2()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpOtherAccess().getGroup_0()); }
		(rule__OpOther__Group_0__0)
		{ after(grammarAccess.getOpOtherAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpOtherAccess().getLessThanSignLessThanSignKeyword_1()); }
		'<<'
		{ after(grammarAccess.getOpOtherAccess().getLessThanSignLessThanSignKeyword_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Alternatives_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpOtherAccess().getGroup_0_1_0()); }
		(rule__OpOther__Group_0_1_0__0)
		{ after(grammarAccess.getOpOtherAccess().getGroup_0_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpOtherAccess().getGreaterThanSignKeyword_0_1_1()); }
		'>'
		{ after(grammarAccess.getOpOtherAccess().getGreaterThanSignKeyword_0_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpUnary__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpUnaryAccess().getExclamationMarkKeyword_0()); }
		'!'
		{ after(grammarAccess.getOpUnaryAccess().getExclamationMarkKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpUnaryAccess().getHyphenMinusKeyword_1()); }
		'-'
		{ after(grammarAccess.getOpUnaryAccess().getHyphenMinusKeyword_1()); }
	)
	|
	(
		{ before(grammarAccess.getOpUnaryAccess().getPlusSignKeyword_2()); }
		'+'
		{ after(grammarAccess.getOpUnaryAccess().getPlusSignKeyword_2()); }
	)
	|
	(
		{ before(grammarAccess.getOpUnaryAccess().getTildeKeyword_3()); }
		'~'
		{ after(grammarAccess.getOpUnaryAccess().getTildeKeyword_3()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0()); }
		(rule__XAnnotationElementValueOrCommaList__Group_0__0)
		{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getXAnnotationOrExpressionParserRuleCall_1()); }
		ruleXAnnotationOrExpression
		{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getXAnnotationOrExpressionParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationElementValueAccess().getGroup_0()); }
		(rule__XAnnotationElementValue__Group_0__0)
		{ after(grammarAccess.getXAnnotationElementValueAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getXAnnotationElementValueAccess().getXAnnotationOrExpressionParserRuleCall_1()); }
		ruleXAnnotationOrExpression
		{ after(grammarAccess.getXAnnotationElementValueAccess().getXAnnotationOrExpressionParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Alternatives_3_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationAccess().getGroup_3_1_0()); }
		(rule__XAnnotation__Group_3_1_0__0)
		{ after(grammarAccess.getXAnnotationAccess().getGroup_3_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getXAnnotationAccess().getValueAssignment_3_1_1()); }
		(rule__XAnnotation__ValueAssignment_3_1_1)
		{ after(grammarAccess.getXAnnotationAccess().getValueAssignment_3_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationOrExpression__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationOrExpressionAccess().getXAnnotationParserRuleCall_0()); }
		ruleXAnnotation
		{ after(grammarAccess.getXAnnotationOrExpressionAccess().getXAnnotationParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getXAnnotationOrExpressionAccess().getXExpressionParserRuleCall_1()); }
		ruleXExpression
		{ after(grammarAccess.getXAnnotationOrExpressionAccess().getXExpressionParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Alternatives_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0()); }
		(rule__XRelationalExpression__Group_1_0__0)
		{ after(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1()); }
		(rule__XRelationalExpression__Group_1_1__0)
		{ after(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpCompare__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpCompareAccess().getGreaterThanSignEqualsSignKeyword_0()); }
		'>='
		{ after(grammarAccess.getOpCompareAccess().getGreaterThanSignEqualsSignKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpCompareAccess().getGroup_1()); }
		(rule__OpCompare__Group_1__0)
		{ after(grammarAccess.getOpCompareAccess().getGroup_1()); }
	)
	|
	(
		{ before(grammarAccess.getOpCompareAccess().getGreaterThanSignKeyword_2()); }
		'>'
		{ after(grammarAccess.getOpCompareAccess().getGreaterThanSignKeyword_2()); }
	)
	|
	(
		{ before(grammarAccess.getOpCompareAccess().getLessThanSignKeyword_3()); }
		'<'
		{ after(grammarAccess.getOpCompareAccess().getLessThanSignKeyword_3()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpAdd__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpAddAccess().getPlusSignKeyword_0()); }
		'+'
		{ after(grammarAccess.getOpAddAccess().getPlusSignKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpAddAccess().getHyphenMinusKeyword_1()); }
		'-'
		{ after(grammarAccess.getOpAddAccess().getHyphenMinusKeyword_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XUnaryOperation__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXUnaryOperationAccess().getGroup_0()); }
		(rule__XUnaryOperation__Group_0__0)
		{ after(grammarAccess.getXUnaryOperationAccess().getGroup_0()); }
	)
	|
	(
		{ before(grammarAccess.getXUnaryOperationAccess().getXCastedExpressionParserRuleCall_1()); }
		ruleXCastedExpression
		{ after(grammarAccess.getXUnaryOperationAccess().getXCastedExpressionParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpPostfix__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getOpPostfixAccess().getPlusSignPlusSignKeyword_0()); }
		'++'
		{ after(grammarAccess.getOpPostfixAccess().getPlusSignPlusSignKeyword_0()); }
	)
	|
	(
		{ before(grammarAccess.getOpPostfixAccess().getHyphenMinusHyphenMinusKeyword_1()); }
		'--'
		{ after(grammarAccess.getOpPostfixAccess().getHyphenMinusHyphenMinusKeyword_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XExpressionOrVarDeclaration__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXExpressionOrVarDeclarationAccess().getXVariableDeclarationParserRuleCall_0()); }
		(ruleXVariableDeclaration)
		{ after(grammarAccess.getXExpressionOrVarDeclarationAccess().getXVariableDeclarationParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getXExpressionOrVarDeclarationAccess().getXExpressionParserRuleCall_1()); }
		ruleXExpression
		{ after(grammarAccess.getXExpressionOrVarDeclarationAccess().getXExpressionParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__FeatureCallID__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getFeatureCallIDAccess().getValidIDParserRuleCall_0()); }
		ruleValidID
		{ after(grammarAccess.getFeatureCallIDAccess().getValidIDParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getFeatureCallIDAccess().getExtendsKeyword_1()); }
		'extends'
		{ after(grammarAccess.getFeatureCallIDAccess().getExtendsKeyword_1()); }
	)
	|
	(
		{ before(grammarAccess.getFeatureCallIDAccess().getStaticKeyword_2()); }
		'static'
		{ after(grammarAccess.getFeatureCallIDAccess().getStaticKeyword_2()); }
	)
	|
	(
		{ before(grammarAccess.getFeatureCallIDAccess().getImportKeyword_3()); }
		'import'
		{ after(grammarAccess.getFeatureCallIDAccess().getImportKeyword_3()); }
	)
	|
	(
		{ before(grammarAccess.getFeatureCallIDAccess().getExtensionKeyword_4()); }
		'extension'
		{ after(grammarAccess.getFeatureCallIDAccess().getExtensionKeyword_4()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__IdOrSuper__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getIdOrSuperAccess().getFeatureCallIDParserRuleCall_0()); }
		ruleFeatureCallID
		{ after(grammarAccess.getIdOrSuperAccess().getFeatureCallIDParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getIdOrSuperAccess().getSuperKeyword_1()); }
		'super'
		{ after(grammarAccess.getIdOrSuperAccess().getSuperKeyword_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBooleanLiteral__Alternatives_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBooleanLiteralAccess().getFalseKeyword_1_0()); }
		'false'
		{ after(grammarAccess.getXBooleanLiteralAccess().getFalseKeyword_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getXBooleanLiteralAccess().getIsTrueAssignment_1_1()); }
		(rule__XBooleanLiteral__IsTrueAssignment_1_1)
		{ after(grammarAccess.getXBooleanLiteralAccess().getIsTrueAssignment_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getNumberAccess().getHEXTerminalRuleCall_0()); }
		RULE_HEX
		{ after(grammarAccess.getNumberAccess().getHEXTerminalRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getNumberAccess().getGroup_1()); }
		(rule__Number__Group_1__0)
		{ after(grammarAccess.getNumberAccess().getGroup_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Alternatives_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getNumberAccess().getINTTerminalRuleCall_1_0_0()); }
		RULE_INT
		{ after(grammarAccess.getNumberAccess().getINTTerminalRuleCall_1_0_0()); }
	)
	|
	(
		{ before(grammarAccess.getNumberAccess().getDECIMALTerminalRuleCall_1_0_1()); }
		RULE_DECIMAL
		{ after(grammarAccess.getNumberAccess().getDECIMALTerminalRuleCall_1_0_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Alternatives_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getNumberAccess().getINTTerminalRuleCall_1_1_1_0()); }
		RULE_INT
		{ after(grammarAccess.getNumberAccess().getINTTerminalRuleCall_1_1_1_0()); }
	)
	|
	(
		{ before(grammarAccess.getNumberAccess().getDECIMALTerminalRuleCall_1_1_1_1()); }
		RULE_DECIMAL
		{ after(grammarAccess.getNumberAccess().getDECIMALTerminalRuleCall_1_1_1_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmArgumentTypeReference__Alternatives
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmArgumentTypeReferenceAccess().getJvmTypeReferenceParserRuleCall_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJvmArgumentTypeReferenceAccess().getJvmTypeReferenceParserRuleCall_0()); }
	)
	|
	(
		{ before(grammarAccess.getJvmArgumentTypeReferenceAccess().getJvmWildcardTypeReferenceParserRuleCall_1()); }
		ruleJvmWildcardTypeReference
		{ after(grammarAccess.getJvmArgumentTypeReferenceAccess().getJvmWildcardTypeReferenceParserRuleCall_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Alternatives_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getGroup_2_0()); }
		(rule__JvmWildcardTypeReference__Group_2_0__0)
		{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getGroup_2_0()); }
	)
	|
	(
		{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getGroup_2_1()); }
		(rule__JvmWildcardTypeReference__Group_2_1__0)
		{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getGroup_2_1()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__0__Impl
	rule__CbCFormula__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getFormulaKeyword_0()); }
	'Formula'
	{ after(grammarAccess.getCbCFormulaAccess().getFormulaKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__1__Impl
	rule__CbCFormula__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getNameAssignment_1()); }
	(rule__CbCFormula__NameAssignment_1)
	{ after(grammarAccess.getCbCFormulaAccess().getNameAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__2__Impl
	rule__CbCFormula__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getPreKeyword_2()); }
	'Pre:'
	{ after(grammarAccess.getCbCFormulaAccess().getPreKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__3__Impl
	rule__CbCFormula__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getLeftParenthesisKeyword_3()); }
	'('
	{ after(grammarAccess.getCbCFormulaAccess().getLeftParenthesisKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__4__Impl
	rule__CbCFormula__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getPreConditionAssignment_4()); }
	(rule__CbCFormula__PreConditionAssignment_4)
	{ after(grammarAccess.getCbCFormulaAccess().getPreConditionAssignment_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__5__Impl
	rule__CbCFormula__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getRightParenthesisKeyword_5()); }
	')'
	{ after(grammarAccess.getCbCFormulaAccess().getRightParenthesisKeyword_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__6__Impl
	rule__CbCFormula__Group__7
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getLeftCurlyBracketKeyword_6()); }
	'{'
	{ after(grammarAccess.getCbCFormulaAccess().getLeftCurlyBracketKeyword_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__7__Impl
	rule__CbCFormula__Group__8
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getStatementAssignment_7()); }
	(rule__CbCFormula__StatementAssignment_7)
	{ after(grammarAccess.getCbCFormulaAccess().getStatementAssignment_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__8
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__8__Impl
	rule__CbCFormula__Group__9
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__8__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getRightCurlyBracketKeyword_8()); }
	'}'
	{ after(grammarAccess.getCbCFormulaAccess().getRightCurlyBracketKeyword_8()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__9
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__9__Impl
	rule__CbCFormula__Group__10
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__9__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getPostKeyword_9()); }
	'Post:'
	{ after(grammarAccess.getCbCFormulaAccess().getPostKeyword_9()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__10
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__10__Impl
	rule__CbCFormula__Group__11
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__10__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getLeftParenthesisKeyword_10()); }
	'('
	{ after(grammarAccess.getCbCFormulaAccess().getLeftParenthesisKeyword_10()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__11
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__11__Impl
	rule__CbCFormula__Group__12
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__11__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getPostConditionAssignment_11()); }
	(rule__CbCFormula__PostConditionAssignment_11)
	{ after(grammarAccess.getCbCFormulaAccess().getPostConditionAssignment_11()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__12
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCFormula__Group__12__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__Group__12__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCbCFormulaAccess().getRightParenthesisKeyword_12()); }
	')'
	{ after(grammarAccess.getCbCFormulaAccess().getRightParenthesisKeyword_12()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__CodeString__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group__0__Impl
	rule__CodeString__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getVariableStringParserRuleCall_0()); }
	ruleVariableString
	{ after(grammarAccess.getCodeStringAccess().getVariableStringParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group__1__Impl
	rule__CodeString__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getGroup_1()); }
	(rule__CodeString__Group_1__0)*
	{ after(grammarAccess.getCodeStringAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group__2__Impl
	rule__CodeString__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getEqualsSignKeyword_2()); }
	'='
	{ after(grammarAccess.getCodeStringAccess().getEqualsSignKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group__3__Impl
	rule__CodeString__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getAlternatives_3()); }
	(rule__CodeString__Alternatives_3)
	{ after(grammarAccess.getCodeStringAccess().getAlternatives_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getSemicolonKeyword_4()); }
	';'
	{ after(grammarAccess.getCodeStringAccess().getSemicolonKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__CodeString__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group_1__0__Impl
	rule__CodeString__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getOperationParserRuleCall_1_0()); }
	ruleOperation
	{ after(grammarAccess.getCodeStringAccess().getOperationParserRuleCall_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getVariableStringParserRuleCall_1_1()); }
	ruleVariableString
	{ after(grammarAccess.getCodeStringAccess().getVariableStringParserRuleCall_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__CodeString__Group_3_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group_3_0__0__Impl
	rule__CodeString__Group_3_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getLeftParenthesisKeyword_3_0_0()); }
	('(')?
	{ after(grammarAccess.getCodeStringAccess().getLeftParenthesisKeyword_3_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group_3_0__1__Impl
	rule__CodeString__Group_3_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getVariableStringParserRuleCall_3_0_1()); }
	ruleVariableString
	{ after(grammarAccess.getCodeStringAccess().getVariableStringParserRuleCall_3_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group_3_0__2__Impl
	rule__CodeString__Group_3_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getGroup_3_0_2()); }
	(rule__CodeString__Group_3_0_2__0)*
	{ after(grammarAccess.getCodeStringAccess().getGroup_3_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group_3_0__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getRightParenthesisKeyword_3_0_3()); }
	(')')?
	{ after(grammarAccess.getCodeStringAccess().getRightParenthesisKeyword_3_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__CodeString__Group_3_0_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group_3_0_2__0__Impl
	rule__CodeString__Group_3_0_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getOperationParserRuleCall_3_0_2_0()); }
	ruleOperation
	{ after(grammarAccess.getCodeStringAccess().getOperationParserRuleCall_3_0_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CodeString__Group_3_0_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__CodeString__Group_3_0_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCodeStringAccess().getVariableStringParserRuleCall_3_0_2_1()); }
	ruleVariableString
	{ after(grammarAccess.getCodeStringAccess().getVariableStringParserRuleCall_3_0_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableString__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group__0__Impl
	rule__VariableString__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_0()); }
	('-')?
	{ after(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getAlternatives_1()); }
	(rule__VariableString__Alternatives_1)
	{ after(grammarAccess.getVariableStringAccess().getAlternatives_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableString__Group_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1__0__Impl
	rule__VariableString__Group_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_0()); }
	ruleQualifiedName
	{ after(grammarAccess.getVariableStringAccess().getQualifiedNameParserRuleCall_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1()); }
	(rule__VariableString__Alternatives_1_1_1)?
	{ after(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableString__Group_1_1_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0__0__Impl
	rule__VariableString__Group_1_1_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getLeftParenthesisKeyword_1_1_1_0_0()); }
	'('
	{ after(grammarAccess.getVariableStringAccess().getLeftParenthesisKeyword_1_1_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0__1__Impl
	rule__VariableString__Group_1_1_1_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getGroup_1_1_1_0_1()); }
	(rule__VariableString__Group_1_1_1_0_1__0)?
	{ after(grammarAccess.getVariableStringAccess().getGroup_1_1_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getRightParenthesisKeyword_1_1_1_0_2()); }
	')'
	{ after(grammarAccess.getVariableStringAccess().getRightParenthesisKeyword_1_1_1_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableString__Group_1_1_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0_1__0__Impl
	rule__VariableString__Group_1_1_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_1_1_1_0_1_0()); }
	('-')?
	{ after(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_1_1_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0_1__1__Impl
	rule__VariableString__Group_1_1_1_0_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1_0_1_1()); }
	(rule__VariableString__Alternatives_1_1_1_0_1_1)
	{ after(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0_1__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getGroup_1_1_1_0_1_2()); }
	(rule__VariableString__Group_1_1_1_0_1_2__0)*
	{ after(grammarAccess.getVariableStringAccess().getGroup_1_1_1_0_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableString__Group_1_1_1_0_1_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0_1_2__0__Impl
	rule__VariableString__Group_1_1_1_0_1_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getCommaKeyword_1_1_1_0_1_2_0()); }
	','
	{ after(grammarAccess.getVariableStringAccess().getCommaKeyword_1_1_1_0_1_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0_1_2__1__Impl
	rule__VariableString__Group_1_1_1_0_1_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_1_1_1_0_1_2_1()); }
	('-')?
	{ after(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_1_1_1_0_1_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_0_1_2__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_0_1_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1_0_1_2_2()); }
	(rule__VariableString__Alternatives_1_1_1_0_1_2_2)
	{ after(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1_0_1_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableString__Group_1_1_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_1__0__Impl
	rule__VariableString__Group_1_1_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getLeftSquareBracketKeyword_1_1_1_1_0()); }
	'['
	{ after(grammarAccess.getVariableStringAccess().getLeftSquareBracketKeyword_1_1_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_1__1__Impl
	rule__VariableString__Group_1_1_1_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_1_1_1_1_1()); }
	('-')?
	{ after(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_1_1_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_1__2__Impl
	rule__VariableString__Group_1_1_1_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1_1_2()); }
	(rule__VariableString__Alternatives_1_1_1_1_2)
	{ after(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_1__3__Impl
	rule__VariableString__Group_1_1_1_1__4
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getGroup_1_1_1_1_3()); }
	(rule__VariableString__Group_1_1_1_1_3__0)*
	{ after(grammarAccess.getVariableStringAccess().getGroup_1_1_1_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_1__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getRightSquareBracketKeyword_1_1_1_1_4()); }
	']'
	{ after(grammarAccess.getVariableStringAccess().getRightSquareBracketKeyword_1_1_1_1_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableString__Group_1_1_1_1_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_1_3__0__Impl
	rule__VariableString__Group_1_1_1_1_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getOperationParserRuleCall_1_1_1_1_3_0()); }
	ruleOperation
	{ after(grammarAccess.getVariableStringAccess().getOperationParserRuleCall_1_1_1_1_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_1_3__1__Impl
	rule__VariableString__Group_1_1_1_1_3__2
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_1_1_1_1_3_1()); }
	('-')?
	{ after(grammarAccess.getVariableStringAccess().getHyphenMinusKeyword_1_1_1_1_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1_3__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableString__Group_1_1_1_1_3__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableString__Group_1_1_1_1_3__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1_1_3_2()); }
	(rule__VariableString__Alternatives_1_1_1_1_3_2)
	{ after(grammarAccess.getVariableStringAccess().getAlternatives_1_1_1_1_3_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__AbstractStatement_Impl__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AbstractStatement_Impl__Group__0__Impl
	rule__AbstractStatement_Impl__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__AbstractStatement_Impl__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAbstractStatement_ImplAccess().getAbstractStatementAction_0()); }
	()
	{ after(grammarAccess.getAbstractStatement_ImplAccess().getAbstractStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__AbstractStatement_Impl__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AbstractStatement_Impl__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__AbstractStatement_Impl__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAbstractStatement_ImplAccess().getNameAssignment_1()); }
	(rule__AbstractStatement_Impl__NameAssignment_1)
	{ after(grammarAccess.getAbstractStatement_ImplAccess().getNameAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__MethodStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MethodStatement__Group__0__Impl
	rule__MethodStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__MethodStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMethodStatementAccess().getMethodStatementAction_0()); }
	()
	{ after(grammarAccess.getMethodStatementAccess().getMethodStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__MethodStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MethodStatement__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__MethodStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMethodStatementAccess().getNameAssignment_1()); }
	(rule__MethodStatement__NameAssignment_1)
	{ after(grammarAccess.getMethodStatementAccess().getNameAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__ReturnStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__ReturnStatement__Group__0__Impl
	rule__ReturnStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__ReturnStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getReturnStatementAccess().getReturnStatementAction_0()); }
	()
	{ after(grammarAccess.getReturnStatementAccess().getReturnStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__ReturnStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__ReturnStatement__Group__1__Impl
	rule__ReturnStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__ReturnStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getReturnStatementAccess().getReturnKeyword_1()); }
	'Return'
	{ after(grammarAccess.getReturnStatementAccess().getReturnKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__ReturnStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__ReturnStatement__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__ReturnStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getReturnStatementAccess().getNameAssignment_2()); }
	(rule__ReturnStatement__NameAssignment_2)
	{ after(grammarAccess.getReturnStatementAccess().getNameAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__StrengthWeakStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__0__Impl
	rule__StrengthWeakStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getStrengthWeakStatementAction_0()); }
	()
	{ after(grammarAccess.getStrengthWeakStatementAccess().getStrengthWeakStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__1__Impl
	rule__StrengthWeakStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getPreKeyword_1()); }
	'Pre:'
	{ after(grammarAccess.getStrengthWeakStatementAccess().getPreKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__2__Impl
	rule__StrengthWeakStatement__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getLeftParenthesisKeyword_2()); }
	'('
	{ after(grammarAccess.getStrengthWeakStatementAccess().getLeftParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__3__Impl
	rule__StrengthWeakStatement__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getWeakPreConditionAssignment_3()); }
	(rule__StrengthWeakStatement__WeakPreConditionAssignment_3)
	{ after(grammarAccess.getStrengthWeakStatementAccess().getWeakPreConditionAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__4__Impl
	rule__StrengthWeakStatement__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getRightParenthesisKeyword_4()); }
	')'
	{ after(grammarAccess.getStrengthWeakStatementAccess().getRightParenthesisKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__5__Impl
	rule__StrengthWeakStatement__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getLeftCurlyBracketKeyword_5()); }
	'{'
	{ after(grammarAccess.getStrengthWeakStatementAccess().getLeftCurlyBracketKeyword_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__6__Impl
	rule__StrengthWeakStatement__Group__7
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getNameAssignment_6()); }
	(rule__StrengthWeakStatement__NameAssignment_6)
	{ after(grammarAccess.getStrengthWeakStatementAccess().getNameAssignment_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__7__Impl
	rule__StrengthWeakStatement__Group__8
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getRightCurlyBracketKeyword_7()); }
	'}'
	{ after(grammarAccess.getStrengthWeakStatementAccess().getRightCurlyBracketKeyword_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__8
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__8__Impl
	rule__StrengthWeakStatement__Group__9
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__8__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getPostKeyword_8()); }
	'Post:'
	{ after(grammarAccess.getStrengthWeakStatementAccess().getPostKeyword_8()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__9
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__9__Impl
	rule__StrengthWeakStatement__Group__10
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__9__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getLeftParenthesisKeyword_9()); }
	'('
	{ after(grammarAccess.getStrengthWeakStatementAccess().getLeftParenthesisKeyword_9()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__10
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__10__Impl
	rule__StrengthWeakStatement__Group__11
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__10__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getStrongPostConditionAssignment_10()); }
	(rule__StrengthWeakStatement__StrongPostConditionAssignment_10)
	{ after(grammarAccess.getStrengthWeakStatementAccess().getStrongPostConditionAssignment_10()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__11
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__StrengthWeakStatement__Group__11__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__Group__11__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getStrengthWeakStatementAccess().getRightParenthesisKeyword_11()); }
	')'
	{ after(grammarAccess.getStrengthWeakStatementAccess().getRightParenthesisKeyword_11()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Condition__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Condition__Group__0__Impl
	rule__Condition__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Condition__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConditionAccess().getConditionAction_0()); }
	()
	{ after(grammarAccess.getConditionAccess().getConditionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Condition__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Condition__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Condition__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConditionAccess().getConditionAssignment_1()); }
	(rule__Condition__ConditionAssignment_1)
	{ after(grammarAccess.getConditionAccess().getConditionAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__CompositionStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__0__Impl
	rule__CompositionStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getCompositionStatementAction_0()); }
	()
	{ after(grammarAccess.getCompositionStatementAccess().getCompositionStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__1__Impl
	rule__CompositionStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getLeftCurlyBracketKeyword_1()); }
	'{'
	{ after(grammarAccess.getCompositionStatementAccess().getLeftCurlyBracketKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__2__Impl
	rule__CompositionStatement__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getFirstStatementAssignment_2()); }
	(rule__CompositionStatement__FirstStatementAssignment_2)
	{ after(grammarAccess.getCompositionStatementAccess().getFirstStatementAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__3__Impl
	rule__CompositionStatement__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getRightCurlyBracketKeyword_3()); }
	'}'
	{ after(grammarAccess.getCompositionStatementAccess().getRightCurlyBracketKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__4__Impl
	rule__CompositionStatement__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getIntmKeyword_4()); }
	'Intm:'
	{ after(grammarAccess.getCompositionStatementAccess().getIntmKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__5__Impl
	rule__CompositionStatement__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getLeftParenthesisKeyword_5()); }
	'('
	{ after(grammarAccess.getCompositionStatementAccess().getLeftParenthesisKeyword_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__6__Impl
	rule__CompositionStatement__Group__7
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getIntermediateConditionAssignment_6()); }
	(rule__CompositionStatement__IntermediateConditionAssignment_6)
	{ after(grammarAccess.getCompositionStatementAccess().getIntermediateConditionAssignment_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__7__Impl
	rule__CompositionStatement__Group__8
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getRightParenthesisKeyword_7()); }
	')'
	{ after(grammarAccess.getCompositionStatementAccess().getRightParenthesisKeyword_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__8
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__8__Impl
	rule__CompositionStatement__Group__9
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__8__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getLeftCurlyBracketKeyword_8()); }
	'{'
	{ after(grammarAccess.getCompositionStatementAccess().getLeftCurlyBracketKeyword_8()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__9
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__9__Impl
	rule__CompositionStatement__Group__10
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__9__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getSecondStatementAssignment_9()); }
	(rule__CompositionStatement__SecondStatementAssignment_9)
	{ after(grammarAccess.getCompositionStatementAccess().getSecondStatementAssignment_9()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__10
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CompositionStatement__Group__10__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__Group__10__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getCompositionStatementAccess().getRightCurlyBracketKeyword_10()); }
	'}'
	{ after(grammarAccess.getCompositionStatementAccess().getRightCurlyBracketKeyword_10()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JavaStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaStatement__Group__0__Impl
	rule__JavaStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaStatementAccess().getJavaStatementAction_0()); }
	()
	{ after(grammarAccess.getJavaStatementAccess().getJavaStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaStatement__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	(
		{ before(grammarAccess.getJavaStatementAccess().getStatementAssignment_1()); }
		(rule__JavaStatement__StatementAssignment_1)
		{ after(grammarAccess.getJavaStatementAccess().getStatementAssignment_1()); }
	)
	(
		{ before(grammarAccess.getJavaStatementAccess().getStatementAssignment_1()); }
		(rule__JavaStatement__StatementAssignment_1)*
		{ after(grammarAccess.getJavaStatementAccess().getStatementAssignment_1()); }
	)
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLWhileExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLWhileExpression__Group__0__Impl
	rule__JMLWhileExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLWhileExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLWhileExpressionAccess().getJMLWhileExpressionAction_0()); }
	()
	{ after(grammarAccess.getJMLWhileExpressionAccess().getJMLWhileExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLWhileExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLWhileExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLWhileExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLWhileExpressionAccess().getGroup_1()); }
	(rule__JMLWhileExpression__Group_1__0)
	{ after(grammarAccess.getJMLWhileExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLWhileExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLWhileExpression__Group_1__0__Impl
	rule__JMLWhileExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLWhileExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLWhileExpressionAccess().getAnnotationAssignment_1_0()); }
	(rule__JMLWhileExpression__AnnotationAssignment_1_0)
	{ after(grammarAccess.getJMLWhileExpressionAccess().getAnnotationAssignment_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLWhileExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLWhileExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLWhileExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLWhileExpressionAccess().getWhileAssignment_1_1()); }
	(rule__JMLWhileExpression__WhileAssignment_1_1)
	{ after(grammarAccess.getJMLWhileExpressionAccess().getWhileAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLAnnotation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group__0__Impl
	rule__JMLAnnotation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getSolidusAsteriskKeyword_0()); }
	'/*'
	{ after(grammarAccess.getJMLAnnotationAccess().getSolidusAsteriskKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group__1__Impl
	rule__JMLAnnotation__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getGroup_1()); }
	(rule__JMLAnnotation__Group_1__0)
	{ after(grammarAccess.getJMLAnnotationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group__2__Impl
	rule__JMLAnnotation__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_2()); }
	('@')*
	{ after(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getAsteriskSolidusKeyword_3()); }
	'*/'
	{ after(grammarAccess.getJMLAnnotationAccess().getAsteriskSolidusKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLAnnotation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1__0__Impl
	rule__JMLAnnotation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getGroup_1_0()); }
	(rule__JMLAnnotation__Group_1_0__0)
	{ after(grammarAccess.getJMLAnnotationAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getGroup_1_1()); }
	(rule__JMLAnnotation__Group_1_1__0)
	{ after(grammarAccess.getJMLAnnotationAccess().getGroup_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLAnnotation__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1_0__0__Impl
	rule__JMLAnnotation__Group_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	(
		{ before(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_1_0_0()); }
		('@')
		{ after(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_1_0_0()); }
	)
	(
		{ before(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_1_0_0()); }
		('@')*
		{ after(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_1_0_0()); }
	)
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1_0__1__Impl
	rule__JMLAnnotation__Group_1_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getRequiresKeyword_1_0_1()); }
	'requires'
	{ after(grammarAccess.getJMLAnnotationAccess().getRequiresKeyword_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1_0__2__Impl
	rule__JMLAnnotation__Group_1_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getRequiresAssignment_1_0_2()); }
	(rule__JMLAnnotation__RequiresAssignment_1_0_2)
	{ after(grammarAccess.getJMLAnnotationAccess().getRequiresAssignment_1_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1_0__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getSemicolonKeyword_1_0_3()); }
	';'
	{ after(grammarAccess.getJMLAnnotationAccess().getSemicolonKeyword_1_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLAnnotation__Group_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1_1__0__Impl
	rule__JMLAnnotation__Group_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	(
		{ before(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_1_1_0()); }
		('@')
		{ after(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_1_1_0()); }
	)
	(
		{ before(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_1_1_0()); }
		('@')*
		{ after(grammarAccess.getJMLAnnotationAccess().getCommercialAtKeyword_1_1_0()); }
	)
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1_1__1__Impl
	rule__JMLAnnotation__Group_1_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getEnsuresKeyword_1_1_1()); }
	'ensures'
	{ after(grammarAccess.getJMLAnnotationAccess().getEnsuresKeyword_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1_1__2__Impl
	rule__JMLAnnotation__Group_1_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getEnsuresAssignment_1_1_2()); }
	(rule__JMLAnnotation__EnsuresAssignment_1_1_2)
	{ after(grammarAccess.getJMLAnnotationAccess().getEnsuresAssignment_1_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAnnotation__Group_1_1__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__Group_1_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAnnotationAccess().getSemicolonKeyword_1_1_3()); }
	';'
	{ after(grammarAccess.getJMLAnnotationAccess().getSemicolonKeyword_1_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__LoopInvariantAnnotation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group__0__Impl
	rule__LoopInvariantAnnotation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getSolidusAsteriskKeyword_0()); }
	'/*'
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getSolidusAsteriskKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group__1__Impl
	rule__LoopInvariantAnnotation__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1()); }
	(rule__LoopInvariantAnnotation__Group_1__0)
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group__2__Impl
	rule__LoopInvariantAnnotation__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_2()); }
	('@')*
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getAsteriskSolidusKeyword_3()); }
	'*/'
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getAsteriskSolidusKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__LoopInvariantAnnotation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1__0__Impl
	rule__LoopInvariantAnnotation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1_0()); }
	(rule__LoopInvariantAnnotation__Group_1_0__0)
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1__1__Impl
	rule__LoopInvariantAnnotation__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1_1()); }
	(rule__LoopInvariantAnnotation__Group_1_1__0)?
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1_2()); }
	(rule__LoopInvariantAnnotation__Group_1_2__0)
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__LoopInvariantAnnotation__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_0__0__Impl
	rule__LoopInvariantAnnotation__Group_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_0_0()); }
		('@')
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_0_0()); }
	)
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_0_0()); }
		('@')*
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_0_0()); }
	)
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_0__1__Impl
	rule__LoopInvariantAnnotation__Group_1_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getLoop_invariantKeyword_1_0_1()); }
	'loop_invariant'
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getLoop_invariantKeyword_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_0__2__Impl
	rule__LoopInvariantAnnotation__Group_1_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getLoopInvariantAssignment_1_0_2()); }
	(rule__LoopInvariantAnnotation__LoopInvariantAssignment_1_0_2)
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getLoopInvariantAssignment_1_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_0__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getSemicolonKeyword_1_0_3()); }
	';'
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getSemicolonKeyword_1_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__LoopInvariantAnnotation__Group_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_1__0__Impl
	rule__LoopInvariantAnnotation__Group_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_1_0()); }
		('@')
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_1_0()); }
	)
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_1_0()); }
		('@')*
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_1_0()); }
	)
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_1__1__Impl
	rule__LoopInvariantAnnotation__Group_1_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableKeyword_1_1_1()); }
	'assignable'
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableKeyword_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_1__2__Impl
	rule__LoopInvariantAnnotation__Group_1_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableAssignment_1_1_2()); }
	(rule__LoopInvariantAnnotation__AssignableAssignment_1_1_2)
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableAssignment_1_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_1__3__Impl
	rule__LoopInvariantAnnotation__Group_1_1__4
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1_1_3()); }
	(rule__LoopInvariantAnnotation__Group_1_1_3__0)*
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getGroup_1_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_1__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getSemicolonKeyword_1_1_4()); }
	';'
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getSemicolonKeyword_1_1_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__LoopInvariantAnnotation__Group_1_1_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_1_3__0__Impl
	rule__LoopInvariantAnnotation__Group_1_1_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getCommaKeyword_1_1_3_0()); }
	','
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getCommaKeyword_1_1_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_1_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_1_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableAssignment_1_1_3_1()); }
	(rule__LoopInvariantAnnotation__AssignableAssignment_1_1_3_1)
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableAssignment_1_1_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__LoopInvariantAnnotation__Group_1_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_2__0__Impl
	rule__LoopInvariantAnnotation__Group_1_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_2_0()); }
		('@')
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_2_0()); }
	)
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_2_0()); }
		('@')*
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getCommercialAtKeyword_1_2_0()); }
	)
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_2__1__Impl
	rule__LoopInvariantAnnotation__Group_1_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getDecreasingKeyword_1_2_1()); }
	'decreasing'
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getDecreasingKeyword_1_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_2__2__Impl
	rule__LoopInvariantAnnotation__Group_1_2__3
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getDecreasesAssignment_1_2_2()); }
	(rule__LoopInvariantAnnotation__DecreasesAssignment_1_2_2)
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getDecreasesAssignment_1_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_2__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotation__Group_1_2__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__Group_1_2__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationAccess().getSemicolonKeyword_1_2_3()); }
	';'
	{ after(grammarAccess.getLoopInvariantAnnotationAccess().getSemicolonKeyword_1_2_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__LoopInvariantAnnotationStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotationStatement__Group__0__Impl
	rule__LoopInvariantAnnotationStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotationStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationStatementAccess().getLoopInvariantAnnotationStatementAction_0()); }
	()
	{ after(grammarAccess.getLoopInvariantAnnotationStatementAccess().getLoopInvariantAnnotationStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotationStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__LoopInvariantAnnotationStatement__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotationStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getLoopInvariantAnnotationStatementAccess().getJmlAnnotationAssignment_1()); }
	(rule__LoopInvariantAnnotationStatement__JmlAnnotationAssignment_1)
	{ after(grammarAccess.getLoopInvariantAnnotationStatementAccess().getJmlAnnotationAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__InlineBlockStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineBlockStatement__Group__0__Impl
	rule__InlineBlockStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineBlockStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineBlockStatementAccess().getInlineBlockStatementAction_0()); }
	()
	{ after(grammarAccess.getInlineBlockStatementAccess().getInlineBlockStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineBlockStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineBlockStatement__Group__1__Impl
	rule__InlineBlockStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineBlockStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineBlockStatementAccess().getBlockKeyword_1()); }
	'Block'
	{ after(grammarAccess.getInlineBlockStatementAccess().getBlockKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineBlockStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineBlockStatement__Group__2__Impl
	rule__InlineBlockStatement__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineBlockStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineBlockStatementAccess().getReferencesAssignment_2()); }
	(rule__InlineBlockStatement__ReferencesAssignment_2)
	{ after(grammarAccess.getInlineBlockStatementAccess().getReferencesAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineBlockStatement__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineBlockStatement__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineBlockStatement__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineBlockStatementAccess().getSemicolonKeyword_3()); }
	';'
	{ after(grammarAccess.getInlineBlockStatementAccess().getSemicolonKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__BlockStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group__0__Impl
	rule__BlockStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getBlockStatementAction_0()); }
	()
	{ after(grammarAccess.getBlockStatementAccess().getBlockStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group__1__Impl
	rule__BlockStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getBlockKeyword_1()); }
	'Block'
	{ after(grammarAccess.getBlockStatementAccess().getBlockKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group__2__Impl
	rule__BlockStatement__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getNameAssignment_2()); }
	(rule__BlockStatement__NameAssignment_2)
	{ after(grammarAccess.getBlockStatementAccess().getNameAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group__3__Impl
	rule__BlockStatement__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getColonKeyword_3()); }
	':'
	{ after(grammarAccess.getBlockStatementAccess().getColonKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group__4__Impl
	rule__BlockStatement__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getJmlAnnotationAssignment_4()); }
	(rule__BlockStatement__JmlAnnotationAssignment_4)
	{ after(grammarAccess.getBlockStatementAccess().getJmlAnnotationAssignment_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group__5__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getGroup_5()); }
	(rule__BlockStatement__Group_5__0)
	{ after(grammarAccess.getBlockStatementAccess().getGroup_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__BlockStatement__Group_5__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group_5__0__Impl
	rule__BlockStatement__Group_5__1
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group_5__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getLeftCurlyBracketKeyword_5_0()); }
	'{'
	{ after(grammarAccess.getBlockStatementAccess().getLeftCurlyBracketKeyword_5_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group_5__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group_5__1__Impl
	rule__BlockStatement__Group_5__2
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group_5__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getAlternatives_5_1()); }
	(rule__BlockStatement__Alternatives_5_1)
	{ after(grammarAccess.getBlockStatementAccess().getAlternatives_5_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group_5__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group_5__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group_5__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getRightCurlyBracketKeyword_5_2()); }
	'}'
	{ after(grammarAccess.getBlockStatementAccess().getRightCurlyBracketKeyword_5_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__BlockStatement__Group_5_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group_5_1_1__0__Impl
	rule__BlockStatement__Group_5_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group_5_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getCbCKeyword_5_1_1_0()); }
	'CbC:'
	{ after(grammarAccess.getBlockStatementAccess().getCbCKeyword_5_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group_5_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__BlockStatement__Group_5_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__Group_5_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getBlockStatementAccess().getCbcStatementAssignment_5_1_1_1()); }
	(rule__BlockStatement__CbcStatementAssignment_5_1_1_1)
	{ after(grammarAccess.getBlockStatementAccess().getCbcStatementAssignment_5_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__InlineJavaBlockStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineJavaBlockStatement__Group__0__Impl
	rule__InlineJavaBlockStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineJavaBlockStatementAccess().getInlineJavaBlockStatementAction_0()); }
	()
	{ after(grammarAccess.getInlineJavaBlockStatementAccess().getInlineJavaBlockStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineJavaBlockStatement__Group__1__Impl
	rule__InlineJavaBlockStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineJavaBlockStatementAccess().getJmlAnnotationAssignment_1()); }
	(rule__InlineJavaBlockStatement__JmlAnnotationAssignment_1)
	{ after(grammarAccess.getInlineJavaBlockStatementAccess().getJmlAnnotationAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineJavaBlockStatement__Group__2__Impl
	rule__InlineJavaBlockStatement__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineJavaBlockStatementAccess().getBlockKeyword_2()); }
	'Block'
	{ after(grammarAccess.getInlineJavaBlockStatementAccess().getBlockKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineJavaBlockStatement__Group__3__Impl
	rule__InlineJavaBlockStatement__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineJavaBlockStatementAccess().getReferencesAssignment_3()); }
	(rule__InlineJavaBlockStatement__ReferencesAssignment_3)
	{ after(grammarAccess.getInlineJavaBlockStatementAccess().getReferencesAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__InlineJavaBlockStatement__Group__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getInlineJavaBlockStatementAccess().getSemicolonKeyword_4()); }
	';'
	{ after(grammarAccess.getInlineJavaBlockStatementAccess().getSemicolonKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__SelectionStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__0__Impl
	rule__SelectionStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getSelectionStatementAction_0()); }
	()
	{ after(grammarAccess.getSelectionStatementAccess().getSelectionStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__1__Impl
	rule__SelectionStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getIfKeyword_1()); }
	'If'
	{ after(grammarAccess.getSelectionStatementAccess().getIfKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__2__Impl
	rule__SelectionStatement__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getLeftParenthesisKeyword_2()); }
	'('
	{ after(grammarAccess.getSelectionStatementAccess().getLeftParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__3__Impl
	rule__SelectionStatement__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getGuardsAssignment_3()); }
	(rule__SelectionStatement__GuardsAssignment_3)
	{ after(grammarAccess.getSelectionStatementAccess().getGuardsAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__4__Impl
	rule__SelectionStatement__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getRightParenthesisKeyword_4()); }
	')'
	{ after(grammarAccess.getSelectionStatementAccess().getRightParenthesisKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__5__Impl
	rule__SelectionStatement__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getThenKeyword_5()); }
	'Then'
	{ after(grammarAccess.getSelectionStatementAccess().getThenKeyword_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__6__Impl
	rule__SelectionStatement__Group__7
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getLeftCurlyBracketKeyword_6()); }
	'{'
	{ after(grammarAccess.getSelectionStatementAccess().getLeftCurlyBracketKeyword_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__7__Impl
	rule__SelectionStatement__Group__8
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getCommandsAssignment_7()); }
	(rule__SelectionStatement__CommandsAssignment_7)
	{ after(grammarAccess.getSelectionStatementAccess().getCommandsAssignment_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__8
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__8__Impl
	rule__SelectionStatement__Group__9
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__8__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getRightCurlyBracketKeyword_8()); }
	'}'
	{ after(grammarAccess.getSelectionStatementAccess().getRightCurlyBracketKeyword_8()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__9
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__9__Impl
	rule__SelectionStatement__Group__10
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__9__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getGroup_9()); }
	(rule__SelectionStatement__Group_9__0)*
	{ after(grammarAccess.getSelectionStatementAccess().getGroup_9()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__10
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group__10__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group__10__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getFiKeyword_10()); }
	'Fi'
	{ after(grammarAccess.getSelectionStatementAccess().getFiKeyword_10()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__SelectionStatement__Group_9__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group_9__0__Impl
	rule__SelectionStatement__Group_9__1
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getElseifKeyword_9_0()); }
	'Elseif'
	{ after(grammarAccess.getSelectionStatementAccess().getElseifKeyword_9_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group_9__1__Impl
	rule__SelectionStatement__Group_9__2
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getLeftParenthesisKeyword_9_1()); }
	'('
	{ after(grammarAccess.getSelectionStatementAccess().getLeftParenthesisKeyword_9_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group_9__2__Impl
	rule__SelectionStatement__Group_9__3
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getGuardsAssignment_9_2()); }
	(rule__SelectionStatement__GuardsAssignment_9_2)
	{ after(grammarAccess.getSelectionStatementAccess().getGuardsAssignment_9_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group_9__3__Impl
	rule__SelectionStatement__Group_9__4
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getRightParenthesisKeyword_9_3()); }
	')'
	{ after(grammarAccess.getSelectionStatementAccess().getRightParenthesisKeyword_9_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group_9__4__Impl
	rule__SelectionStatement__Group_9__5
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getThenKeyword_9_4()); }
	'Then'
	{ after(grammarAccess.getSelectionStatementAccess().getThenKeyword_9_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group_9__5__Impl
	rule__SelectionStatement__Group_9__6
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getLeftCurlyBracketKeyword_9_5()); }
	'{'
	{ after(grammarAccess.getSelectionStatementAccess().getLeftCurlyBracketKeyword_9_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group_9__6__Impl
	rule__SelectionStatement__Group_9__7
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getCommandsAssignment_9_6()); }
	(rule__SelectionStatement__CommandsAssignment_9_6)
	{ after(grammarAccess.getSelectionStatementAccess().getCommandsAssignment_9_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SelectionStatement__Group_9__7__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__Group_9__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSelectionStatementAccess().getRightCurlyBracketKeyword_9_7()); }
	'}'
	{ after(grammarAccess.getSelectionStatementAccess().getRightCurlyBracketKeyword_9_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__SmallRepetitionStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__0__Impl
	rule__SmallRepetitionStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getSmallRepetitionStatementAction_0()); }
	()
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getSmallRepetitionStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__1__Impl
	rule__SmallRepetitionStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getWhileKeyword_1()); }
	'While'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getWhileKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__2__Impl
	rule__SmallRepetitionStatement__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getLeftParenthesisKeyword_2()); }
	'('
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getLeftParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__3__Impl
	rule__SmallRepetitionStatement__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getGuardAssignment_3()); }
	(rule__SmallRepetitionStatement__GuardAssignment_3)
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getGuardAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__4__Impl
	rule__SmallRepetitionStatement__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getRightParenthesisKeyword_4()); }
	')'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getRightParenthesisKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__5__Impl
	rule__SmallRepetitionStatement__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getDoKeyword_5()); }
	'Do'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getDoKeyword_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__6__Impl
	rule__SmallRepetitionStatement__Group__7
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getInvKeyword_6()); }
	'Inv:'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getInvKeyword_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__7__Impl
	rule__SmallRepetitionStatement__Group__8
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getLeftParenthesisKeyword_7()); }
	'('
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getLeftParenthesisKeyword_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__8
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__8__Impl
	rule__SmallRepetitionStatement__Group__9
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__8__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getInvariantAssignment_8()); }
	(rule__SmallRepetitionStatement__InvariantAssignment_8)
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getInvariantAssignment_8()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__9
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__9__Impl
	rule__SmallRepetitionStatement__Group__10
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__9__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getRightParenthesisKeyword_9()); }
	')'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getRightParenthesisKeyword_9()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__10
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__10__Impl
	rule__SmallRepetitionStatement__Group__11
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__10__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getVarKeyword_10()); }
	'Var:'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getVarKeyword_10()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__11
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__11__Impl
	rule__SmallRepetitionStatement__Group__12
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__11__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getLeftParenthesisKeyword_11()); }
	'('
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getLeftParenthesisKeyword_11()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__12
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__12__Impl
	rule__SmallRepetitionStatement__Group__13
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__12__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getVariantAssignment_12()); }
	(rule__SmallRepetitionStatement__VariantAssignment_12)
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getVariantAssignment_12()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__13
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__13__Impl
	rule__SmallRepetitionStatement__Group__14
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__13__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getRightParenthesisKeyword_13()); }
	')'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getRightParenthesisKeyword_13()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__14
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__14__Impl
	rule__SmallRepetitionStatement__Group__15
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__14__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getLeftCurlyBracketKeyword_14()); }
	'{'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getLeftCurlyBracketKeyword_14()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__15
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__15__Impl
	rule__SmallRepetitionStatement__Group__16
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__15__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getLoopStatementAssignment_15()); }
	(rule__SmallRepetitionStatement__LoopStatementAssignment_15)
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getLoopStatementAssignment_15()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__16
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__16__Impl
	rule__SmallRepetitionStatement__Group__17
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__16__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getRightCurlyBracketKeyword_16()); }
	'}'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getRightCurlyBracketKeyword_16()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__17
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__SmallRepetitionStatement__Group__17__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__Group__17__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getSmallRepetitionStatementAccess().getOdKeyword_17()); }
	'Od'
	{ after(grammarAccess.getSmallRepetitionStatementAccess().getOdKeyword_17()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Variant__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Variant__Group__0__Impl
	rule__Variant__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Variant__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariantAccess().getVariantAction_0()); }
	()
	{ after(grammarAccess.getVariantAccess().getVariantAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Variant__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Variant__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Variant__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariantAccess().getVarAssignment_1()); }
	(rule__Variant__VarAssignment_1)
	{ after(grammarAccess.getVariantAccess().getVarAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JavaVariables__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group__0__Impl
	rule__JavaVariables__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getJavaVariablesAction_0()); }
	()
	{ after(grammarAccess.getJavaVariablesAccess().getJavaVariablesAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group__1__Impl
	rule__JavaVariables__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getJavaVariablesKeyword_1()); }
	'JavaVariables'
	{ after(grammarAccess.getJavaVariablesAccess().getJavaVariablesKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getGroup_2()); }
	(rule__JavaVariables__Group_2__0)?
	{ after(grammarAccess.getJavaVariablesAccess().getGroup_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JavaVariables__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group_2__0__Impl
	rule__JavaVariables__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getVariablesKeyword_2_0()); }
	'Variables'
	{ after(grammarAccess.getJavaVariablesAccess().getVariablesKeyword_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group_2__1__Impl
	rule__JavaVariables__Group_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getLeftSquareBracketKeyword_2_1()); }
	'['
	{ after(grammarAccess.getJavaVariablesAccess().getLeftSquareBracketKeyword_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group_2__2__Impl
	rule__JavaVariables__Group_2__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getVariablesAssignment_2_2()); }
	(rule__JavaVariables__VariablesAssignment_2_2)
	{ after(grammarAccess.getJavaVariablesAccess().getVariablesAssignment_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group_2__3__Impl
	rule__JavaVariables__Group_2__4
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getGroup_2_3()); }
	(rule__JavaVariables__Group_2_3__0)*
	{ after(grammarAccess.getJavaVariablesAccess().getGroup_2_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group_2__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getRightSquareBracketKeyword_2_4()); }
	']'
	{ after(grammarAccess.getJavaVariablesAccess().getRightSquareBracketKeyword_2_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JavaVariables__Group_2_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group_2_3__0__Impl
	rule__JavaVariables__Group_2_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getCommaKeyword_2_3_0()); }
	','
	{ after(grammarAccess.getJavaVariablesAccess().getCommaKeyword_2_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariables__Group_2_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__Group_2_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariablesAccess().getVariablesAssignment_2_3_1()); }
	(rule__JavaVariables__VariablesAssignment_2_3_1)
	{ after(grammarAccess.getJavaVariablesAccess().getVariablesAssignment_2_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JavaVariable__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariable__Group__0__Impl
	rule__JavaVariable__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariable__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariableAccess().getJavaVariableAction_0()); }
	()
	{ after(grammarAccess.getJavaVariableAccess().getJavaVariableAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariable__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariable__Group__1__Impl
	rule__JavaVariable__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariable__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariableAccess().getTypeAssignment_1()); }
	(rule__JavaVariable__TypeAssignment_1)
	{ after(grammarAccess.getJavaVariableAccess().getTypeAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariable__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JavaVariable__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariable__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJavaVariableAccess().getVarAssignment_2()); }
	(rule__JavaVariable__VarAssignment_2)
	{ after(grammarAccess.getJavaVariableAccess().getVarAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__GlobalConditions__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group__0__Impl
	rule__GlobalConditions__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getGlobalConditionsAction_0()); }
	()
	{ after(grammarAccess.getGlobalConditionsAccess().getGlobalConditionsAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group__1__Impl
	rule__GlobalConditions__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getGlobalConditionsKeyword_1()); }
	'GlobalConditions'
	{ after(grammarAccess.getGlobalConditionsAccess().getGlobalConditionsKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getGroup_2()); }
	(rule__GlobalConditions__Group_2__0)?
	{ after(grammarAccess.getGlobalConditionsAccess().getGroup_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__GlobalConditions__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group_2__0__Impl
	rule__GlobalConditions__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getConditionsKeyword_2_0()); }
	'Conditions'
	{ after(grammarAccess.getGlobalConditionsAccess().getConditionsKeyword_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group_2__1__Impl
	rule__GlobalConditions__Group_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getLeftSquareBracketKeyword_2_1()); }
	'['
	{ after(grammarAccess.getGlobalConditionsAccess().getLeftSquareBracketKeyword_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group_2__2__Impl
	rule__GlobalConditions__Group_2__3
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getConditionsAssignment_2_2()); }
	(rule__GlobalConditions__ConditionsAssignment_2_2)
	{ after(grammarAccess.getGlobalConditionsAccess().getConditionsAssignment_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group_2__3__Impl
	rule__GlobalConditions__Group_2__4
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getGroup_2_3()); }
	(rule__GlobalConditions__Group_2_3__0)*
	{ after(grammarAccess.getGlobalConditionsAccess().getGroup_2_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group_2__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getRightSquareBracketKeyword_2_4()); }
	']'
	{ after(grammarAccess.getGlobalConditionsAccess().getRightSquareBracketKeyword_2_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__GlobalConditions__Group_2_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group_2_3__0__Impl
	rule__GlobalConditions__Group_2_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getCommaKeyword_2_3_0()); }
	','
	{ after(grammarAccess.getGlobalConditionsAccess().getCommaKeyword_2_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__GlobalConditions__Group_2_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__Group_2_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getGlobalConditionsAccess().getConditionsAssignment_2_3_1()); }
	(rule__GlobalConditions__ConditionsAssignment_2_3_1)
	{ after(grammarAccess.getGlobalConditionsAccess().getConditionsAssignment_2_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Renaming__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Renaming__Group__0__Impl
	rule__Renaming__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenamingAccess().getRenamingAction_0()); }
	()
	{ after(grammarAccess.getRenamingAccess().getRenamingAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Renaming__Group__1__Impl
	rule__Renaming__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenamingAccess().getRenamingKeyword_1()); }
	'Renaming'
	{ after(grammarAccess.getRenamingAccess().getRenamingKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Renaming__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenamingAccess().getGroup_2()); }
	(rule__Renaming__Group_2__0)?
	{ after(grammarAccess.getRenamingAccess().getGroup_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Renaming__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Renaming__Group_2__0__Impl
	rule__Renaming__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenamingAccess().getRenamesKeyword_2_0()); }
	'Renames'
	{ after(grammarAccess.getRenamingAccess().getRenamesKeyword_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Renaming__Group_2__1__Impl
	rule__Renaming__Group_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenamingAccess().getLeftSquareBracketKeyword_2_1()); }
	'['
	{ after(grammarAccess.getRenamingAccess().getLeftSquareBracketKeyword_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Renaming__Group_2__2__Impl
	rule__Renaming__Group_2__3
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenamingAccess().getRenameAssignment_2_2()); }
	(rule__Renaming__RenameAssignment_2_2)
	{ after(grammarAccess.getRenamingAccess().getRenameAssignment_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Renaming__Group_2__3__Impl
	rule__Renaming__Group_2__4
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenamingAccess().getRenameAssignment_2_3()); }
	(rule__Renaming__RenameAssignment_2_3)*
	{ after(grammarAccess.getRenamingAccess().getRenameAssignment_2_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Renaming__Group_2__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__Group_2__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenamingAccess().getRightSquareBracketKeyword_2_4()); }
	']'
	{ after(grammarAccess.getRenamingAccess().getRightSquareBracketKeyword_2_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Rename__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__0__Impl
	rule__Rename__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getRenameAction_0()); }
	()
	{ after(grammarAccess.getRenameAccess().getRenameAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__1__Impl
	rule__Rename__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getLeftCurlyBracketKeyword_1()); }
	'{'
	{ after(grammarAccess.getRenameAccess().getLeftCurlyBracketKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__2__Impl
	rule__Rename__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getTypeKeyword_2()); }
	'Type'
	{ after(grammarAccess.getRenameAccess().getTypeKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__3__Impl
	rule__Rename__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getTypeAssignment_3()); }
	(rule__Rename__TypeAssignment_3)
	{ after(grammarAccess.getRenameAccess().getTypeAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__4__Impl
	rule__Rename__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getFunctionKeyword_4()); }
	'Function'
	{ after(grammarAccess.getRenameAccess().getFunctionKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__5__Impl
	rule__Rename__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getFunctionAssignment_5()); }
	(rule__Rename__FunctionAssignment_5)
	{ after(grammarAccess.getRenameAccess().getFunctionAssignment_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__6__Impl
	rule__Rename__Group__7
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getNewNameKeyword_6()); }
	'NewName'
	{ after(grammarAccess.getRenameAccess().getNewNameKeyword_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__7__Impl
	rule__Rename__Group__8
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getNewNameAssignment_7()); }
	(rule__Rename__NewNameAssignment_7)
	{ after(grammarAccess.getRenameAccess().getNewNameAssignment_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__8
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Rename__Group__8__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__Group__8__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRenameAccess().getRightCurlyBracketKeyword_8()); }
	'}'
	{ after(grammarAccess.getRenameAccess().getRightCurlyBracketKeyword_8()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableOrMethodName__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_0__0__Impl
	rule__VariableOrMethodName__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getNameAssignment_0_0()); }
	(rule__VariableOrMethodName__NameAssignment_0_0)
	{ after(grammarAccess.getVariableOrMethodNameAccess().getNameAssignment_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getGroup_0_1()); }
	(rule__VariableOrMethodName__Group_0_1__0)?
	{ after(grammarAccess.getVariableOrMethodNameAccess().getGroup_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableOrMethodName__Group_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_0_1__0__Impl
	rule__VariableOrMethodName__Group_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getLeftSquareBracketKeyword_0_1_0()); }
	'['
	{ after(grammarAccess.getVariableOrMethodNameAccess().getLeftSquareBracketKeyword_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_0_1__1__Impl
	rule__VariableOrMethodName__Group_0_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getAsteriskKeyword_0_1_1()); }
	'*'
	{ after(grammarAccess.getVariableOrMethodNameAccess().getAsteriskKeyword_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_0_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_0_1__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_0_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getRightSquareBracketKeyword_0_1_2()); }
	']'
	{ after(grammarAccess.getVariableOrMethodNameAccess().getRightSquareBracketKeyword_0_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableOrMethodName__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_1__0__Impl
	rule__VariableOrMethodName__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getOldKeyword_1_0()); }
	'\\old('
	{ after(grammarAccess.getVariableOrMethodNameAccess().getOldKeyword_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_1__1__Impl
	rule__VariableOrMethodName__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getGroup_1_1()); }
	(rule__VariableOrMethodName__Group_1_1__0)
	{ after(grammarAccess.getVariableOrMethodNameAccess().getGroup_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_1__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getRightParenthesisKeyword_1_2()); }
	')'
	{ after(grammarAccess.getVariableOrMethodNameAccess().getRightParenthesisKeyword_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableOrMethodName__Group_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_1_1__0__Impl
	rule__VariableOrMethodName__Group_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getNameAssignment_1_1_0()); }
	(rule__VariableOrMethodName__NameAssignment_1_1_0)
	{ after(grammarAccess.getVariableOrMethodNameAccess().getNameAssignment_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getGroup_1_1_1()); }
	(rule__VariableOrMethodName__Group_1_1_1__0)?
	{ after(grammarAccess.getVariableOrMethodNameAccess().getGroup_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__VariableOrMethodName__Group_1_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_1_1_1__0__Impl
	rule__VariableOrMethodName__Group_1_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getLeftSquareBracketKeyword_1_1_1_0()); }
	'['
	{ after(grammarAccess.getVariableOrMethodNameAccess().getLeftSquareBracketKeyword_1_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_1_1_1__1__Impl
	rule__VariableOrMethodName__Group_1_1_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getAsteriskKeyword_1_1_1_1()); }
	'*'
	{ after(grammarAccess.getVariableOrMethodNameAccess().getAsteriskKeyword_1_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1_1_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__VariableOrMethodName__Group_1_1_1__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__Group_1_1_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getVariableOrMethodNameAccess().getRightSquareBracketKeyword_1_1_1_2()); }
	']'
	{ after(grammarAccess.getVariableOrMethodNameAccess().getRightSquareBracketKeyword_1_1_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Implication__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Implication__Group__0__Impl
	rule__Implication__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getImplicationAccess().getConcatParserRuleCall_0()); }
	ruleConcat
	{ after(grammarAccess.getImplicationAccess().getConcatParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Implication__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getImplicationAccess().getGroup_1()); }
	(rule__Implication__Group_1__0)*
	{ after(grammarAccess.getImplicationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Implication__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Implication__Group_1__0__Impl
	rule__Implication__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getImplicationAccess().getGroup_1_0()); }
	(rule__Implication__Group_1_0__0)
	{ after(grammarAccess.getImplicationAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Implication__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getImplicationAccess().getRightAssignment_1_1()); }
	(rule__Implication__RightAssignment_1_1)
	{ after(grammarAccess.getImplicationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Implication__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Implication__Group_1_0__0__Impl
	rule__Implication__Group_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getImplicationAccess().getImplLeftAction_1_0_0()); }
	()
	{ after(grammarAccess.getImplicationAccess().getImplLeftAction_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Implication__Group_1_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getImplicationAccess().getGroup_1_0_1()); }
	(rule__Implication__Group_1_0_1__0)
	{ after(grammarAccess.getImplicationAccess().getGroup_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Implication__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Implication__Group_1_0_1__0__Impl
	rule__Implication__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getImplicationAccess().getLessThanSignKeyword_1_0_1_0()); }
	('<')?
	{ after(grammarAccess.getImplicationAccess().getLessThanSignKeyword_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Implication__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getImplicationAccess().getHyphenMinusGreaterThanSignKeyword_1_0_1_1()); }
	'->'
	{ after(grammarAccess.getImplicationAccess().getHyphenMinusGreaterThanSignKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Concat__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Concat__Group__0__Impl
	rule__Concat__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConcatAccess().getForeachParserRuleCall_0()); }
	ruleForeach
	{ after(grammarAccess.getConcatAccess().getForeachParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Concat__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConcatAccess().getGroup_1()); }
	(rule__Concat__Group_1__0)*
	{ after(grammarAccess.getConcatAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Concat__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Concat__Group_1__0__Impl
	rule__Concat__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConcatAccess().getAlternatives_1_0()); }
	(rule__Concat__Alternatives_1_0)
	{ after(grammarAccess.getConcatAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Concat__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConcatAccess().getRightAssignment_1_1()); }
	(rule__Concat__RightAssignment_1_1)
	{ after(grammarAccess.getConcatAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Concat__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Concat__Group_1_0_0__0__Impl
	rule__Concat__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConcatAccess().getAndLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getConcatAccess().getAndLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Concat__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConcatAccess().getAmpersandKeyword_1_0_0_1()); }
	'&'
	{ after(grammarAccess.getConcatAccess().getAmpersandKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Concat__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Concat__Group_1_0_1__0__Impl
	rule__Concat__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConcatAccess().getOrLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getConcatAccess().getOrLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Concat__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getConcatAccess().getVerticalLineKeyword_1_0_1_1()); }
	'|'
	{ after(grammarAccess.getConcatAccess().getVerticalLineKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Foreach__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Foreach__Group_1__0__Impl
	rule__Foreach__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getForeachAccess().getForeachAssignment_1_0()); }
	(rule__Foreach__ForeachAssignment_1_0)
	{ after(grammarAccess.getForeachAccess().getForeachAssignment_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Foreach__Group_1__1__Impl
	rule__Foreach__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getForeachAccess().getTypeAssignment_1_1()); }
	(rule__Foreach__TypeAssignment_1_1)
	{ after(grammarAccess.getForeachAccess().getTypeAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Foreach__Group_1__2__Impl
	rule__Foreach__Group_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getForeachAccess().getVarAssignment_1_2()); }
	(rule__Foreach__VarAssignment_1_2)
	{ after(grammarAccess.getForeachAccess().getVarAssignment_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Foreach__Group_1__3__Impl
	rule__Foreach__Group_1__4
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getForeachAccess().getSemicolonKeyword_1_3()); }
	';'
	{ after(grammarAccess.getForeachAccess().getSemicolonKeyword_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Foreach__Group_1__4__Impl
	rule__Foreach__Group_1__5
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getForeachAccess().getRightAssignment_1_4()); }
	(rule__Foreach__RightAssignment_1_4)
	{ after(grammarAccess.getForeachAccess().getRightAssignment_1_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Foreach__Group_1__5__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__Group_1__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getForeachAccess().getRightParenthesisKeyword_1_5()); }
	')'
	{ after(grammarAccess.getForeachAccess().getRightParenthesisKeyword_1_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Exists__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Exists__Group_1__0__Impl
	rule__Exists__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getExistsAccess().getExistsAssignment_1_0()); }
	(rule__Exists__ExistsAssignment_1_0)
	{ after(grammarAccess.getExistsAccess().getExistsAssignment_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Exists__Group_1__1__Impl
	rule__Exists__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getExistsAccess().getTypeAssignment_1_1()); }
	(rule__Exists__TypeAssignment_1_1)
	{ after(grammarAccess.getExistsAccess().getTypeAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Exists__Group_1__2__Impl
	rule__Exists__Group_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getExistsAccess().getVarAssignment_1_2()); }
	(rule__Exists__VarAssignment_1_2)
	{ after(grammarAccess.getExistsAccess().getVarAssignment_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Exists__Group_1__3__Impl
	rule__Exists__Group_1__4
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getExistsAccess().getSemicolonKeyword_1_3()); }
	';'
	{ after(grammarAccess.getExistsAccess().getSemicolonKeyword_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Exists__Group_1__4__Impl
	rule__Exists__Group_1__5
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getExistsAccess().getRightAssignment_1_4()); }
	(rule__Exists__RightAssignment_1_4)
	{ after(grammarAccess.getExistsAccess().getRightAssignment_1_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Exists__Group_1__5__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__Group_1__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getExistsAccess().getRightParenthesisKeyword_1_5()); }
	')'
	{ after(grammarAccess.getExistsAccess().getRightParenthesisKeyword_1_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Relation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group__0__Impl
	rule__Relation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getMultiMathOperationParserRuleCall_0()); }
	ruleMultiMathOperation
	{ after(grammarAccess.getRelationAccess().getMultiMathOperationParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getGroup_1()); }
	(rule__Relation__Group_1__0)*
	{ after(grammarAccess.getRelationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Relation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1__0__Impl
	rule__Relation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getAlternatives_1_0()); }
	(rule__Relation__Alternatives_1_0)
	{ after(grammarAccess.getRelationAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getRightAssignment_1_1()); }
	(rule__Relation__RightAssignment_1_1)
	{ after(grammarAccess.getRelationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Relation__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_0__0__Impl
	rule__Relation__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getLowerLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getRelationAccess().getLowerLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getLessThanSignKeyword_1_0_0_1()); }
	'<'
	{ after(grammarAccess.getRelationAccess().getLessThanSignKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Relation__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_1__0__Impl
	rule__Relation__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getGreaterLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getRelationAccess().getGreaterLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getGreaterThanSignKeyword_1_0_1_1()); }
	'>'
	{ after(grammarAccess.getRelationAccess().getGreaterThanSignKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Relation__Group_1_0_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_2__0__Impl
	rule__Relation__Group_1_0_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getEqualLeftAction_1_0_2_0()); }
	()
	{ after(grammarAccess.getRelationAccess().getEqualLeftAction_1_0_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getEqualsSignKeyword_1_0_2_1()); }
	'='
	{ after(grammarAccess.getRelationAccess().getEqualsSignKeyword_1_0_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Relation__Group_1_0_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_3__0__Impl
	rule__Relation__Group_1_0_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getNotEqualLeftAction_1_0_3_0()); }
	()
	{ after(grammarAccess.getRelationAccess().getNotEqualLeftAction_1_0_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getExclamationMarkEqualsSignKeyword_1_0_3_1()); }
	'!='
	{ after(grammarAccess.getRelationAccess().getExclamationMarkEqualsSignKeyword_1_0_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Relation__Group_1_0_4__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_4__0__Impl
	rule__Relation__Group_1_0_4__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_4__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getLowerEqualLeftAction_1_0_4_0()); }
	()
	{ after(grammarAccess.getRelationAccess().getLowerEqualLeftAction_1_0_4_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_4__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_4__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_4__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getLessThanSignEqualsSignKeyword_1_0_4_1()); }
	'<='
	{ after(grammarAccess.getRelationAccess().getLessThanSignEqualsSignKeyword_1_0_4_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Relation__Group_1_0_5__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_5__0__Impl
	rule__Relation__Group_1_0_5__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_5__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getGreaterEqualLeftAction_1_0_5_0()); }
	()
	{ after(grammarAccess.getRelationAccess().getGreaterEqualLeftAction_1_0_5_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_5__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Relation__Group_1_0_5__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__Group_1_0_5__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getRelationAccess().getGreaterThanSignEqualsSignKeyword_1_0_5_1()); }
	'>='
	{ after(grammarAccess.getRelationAccess().getGreaterThanSignEqualsSignKeyword_1_0_5_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__MultiMathOperation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group__0__Impl
	rule__MultiMathOperation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getAddMathOperationParserRuleCall_0()); }
	ruleAddMathOperation
	{ after(grammarAccess.getMultiMathOperationAccess().getAddMathOperationParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getGroup_1()); }
	(rule__MultiMathOperation__Group_1__0)*
	{ after(grammarAccess.getMultiMathOperationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__MultiMathOperation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group_1__0__Impl
	rule__MultiMathOperation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getAlternatives_1_0()); }
	(rule__MultiMathOperation__Alternatives_1_0)
	{ after(grammarAccess.getMultiMathOperationAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getRightAssignment_1_1()); }
	(rule__MultiMathOperation__RightAssignment_1_1)
	{ after(grammarAccess.getMultiMathOperationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__MultiMathOperation__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group_1_0_0__0__Impl
	rule__MultiMathOperation__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getMultiplicationLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getMultiMathOperationAccess().getMultiplicationLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getAsteriskKeyword_1_0_0_1()); }
	'*'
	{ after(grammarAccess.getMultiMathOperationAccess().getAsteriskKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__MultiMathOperation__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group_1_0_1__0__Impl
	rule__MultiMathOperation__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getDivisionLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getMultiMathOperationAccess().getDivisionLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getSolidusKeyword_1_0_1_1()); }
	'/'
	{ after(grammarAccess.getMultiMathOperationAccess().getSolidusKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__MultiMathOperation__Group_1_0_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group_1_0_2__0__Impl
	rule__MultiMathOperation__Group_1_0_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getModuloLeftAction_1_0_2_0()); }
	()
	{ after(grammarAccess.getMultiMathOperationAccess().getModuloLeftAction_1_0_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__MultiMathOperation__Group_1_0_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__Group_1_0_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getMultiMathOperationAccess().getPercentSignKeyword_1_0_2_1()); }
	'%'
	{ after(grammarAccess.getMultiMathOperationAccess().getPercentSignKeyword_1_0_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__AddMathOperation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AddMathOperation__Group__0__Impl
	rule__AddMathOperation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAddMathOperationAccess().getQualifiedExpressionParserRuleCall_0()); }
	ruleQualifiedExpression
	{ after(grammarAccess.getAddMathOperationAccess().getQualifiedExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AddMathOperation__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAddMathOperationAccess().getGroup_1()); }
	(rule__AddMathOperation__Group_1__0)*
	{ after(grammarAccess.getAddMathOperationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__AddMathOperation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AddMathOperation__Group_1__0__Impl
	rule__AddMathOperation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAddMathOperationAccess().getAlternatives_1_0()); }
	(rule__AddMathOperation__Alternatives_1_0)
	{ after(grammarAccess.getAddMathOperationAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AddMathOperation__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAddMathOperationAccess().getRightAssignment_1_1()); }
	(rule__AddMathOperation__RightAssignment_1_1)
	{ after(grammarAccess.getAddMathOperationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__AddMathOperation__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AddMathOperation__Group_1_0_0__0__Impl
	rule__AddMathOperation__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAddMathOperationAccess().getAdditionLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getAddMathOperationAccess().getAdditionLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AddMathOperation__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAddMathOperationAccess().getPlusSignKeyword_1_0_0_1()); }
	'+'
	{ after(grammarAccess.getAddMathOperationAccess().getPlusSignKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__AddMathOperation__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AddMathOperation__Group_1_0_1__0__Impl
	rule__AddMathOperation__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAddMathOperationAccess().getSubtractionLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getAddMathOperationAccess().getSubtractionLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__AddMathOperation__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getAddMathOperationAccess().getHyphenMinusKeyword_1_0_1_1()); }
	'-'
	{ after(grammarAccess.getAddMathOperationAccess().getHyphenMinusKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__QualifiedExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedExpression__Group__0__Impl
	rule__QualifiedExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedExpressionAccess().getPrimaryExpressionParserRuleCall_0()); }
	rulePrimaryExpression
	{ after(grammarAccess.getQualifiedExpressionAccess().getPrimaryExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedExpressionAccess().getGroup_1()); }
	(rule__QualifiedExpression__Group_1__0)*
	{ after(grammarAccess.getQualifiedExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__QualifiedExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedExpression__Group_1__0__Impl
	rule__QualifiedExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedExpressionAccess().getGroup_1_0()); }
	(rule__QualifiedExpression__Group_1_0__0)
	{ after(grammarAccess.getQualifiedExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedExpressionAccess().getRightAssignment_1_1()); }
	(rule__QualifiedExpression__RightAssignment_1_1)
	{ after(grammarAccess.getQualifiedExpressionAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__QualifiedExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedExpressionAccess().getGroup_1_0_0()); }
	(rule__QualifiedExpression__Group_1_0_0__0)
	{ after(grammarAccess.getQualifiedExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__QualifiedExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedExpression__Group_1_0_0__0__Impl
	rule__QualifiedExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedExpressionAccess().getQualifierLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getQualifiedExpressionAccess().getQualifierLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedExpressionAccess().getFullStopKeyword_1_0_0_1()); }
	'.'
	{ after(grammarAccess.getQualifiedExpressionAccess().getFullStopKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_0__0__Impl
	rule__PrimaryExpression__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_0_0()); }
	()
	{ after(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_0_1()); }
	(rule__PrimaryExpression__Group_0_1__0)
	{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_0_1__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getAlternatives_0_1_0()); }
	(rule__PrimaryExpression__Alternatives_0_1_0)
	{ after(grammarAccess.getPrimaryExpressionAccess().getAlternatives_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_1__0__Impl
	rule__PrimaryExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getExclamationMarkKeyword_1_0()); }
	('!')?
	{ after(grammarAccess.getPrimaryExpressionAccess().getExclamationMarkKeyword_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_1__1__Impl
	rule__PrimaryExpression__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getLeftParenthesisKeyword_1_1()); }
	'('
	{ after(grammarAccess.getPrimaryExpressionAccess().getLeftParenthesisKeyword_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_1__2__Impl
	rule__PrimaryExpression__Group_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getExpressionParserRuleCall_1_2()); }
	ruleExpression
	{ after(grammarAccess.getPrimaryExpressionAccess().getExpressionParserRuleCall_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_1__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getRightParenthesisKeyword_1_3()); }
	')'
	{ after(grammarAccess.getPrimaryExpressionAccess().getRightParenthesisKeyword_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2__0__Impl
	rule__PrimaryExpression__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getFunctionCallAction_2_0()); }
	()
	{ after(grammarAccess.getPrimaryExpressionAccess().getFunctionCallAction_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2__1__Impl
	rule__PrimaryExpression__Group_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getExclamationMarkKeyword_2_1()); }
	('!')?
	{ after(grammarAccess.getPrimaryExpressionAccess().getExclamationMarkKeyword_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2__2__Impl
	rule__PrimaryExpression__Group_2__3
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getFuncAssignment_2_2()); }
	(rule__PrimaryExpression__FuncAssignment_2_2)
	{ after(grammarAccess.getPrimaryExpressionAccess().getFuncAssignment_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getAlternatives_2_3()); }
	(rule__PrimaryExpression__Alternatives_2_3)*
	{ after(grammarAccess.getPrimaryExpressionAccess().getAlternatives_2_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_2_3_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_0__0__Impl
	rule__PrimaryExpression__Group_2_3_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getLeftParenthesisKeyword_2_3_0_0()); }
	'('
	{ after(grammarAccess.getPrimaryExpressionAccess().getLeftParenthesisKeyword_2_3_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_0__1__Impl
	rule__PrimaryExpression__Group_2_3_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_2_3_0_1()); }
	(rule__PrimaryExpression__Group_2_3_0_1__0)?
	{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_2_3_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getRightParenthesisKeyword_2_3_0_2()); }
	')'
	{ after(grammarAccess.getPrimaryExpressionAccess().getRightParenthesisKeyword_2_3_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_2_3_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_0_1__0__Impl
	rule__PrimaryExpression__Group_2_3_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getArgsAssignment_2_3_0_1_0()); }
	(rule__PrimaryExpression__ArgsAssignment_2_3_0_1_0)
	{ after(grammarAccess.getPrimaryExpressionAccess().getArgsAssignment_2_3_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getGroup_2_3_0_1_1()); }
	(rule__PrimaryExpression__Group_2_3_0_1_1__0)*
	{ after(grammarAccess.getPrimaryExpressionAccess().getGroup_2_3_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_2_3_0_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_0_1_1__0__Impl
	rule__PrimaryExpression__Group_2_3_0_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getCommaKeyword_2_3_0_1_1_0()); }
	','
	{ after(grammarAccess.getPrimaryExpressionAccess().getCommaKeyword_2_3_0_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_0_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_0_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getArgsAssignment_2_3_0_1_1_1()); }
	(rule__PrimaryExpression__ArgsAssignment_2_3_0_1_1_1)
	{ after(grammarAccess.getPrimaryExpressionAccess().getArgsAssignment_2_3_0_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_2_3_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_1__0__Impl
	rule__PrimaryExpression__Group_2_3_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getLeftSquareBracketKeyword_2_3_1_0()); }
	'['
	{ after(grammarAccess.getPrimaryExpressionAccess().getLeftSquareBracketKeyword_2_3_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_1__1__Impl
	rule__PrimaryExpression__Group_2_3_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getArgsAssignment_2_3_1_1()); }
	(rule__PrimaryExpression__ArgsAssignment_2_3_1_1)
	{ after(grammarAccess.getPrimaryExpressionAccess().getArgsAssignment_2_3_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_2_3_1__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_2_3_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getRightSquareBracketKeyword_2_3_1_2()); }
	']'
	{ after(grammarAccess.getPrimaryExpressionAccess().getRightSquareBracketKeyword_2_3_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_3__0__Impl
	rule__PrimaryExpression__Group_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_3_0()); }
	()
	{ after(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getINTTerminalRuleCall_3_1()); }
	RULE_INT
	{ after(grammarAccess.getPrimaryExpressionAccess().getINTTerminalRuleCall_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_4__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_4__0__Impl
	rule__PrimaryExpression__Group_4__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_4__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_4_0()); }
	()
	{ after(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_4_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_4__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_4__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_4__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getSTRINGTerminalRuleCall_4_1()); }
	RULE_STRING
	{ after(grammarAccess.getPrimaryExpressionAccess().getSTRINGTerminalRuleCall_4_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_5__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_5__0__Impl
	rule__PrimaryExpression__Group_5__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_5__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_5_0()); }
	()
	{ after(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_5_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_5__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_5__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_5__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getCHARACTERTerminalRuleCall_5_1()); }
	RULE_CHARACTER
	{ after(grammarAccess.getPrimaryExpressionAccess().getCHARACTERTerminalRuleCall_5_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__PrimaryExpression__Group_6__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_6__0__Impl
	rule__PrimaryExpression__Group_6__1
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_6__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_6_0()); }
	()
	{ after(grammarAccess.getPrimaryExpressionAccess().getExpressionAction_6_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_6__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__PrimaryExpression__Group_6__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__Group_6__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getPrimaryExpressionAccess().getNullKeyword_6_1()); }
	'null'
	{ after(grammarAccess.getPrimaryExpressionAccess().getNullKeyword_6_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLImplication__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group__0__Impl
	rule__JMLImplication__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getJMLConcatParserRuleCall_0()); }
	ruleJMLConcat
	{ after(grammarAccess.getJMLImplicationAccess().getJMLConcatParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getGroup_1()); }
	(rule__JMLImplication__Group_1__0)*
	{ after(grammarAccess.getJMLImplicationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLImplication__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1__0__Impl
	rule__JMLImplication__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getAlternatives_1_0()); }
	(rule__JMLImplication__Alternatives_1_0)
	{ after(grammarAccess.getJMLImplicationAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getRightAssignment_1_1()); }
	(rule__JMLImplication__RightAssignment_1_1)
	{ after(grammarAccess.getJMLImplicationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLImplication__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1_0_0__0__Impl
	rule__JMLImplication__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getJMLImplLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getJMLImplicationAccess().getJMLImplLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getEqualsSignEqualsSignGreaterThanSignKeyword_1_0_0_1()); }
	'==>'
	{ after(grammarAccess.getJMLImplicationAccess().getEqualsSignEqualsSignGreaterThanSignKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLImplication__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1_0_1__0__Impl
	rule__JMLImplication__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getJMLImplLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getJMLImplicationAccess().getJMLImplLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getLessThanSignEqualsSignEqualsSignKeyword_1_0_1_1()); }
	'<=='
	{ after(grammarAccess.getJMLImplicationAccess().getLessThanSignEqualsSignEqualsSignKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLImplication__Group_1_0_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1_0_2__0__Impl
	rule__JMLImplication__Group_1_0_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getJMLImplLeftAction_1_0_2_0()); }
	()
	{ after(grammarAccess.getJMLImplicationAccess().getJMLImplLeftAction_1_0_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1_0_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getLessThanSignEqualsSignEqualsSignGreaterThanSignKeyword_1_0_2_1()); }
	'<==>'
	{ after(grammarAccess.getJMLImplicationAccess().getLessThanSignEqualsSignEqualsSignGreaterThanSignKeyword_1_0_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLImplication__Group_1_0_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1_0_3__0__Impl
	rule__JMLImplication__Group_1_0_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getJMLImplLeftAction_1_0_3_0()); }
	()
	{ after(grammarAccess.getJMLImplicationAccess().getJMLImplLeftAction_1_0_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLImplication__Group_1_0_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__Group_1_0_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLImplicationAccess().getLessThanSignEqualsSignExclamationMarkEqualsSignGreaterThanSignKeyword_1_0_3_1()); }
	'<=!=>'
	{ after(grammarAccess.getJMLImplicationAccess().getLessThanSignEqualsSignExclamationMarkEqualsSignGreaterThanSignKeyword_1_0_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLConcat__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLConcat__Group__0__Impl
	rule__JMLConcat__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLConcatAccess().getJMLForeachParserRuleCall_0()); }
	ruleJMLForeach
	{ after(grammarAccess.getJMLConcatAccess().getJMLForeachParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLConcat__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLConcatAccess().getGroup_1()); }
	(rule__JMLConcat__Group_1__0)*
	{ after(grammarAccess.getJMLConcatAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLConcat__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLConcat__Group_1__0__Impl
	rule__JMLConcat__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLConcatAccess().getAlternatives_1_0()); }
	(rule__JMLConcat__Alternatives_1_0)
	{ after(grammarAccess.getJMLConcatAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLConcat__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLConcatAccess().getRightAssignment_1_1()); }
	(rule__JMLConcat__RightAssignment_1_1)
	{ after(grammarAccess.getJMLConcatAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLConcat__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLConcat__Group_1_0_0__0__Impl
	rule__JMLConcat__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLConcatAccess().getJMLAndLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getJMLConcatAccess().getJMLAndLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLConcat__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLConcatAccess().getAmpersandAmpersandKeyword_1_0_0_1()); }
	'&&'
	{ after(grammarAccess.getJMLConcatAccess().getAmpersandAmpersandKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLConcat__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLConcat__Group_1_0_1__0__Impl
	rule__JMLConcat__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLConcatAccess().getJMLOrLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getJMLConcatAccess().getJMLOrLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLConcat__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLConcatAccess().getVerticalLineVerticalLineKeyword_1_0_1_1()); }
	'||'
	{ after(grammarAccess.getJMLConcatAccess().getVerticalLineVerticalLineKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLForeach__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLForeach__Group_1__0__Impl
	rule__JMLForeach__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLForeachAccess().getForeachAssignment_1_0()); }
	(rule__JMLForeach__ForeachAssignment_1_0)
	{ after(grammarAccess.getJMLForeachAccess().getForeachAssignment_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLForeach__Group_1__1__Impl
	rule__JMLForeach__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLForeachAccess().getTypeAssignment_1_1()); }
	(rule__JMLForeach__TypeAssignment_1_1)
	{ after(grammarAccess.getJMLForeachAccess().getTypeAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLForeach__Group_1__2__Impl
	rule__JMLForeach__Group_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLForeachAccess().getVarAssignment_1_2()); }
	(rule__JMLForeach__VarAssignment_1_2)
	{ after(grammarAccess.getJMLForeachAccess().getVarAssignment_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLForeach__Group_1__3__Impl
	rule__JMLForeach__Group_1__4
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLForeachAccess().getSemicolonKeyword_1_3()); }
	';'
	{ after(grammarAccess.getJMLForeachAccess().getSemicolonKeyword_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLForeach__Group_1__4__Impl
	rule__JMLForeach__Group_1__5
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLForeachAccess().getRightAssignment_1_4()); }
	(rule__JMLForeach__RightAssignment_1_4)
	{ after(grammarAccess.getJMLForeachAccess().getRightAssignment_1_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLForeach__Group_1__5__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__Group_1__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLForeachAccess().getRightParenthesisKeyword_1_5()); }
	')'
	{ after(grammarAccess.getJMLForeachAccess().getRightParenthesisKeyword_1_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLExists__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLExists__Group_1__0__Impl
	rule__JMLExists__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLExistsAccess().getExistsAssignment_1_0()); }
	(rule__JMLExists__ExistsAssignment_1_0)
	{ after(grammarAccess.getJMLExistsAccess().getExistsAssignment_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLExists__Group_1__1__Impl
	rule__JMLExists__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLExistsAccess().getTypeAssignment_1_1()); }
	(rule__JMLExists__TypeAssignment_1_1)
	{ after(grammarAccess.getJMLExistsAccess().getTypeAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLExists__Group_1__2__Impl
	rule__JMLExists__Group_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLExistsAccess().getVarAssignment_1_2()); }
	(rule__JMLExists__VarAssignment_1_2)
	{ after(grammarAccess.getJMLExistsAccess().getVarAssignment_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLExists__Group_1__3__Impl
	rule__JMLExists__Group_1__4
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLExistsAccess().getSemicolonKeyword_1_3()); }
	';'
	{ after(grammarAccess.getJMLExistsAccess().getSemicolonKeyword_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLExists__Group_1__4__Impl
	rule__JMLExists__Group_1__5
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLExistsAccess().getRightAssignment_1_4()); }
	(rule__JMLExists__RightAssignment_1_4)
	{ after(grammarAccess.getJMLExistsAccess().getRightAssignment_1_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLExists__Group_1__5__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__Group_1__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLExistsAccess().getRightParenthesisKeyword_1_5()); }
	')'
	{ after(grammarAccess.getJMLExistsAccess().getRightParenthesisKeyword_1_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLRelation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group__0__Impl
	rule__JMLRelation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getJMLMultiMathOperationParserRuleCall_0()); }
	ruleJMLMultiMathOperation
	{ after(grammarAccess.getJMLRelationAccess().getJMLMultiMathOperationParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getGroup_1()); }
	(rule__JMLRelation__Group_1__0)*
	{ after(grammarAccess.getJMLRelationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLRelation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1__0__Impl
	rule__JMLRelation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getAlternatives_1_0()); }
	(rule__JMLRelation__Alternatives_1_0)
	{ after(grammarAccess.getJMLRelationAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getRightAssignment_1_1()); }
	(rule__JMLRelation__RightAssignment_1_1)
	{ after(grammarAccess.getJMLRelationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLRelation__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_0__0__Impl
	rule__JMLRelation__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getJMLLowerLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getJMLRelationAccess().getJMLLowerLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getLessThanSignKeyword_1_0_0_1()); }
	'<'
	{ after(grammarAccess.getJMLRelationAccess().getLessThanSignKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLRelation__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_1__0__Impl
	rule__JMLRelation__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getJMLGreaterLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getJMLRelationAccess().getJMLGreaterLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getGreaterThanSignKeyword_1_0_1_1()); }
	'>'
	{ after(grammarAccess.getJMLRelationAccess().getGreaterThanSignKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLRelation__Group_1_0_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_2__0__Impl
	rule__JMLRelation__Group_1_0_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getJMLEqualLeftAction_1_0_2_0()); }
	()
	{ after(grammarAccess.getJMLRelationAccess().getJMLEqualLeftAction_1_0_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getEqualsSignEqualsSignKeyword_1_0_2_1()); }
	'=='
	{ after(grammarAccess.getJMLRelationAccess().getEqualsSignEqualsSignKeyword_1_0_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLRelation__Group_1_0_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_3__0__Impl
	rule__JMLRelation__Group_1_0_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getJMLNotEqualLeftAction_1_0_3_0()); }
	()
	{ after(grammarAccess.getJMLRelationAccess().getJMLNotEqualLeftAction_1_0_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getExclamationMarkEqualsSignKeyword_1_0_3_1()); }
	'!='
	{ after(grammarAccess.getJMLRelationAccess().getExclamationMarkEqualsSignKeyword_1_0_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLRelation__Group_1_0_4__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_4__0__Impl
	rule__JMLRelation__Group_1_0_4__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_4__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getJMLLowerEqualLeftAction_1_0_4_0()); }
	()
	{ after(grammarAccess.getJMLRelationAccess().getJMLLowerEqualLeftAction_1_0_4_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_4__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_4__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_4__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getLessThanSignEqualsSignKeyword_1_0_4_1()); }
	'<='
	{ after(grammarAccess.getJMLRelationAccess().getLessThanSignEqualsSignKeyword_1_0_4_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLRelation__Group_1_0_5__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_5__0__Impl
	rule__JMLRelation__Group_1_0_5__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_5__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getJMLGreaterEqualLeftAction_1_0_5_0()); }
	()
	{ after(grammarAccess.getJMLRelationAccess().getJMLGreaterEqualLeftAction_1_0_5_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_5__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLRelation__Group_1_0_5__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__Group_1_0_5__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLRelationAccess().getGreaterThanSignEqualsSignKeyword_1_0_5_1()); }
	'>='
	{ after(grammarAccess.getJMLRelationAccess().getGreaterThanSignEqualsSignKeyword_1_0_5_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLMultiMathOperation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group__0__Impl
	rule__JMLMultiMathOperation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getJMLAddMathOperationParserRuleCall_0()); }
	ruleJMLAddMathOperation
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getJMLAddMathOperationParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getGroup_1()); }
	(rule__JMLMultiMathOperation__Group_1__0)*
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLMultiMathOperation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group_1__0__Impl
	rule__JMLMultiMathOperation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getAlternatives_1_0()); }
	(rule__JMLMultiMathOperation__Alternatives_1_0)
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getRightAssignment_1_1()); }
	(rule__JMLMultiMathOperation__RightAssignment_1_1)
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLMultiMathOperation__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group_1_0_0__0__Impl
	rule__JMLMultiMathOperation__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getJMLMultiplicationLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getJMLMultiplicationLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getAsteriskKeyword_1_0_0_1()); }
	'*'
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getAsteriskKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLMultiMathOperation__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group_1_0_1__0__Impl
	rule__JMLMultiMathOperation__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getJMLDivisionLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getJMLDivisionLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getSolidusKeyword_1_0_1_1()); }
	'/'
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getSolidusKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLMultiMathOperation__Group_1_0_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group_1_0_2__0__Impl
	rule__JMLMultiMathOperation__Group_1_0_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getJMLModuloLeftAction_1_0_2_0()); }
	()
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getJMLModuloLeftAction_1_0_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLMultiMathOperation__Group_1_0_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__Group_1_0_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLMultiMathOperationAccess().getPercentSignKeyword_1_0_2_1()); }
	'%'
	{ after(grammarAccess.getJMLMultiMathOperationAccess().getPercentSignKeyword_1_0_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLAddMathOperation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAddMathOperation__Group__0__Impl
	rule__JMLAddMathOperation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAddMathOperationAccess().getJMLQualifiedExpressionParserRuleCall_0()); }
	ruleJMLQualifiedExpression
	{ after(grammarAccess.getJMLAddMathOperationAccess().getJMLQualifiedExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAddMathOperation__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAddMathOperationAccess().getGroup_1()); }
	(rule__JMLAddMathOperation__Group_1__0)*
	{ after(grammarAccess.getJMLAddMathOperationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLAddMathOperation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAddMathOperation__Group_1__0__Impl
	rule__JMLAddMathOperation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAddMathOperationAccess().getAlternatives_1_0()); }
	(rule__JMLAddMathOperation__Alternatives_1_0)
	{ after(grammarAccess.getJMLAddMathOperationAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAddMathOperation__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAddMathOperationAccess().getRightAssignment_1_1()); }
	(rule__JMLAddMathOperation__RightAssignment_1_1)
	{ after(grammarAccess.getJMLAddMathOperationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLAddMathOperation__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAddMathOperation__Group_1_0_0__0__Impl
	rule__JMLAddMathOperation__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAddMathOperationAccess().getJMLAdditionLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getJMLAddMathOperationAccess().getJMLAdditionLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAddMathOperation__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAddMathOperationAccess().getPlusSignKeyword_1_0_0_1()); }
	'+'
	{ after(grammarAccess.getJMLAddMathOperationAccess().getPlusSignKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLAddMathOperation__Group_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAddMathOperation__Group_1_0_1__0__Impl
	rule__JMLAddMathOperation__Group_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAddMathOperationAccess().getJMLSubtractionLeftAction_1_0_1_0()); }
	()
	{ after(grammarAccess.getJMLAddMathOperationAccess().getJMLSubtractionLeftAction_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLAddMathOperation__Group_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__Group_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLAddMathOperationAccess().getHyphenMinusKeyword_1_0_1_1()); }
	'-'
	{ after(grammarAccess.getJMLAddMathOperationAccess().getHyphenMinusKeyword_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLQualifiedExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLQualifiedExpression__Group__0__Impl
	rule__JMLQualifiedExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLQualifiedExpressionAccess().getJMLPrimaryExpressionParserRuleCall_0()); }
	ruleJMLPrimaryExpression
	{ after(grammarAccess.getJMLQualifiedExpressionAccess().getJMLPrimaryExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLQualifiedExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLQualifiedExpressionAccess().getGroup_1()); }
	(rule__JMLQualifiedExpression__Group_1__0)*
	{ after(grammarAccess.getJMLQualifiedExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLQualifiedExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLQualifiedExpression__Group_1__0__Impl
	rule__JMLQualifiedExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLQualifiedExpressionAccess().getGroup_1_0()); }
	(rule__JMLQualifiedExpression__Group_1_0__0)
	{ after(grammarAccess.getJMLQualifiedExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLQualifiedExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLQualifiedExpressionAccess().getRightAssignment_1_1()); }
	(rule__JMLQualifiedExpression__RightAssignment_1_1)
	{ after(grammarAccess.getJMLQualifiedExpressionAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLQualifiedExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLQualifiedExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLQualifiedExpressionAccess().getGroup_1_0_0()); }
	(rule__JMLQualifiedExpression__Group_1_0_0__0)
	{ after(grammarAccess.getJMLQualifiedExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLQualifiedExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLQualifiedExpression__Group_1_0_0__0__Impl
	rule__JMLQualifiedExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLQualifiedExpressionAccess().getJMLQualifierLeftAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getJMLQualifiedExpressionAccess().getJMLQualifierLeftAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLQualifiedExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLQualifiedExpressionAccess().getFullStopKeyword_1_0_0_1()); }
	'.'
	{ after(grammarAccess.getJMLQualifiedExpressionAccess().getFullStopKeyword_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_0__0__Impl
	rule__JMLPrimaryExpression__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_0_0()); }
	()
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_0_1()); }
	(rule__JMLPrimaryExpression__Group_0_1__0)
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_0_1__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getAlternatives_0_1_0()); }
	(rule__JMLPrimaryExpression__Alternatives_0_1_0)
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getAlternatives_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_1__0__Impl
	rule__JMLPrimaryExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getExclamationMarkKeyword_1_0()); }
	('!')?
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getExclamationMarkKeyword_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_1__1__Impl
	rule__JMLPrimaryExpression__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getLeftParenthesisKeyword_1_1()); }
	'('
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getLeftParenthesisKeyword_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_1__2__Impl
	rule__JMLPrimaryExpression__Group_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionParserRuleCall_1_2()); }
	ruleJMLExpression
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionParserRuleCall_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_1__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getRightParenthesisKeyword_1_3()); }
	')'
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getRightParenthesisKeyword_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2__0__Impl
	rule__JMLPrimaryExpression__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getJMLFunctionCallAction_2_0()); }
	()
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getJMLFunctionCallAction_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2__1__Impl
	rule__JMLPrimaryExpression__Group_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getExclamationMarkKeyword_2_1()); }
	('!')?
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getExclamationMarkKeyword_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2__2__Impl
	rule__JMLPrimaryExpression__Group_2__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getFuncAssignment_2_2()); }
	(rule__JMLPrimaryExpression__FuncAssignment_2_2)
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getFuncAssignment_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getAlternatives_2_3()); }
	(rule__JMLPrimaryExpression__Alternatives_2_3)*
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getAlternatives_2_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_2_3_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_0__0__Impl
	rule__JMLPrimaryExpression__Group_2_3_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getLeftParenthesisKeyword_2_3_0_0()); }
	'('
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getLeftParenthesisKeyword_2_3_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_0__1__Impl
	rule__JMLPrimaryExpression__Group_2_3_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2_3_0_1()); }
	(rule__JMLPrimaryExpression__Group_2_3_0_1__0)?
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2_3_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getRightParenthesisKeyword_2_3_0_2()); }
	')'
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getRightParenthesisKeyword_2_3_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_2_3_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_0_1__0__Impl
	rule__JMLPrimaryExpression__Group_2_3_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getArgsAssignment_2_3_0_1_0()); }
	(rule__JMLPrimaryExpression__ArgsAssignment_2_3_0_1_0)
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getArgsAssignment_2_3_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2_3_0_1_1()); }
	(rule__JMLPrimaryExpression__Group_2_3_0_1_1__0)*
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getGroup_2_3_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_2_3_0_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_0_1_1__0__Impl
	rule__JMLPrimaryExpression__Group_2_3_0_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getCommaKeyword_2_3_0_1_1_0()); }
	','
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getCommaKeyword_2_3_0_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_0_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_0_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getArgsAssignment_2_3_0_1_1_1()); }
	(rule__JMLPrimaryExpression__ArgsAssignment_2_3_0_1_1_1)
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getArgsAssignment_2_3_0_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_2_3_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_1__0__Impl
	rule__JMLPrimaryExpression__Group_2_3_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getLeftSquareBracketKeyword_2_3_1_0()); }
	'['
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getLeftSquareBracketKeyword_2_3_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_1__1__Impl
	rule__JMLPrimaryExpression__Group_2_3_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getArgsAssignment_2_3_1_1()); }
	(rule__JMLPrimaryExpression__ArgsAssignment_2_3_1_1)
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getArgsAssignment_2_3_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_2_3_1__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_2_3_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getRightSquareBracketKeyword_2_3_1_2()); }
	']'
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getRightSquareBracketKeyword_2_3_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_3__0__Impl
	rule__JMLPrimaryExpression__Group_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_3_0()); }
	()
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getINTTerminalRuleCall_3_1()); }
	RULE_INT
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getINTTerminalRuleCall_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_4__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_4__0__Impl
	rule__JMLPrimaryExpression__Group_4__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_4__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_4_0()); }
	()
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_4_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_4__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_4__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_4__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getSTRINGTerminalRuleCall_4_1()); }
	RULE_STRING
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getSTRINGTerminalRuleCall_4_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_5__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_5__0__Impl
	rule__JMLPrimaryExpression__Group_5__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_5__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_5_0()); }
	()
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_5_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_5__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_5__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_5__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getCHARACTERTerminalRuleCall_5_1()); }
	RULE_CHARACTER
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getCHARACTERTerminalRuleCall_5_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JMLPrimaryExpression__Group_6__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_6__0__Impl
	rule__JMLPrimaryExpression__Group_6__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_6__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_6_0()); }
	()
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getJMLExpressionAction_6_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_6__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JMLPrimaryExpression__Group_6__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__Group_6__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJMLPrimaryExpressionAccess().getNullKeyword_6_1()); }
	'null'
	{ after(grammarAccess.getJMLPrimaryExpressionAccess().getNullKeyword_6_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XImportDeclaration__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XImportDeclaration__Group__0__Impl
	rule__XImportDeclaration__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXImportDeclarationAccess().getImportKeyword_0()); }
	('import')
	{ after(grammarAccess.getXImportDeclarationAccess().getImportKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XImportDeclaration__Group__1__Impl
	rule__XImportDeclaration__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXImportDeclarationAccess().getAlternatives_1()); }
	(rule__XImportDeclaration__Alternatives_1)
	{ after(grammarAccess.getXImportDeclarationAccess().getAlternatives_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XImportDeclaration__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXImportDeclarationAccess().getSemicolonKeyword_2()); }
	(';')*
	{ after(grammarAccess.getXImportDeclarationAccess().getSemicolonKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XImportDeclaration__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XImportDeclaration__Group_1_0__0__Impl
	rule__XImportDeclaration__Group_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXImportDeclarationAccess().getStaticAssignment_1_0_0()); }
	(rule__XImportDeclaration__StaticAssignment_1_0_0)
	{ after(grammarAccess.getXImportDeclarationAccess().getStaticAssignment_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XImportDeclaration__Group_1_0__1__Impl
	rule__XImportDeclaration__Group_1_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXImportDeclarationAccess().getImportedTypeAssignment_1_0_1()); }
	(rule__XImportDeclaration__ImportedTypeAssignment_1_0_1)
	{ after(grammarAccess.getXImportDeclarationAccess().getImportedTypeAssignment_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group_1_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XImportDeclaration__Group_1_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__Group_1_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXImportDeclarationAccess().getAlternatives_1_0_2()); }
	(rule__XImportDeclaration__Alternatives_1_0_2)
	{ after(grammarAccess.getXImportDeclarationAccess().getAlternatives_1_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmTypeReference__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmTypeReference__Group__0__Impl
	rule__JvmTypeReference__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmTypeReference__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmTypeReferenceAccess().getJvmParameterizedTypeReferenceParserRuleCall_0()); }
	ruleJvmParameterizedTypeReference
	{ after(grammarAccess.getJvmTypeReferenceAccess().getJvmParameterizedTypeReferenceParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmTypeReference__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmTypeReference__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmTypeReference__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmTypeReferenceAccess().getGroup_1()); }
	(rule__JvmTypeReference__Group_1__0)*
	{ after(grammarAccess.getJvmTypeReferenceAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmTypeReference__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmTypeReference__Group_1__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmTypeReference__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmTypeReferenceAccess().getGroup_1_0()); }
	(rule__JvmTypeReference__Group_1_0__0)
	{ after(grammarAccess.getJvmTypeReferenceAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmTypeReference__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmTypeReference__Group_1_0__0__Impl
	rule__JvmTypeReference__Group_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmTypeReference__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmTypeReferenceAccess().getJvmGenericArrayTypeReferenceComponentTypeAction_1_0_0()); }
	()
	{ after(grammarAccess.getJvmTypeReferenceAccess().getJvmGenericArrayTypeReferenceComponentTypeAction_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmTypeReference__Group_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmTypeReference__Group_1_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmTypeReference__Group_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmTypeReferenceAccess().getArrayBracketsParserRuleCall_1_0_1()); }
	ruleArrayBrackets
	{ after(grammarAccess.getJvmTypeReferenceAccess().getArrayBracketsParserRuleCall_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__FullJvmFormalParameter__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__FullJvmFormalParameter__Group__0__Impl
	rule__FullJvmFormalParameter__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getFullJvmFormalParameterAccess().getXJJvmFormalParameterAction_0()); }
	()
	{ after(grammarAccess.getFullJvmFormalParameterAccess().getXJJvmFormalParameterAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__FullJvmFormalParameter__Group__1__Impl
	rule__FullJvmFormalParameter__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getFullJvmFormalParameterAccess().getFinalAssignment_1()); }
	(rule__FullJvmFormalParameter__FinalAssignment_1)?
	{ after(grammarAccess.getFullJvmFormalParameterAccess().getFinalAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__FullJvmFormalParameter__Group__2__Impl
	rule__FullJvmFormalParameter__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getFullJvmFormalParameterAccess().getParameterTypeAssignment_2()); }
	(rule__FullJvmFormalParameter__ParameterTypeAssignment_2)
	{ after(grammarAccess.getFullJvmFormalParameterAccess().getParameterTypeAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__FullJvmFormalParameter__Group__3__Impl
	rule__FullJvmFormalParameter__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getFullJvmFormalParameterAccess().getVarArgsAssignment_3()); }
	(rule__FullJvmFormalParameter__VarArgsAssignment_3)?
	{ after(grammarAccess.getFullJvmFormalParameterAccess().getVarArgsAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__FullJvmFormalParameter__Group__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getFullJvmFormalParameterAccess().getNameAssignment_4()); }
	(rule__FullJvmFormalParameter__NameAssignment_4)
	{ after(grammarAccess.getFullJvmFormalParameterAccess().getNameAssignment_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJSemicolonStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJSemicolonStatement__Group__0__Impl
	rule__XJSemicolonStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSemicolonStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJSemicolonStatementAccess().getXJSemicolonStatementAction_0()); }
	()
	{ after(grammarAccess.getXJSemicolonStatementAccess().getXJSemicolonStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSemicolonStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJSemicolonStatement__Group__1__Impl
	rule__XJSemicolonStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSemicolonStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJSemicolonStatementAccess().getExpressionAssignment_1()); }
	(rule__XJSemicolonStatement__ExpressionAssignment_1)
	{ after(grammarAccess.getXJSemicolonStatementAccess().getExpressionAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSemicolonStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJSemicolonStatement__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSemicolonStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJSemicolonStatementAccess().getSemicolonAssignment_2()); }
	(rule__XJSemicolonStatement__SemicolonAssignment_2)?
	{ after(grammarAccess.getXJSemicolonStatementAccess().getSemicolonAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJEmptyStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJEmptyStatement__Group__0__Impl
	rule__XJEmptyStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJEmptyStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJEmptyStatementAccess().getXJSemicolonStatementAction_0()); }
	()
	{ after(grammarAccess.getXJEmptyStatementAccess().getXJSemicolonStatementAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJEmptyStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJEmptyStatement__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJEmptyStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJEmptyStatementAccess().getSemicolonAssignment_1()); }
	(rule__XJEmptyStatement__SemicolonAssignment_1)
	{ after(grammarAccess.getXJEmptyStatementAccess().getSemicolonAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJBranchingStatement__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJBranchingStatement__Group_0__0__Impl
	rule__XJBranchingStatement__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJBranchingStatement__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJBranchingStatementAccess().getXJContinueStatementAction_0_0()); }
	()
	{ after(grammarAccess.getXJBranchingStatementAccess().getXJContinueStatementAction_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJBranchingStatement__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJBranchingStatement__Group_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJBranchingStatement__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJBranchingStatementAccess().getContinueKeyword_0_1()); }
	'continue'
	{ after(grammarAccess.getXJBranchingStatementAccess().getContinueKeyword_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJBranchingStatement__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJBranchingStatement__Group_1__0__Impl
	rule__XJBranchingStatement__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJBranchingStatement__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJBranchingStatementAccess().getXJBreakStatementAction_1_0()); }
	()
	{ after(grammarAccess.getXJBranchingStatementAccess().getXJBreakStatementAction_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJBranchingStatement__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJBranchingStatement__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJBranchingStatement__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJBranchingStatementAccess().getBreakKeyword_1_1()); }
	'break'
	{ after(grammarAccess.getXJBranchingStatementAccess().getBreakKeyword_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBlockExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBlockExpression__Group__0__Impl
	rule__XBlockExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBlockExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBlockExpressionAccess().getXBlockExpressionAction_0()); }
	()
	{ after(grammarAccess.getXBlockExpressionAccess().getXBlockExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBlockExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBlockExpression__Group__1__Impl
	rule__XBlockExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XBlockExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBlockExpressionAccess().getLeftCurlyBracketKeyword_1()); }
	'{'
	{ after(grammarAccess.getXBlockExpressionAccess().getLeftCurlyBracketKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBlockExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBlockExpression__Group__2__Impl
	rule__XBlockExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XBlockExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBlockExpressionAccess().getExpressionsAssignment_2()); }
	(rule__XBlockExpression__ExpressionsAssignment_2)*
	{ after(grammarAccess.getXBlockExpressionAccess().getExpressionsAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBlockExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBlockExpression__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBlockExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBlockExpressionAccess().getRightCurlyBracketKeyword_3()); }
	'}'
	{ after(grammarAccess.getXBlockExpressionAccess().getRightCurlyBracketKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XVariableDeclaration__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group__0__Impl
	rule__XVariableDeclaration__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getGroup_0()); }
	(rule__XVariableDeclaration__Group_0__0)
	{ after(grammarAccess.getXVariableDeclarationAccess().getGroup_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group__1__Impl
	rule__XVariableDeclaration__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getGroup_1()); }
	(rule__XVariableDeclaration__Group_1__0)?
	{ after(grammarAccess.getXVariableDeclarationAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getGroup_2()); }
	(rule__XVariableDeclaration__Group_2__0)*
	{ after(grammarAccess.getXVariableDeclarationAccess().getGroup_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XVariableDeclaration__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getGroup_0_0()); }
	(rule__XVariableDeclaration__Group_0_0__0)
	{ after(grammarAccess.getXVariableDeclarationAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XVariableDeclaration__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_0_0__0__Impl
	rule__XVariableDeclaration__Group_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getXJVariableDeclarationAction_0_0_0()); }
	()
	{ after(grammarAccess.getXVariableDeclarationAccess().getXJVariableDeclarationAction_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_0_0__1__Impl
	rule__XVariableDeclaration__Group_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getFinalAssignment_0_0_1()); }
	(rule__XVariableDeclaration__FinalAssignment_0_0_1)?
	{ after(grammarAccess.getXVariableDeclarationAccess().getFinalAssignment_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_0_0__2__Impl
	rule__XVariableDeclaration__Group_0_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getTypeAssignment_0_0_2()); }
	(rule__XVariableDeclaration__TypeAssignment_0_0_2)
	{ after(grammarAccess.getXVariableDeclarationAccess().getTypeAssignment_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_0_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_0_0__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_0_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getNameAssignment_0_0_3()); }
	(rule__XVariableDeclaration__NameAssignment_0_0_3)
	{ after(grammarAccess.getXVariableDeclarationAccess().getNameAssignment_0_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XVariableDeclaration__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_1__0__Impl
	rule__XVariableDeclaration__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getEqualsSignKeyword_1_0()); }
	'='
	{ after(grammarAccess.getXVariableDeclarationAccess().getEqualsSignKeyword_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getRightAssignment_1_1()); }
	(rule__XVariableDeclaration__RightAssignment_1_1)
	{ after(grammarAccess.getXVariableDeclarationAccess().getRightAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XVariableDeclaration__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_2__0__Impl
	rule__XVariableDeclaration__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getCommaKeyword_2_0()); }
	(',')
	{ after(grammarAccess.getXVariableDeclarationAccess().getCommaKeyword_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XVariableDeclaration__Group_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXVariableDeclarationAccess().getAdditionalVariablesAssignment_2_1()); }
	(rule__XVariableDeclaration__AdditionalVariablesAssignment_2_1)
	{ after(grammarAccess.getXVariableDeclarationAccess().getAdditionalVariablesAssignment_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJAdditionalXVariableDeclaration__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJAdditionalXVariableDeclaration__Group__0__Impl
	rule__XJAdditionalXVariableDeclaration__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getXJAdditionalXVariableDeclarationAction_0()); }
	()
	{ after(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getXJAdditionalXVariableDeclarationAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJAdditionalXVariableDeclaration__Group__1__Impl
	rule__XJAdditionalXVariableDeclaration__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getNameAssignment_1()); }
	(rule__XJAdditionalXVariableDeclaration__NameAssignment_1)
	{ after(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getNameAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJAdditionalXVariableDeclaration__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getGroup_2()); }
	(rule__XJAdditionalXVariableDeclaration__Group_2__0)?
	{ after(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getGroup_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJAdditionalXVariableDeclaration__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJAdditionalXVariableDeclaration__Group_2__0__Impl
	rule__XJAdditionalXVariableDeclaration__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getEqualsSignKeyword_2_0()); }
	'='
	{ after(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getEqualsSignKeyword_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJAdditionalXVariableDeclaration__Group_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getRightAssignment_2_1()); }
	(rule__XJAdditionalXVariableDeclaration__RightAssignment_2_1)
	{ after(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getRightAssignment_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0__0__Impl
	rule__XAssignment__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_0_0()); }
	(rule__XAssignment__Group_0_0__0)
	{ after(grammarAccess.getXAssignmentAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getValueAssignment_0_1()); }
	(rule__XAssignment__ValueAssignment_0_1)
	{ after(grammarAccess.getXAssignmentAccess().getValueAssignment_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_0_0_0()); }
	(rule__XAssignment__Group_0_0_0__0)
	{ after(grammarAccess.getXAssignmentAccess().getGroup_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_0_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0__0__Impl
	rule__XAssignment__Group_0_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getXJAssignmentAction_0_0_0_0()); }
	()
	{ after(grammarAccess.getXAssignmentAccess().getXJAssignmentAction_0_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0__1__Impl
	rule__XAssignment__Group_0_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getFeatureAssignment_0_0_0_1()); }
	(rule__XAssignment__FeatureAssignment_0_0_0_1)
	{ after(grammarAccess.getXAssignmentAccess().getFeatureAssignment_0_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0__2__Impl
	rule__XAssignment__Group_0_0_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getLeftSquareBracketKeyword_0_0_0_2()); }
	'['
	{ after(grammarAccess.getXAssignmentAccess().getLeftSquareBracketKeyword_0_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0__3__Impl
	rule__XAssignment__Group_0_0_0__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getIndexesAssignment_0_0_0_3()); }
	(rule__XAssignment__IndexesAssignment_0_0_0_3)
	{ after(grammarAccess.getXAssignmentAccess().getIndexesAssignment_0_0_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0__4__Impl
	rule__XAssignment__Group_0_0_0__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getRightSquareBracketKeyword_0_0_0_4()); }
	']'
	{ after(grammarAccess.getXAssignmentAccess().getRightSquareBracketKeyword_0_0_0_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0__5__Impl
	rule__XAssignment__Group_0_0_0__6
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_0_0_0_5()); }
	(rule__XAssignment__Group_0_0_0_5__0)*
	{ after(grammarAccess.getXAssignmentAccess().getGroup_0_0_0_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0__6__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getOpSingleAssignParserRuleCall_0_0_0_6()); }
	ruleOpSingleAssign
	{ after(grammarAccess.getXAssignmentAccess().getOpSingleAssignParserRuleCall_0_0_0_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_0_0_0_5__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0_5__0__Impl
	rule__XAssignment__Group_0_0_0_5__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0_5__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getLeftSquareBracketKeyword_0_0_0_5_0()); }
	'['
	{ after(grammarAccess.getXAssignmentAccess().getLeftSquareBracketKeyword_0_0_0_5_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0_5__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0_5__1__Impl
	rule__XAssignment__Group_0_0_0_5__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0_5__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getIndexesAssignment_0_0_0_5_1()); }
	(rule__XAssignment__IndexesAssignment_0_0_0_5_1)
	{ after(grammarAccess.getXAssignmentAccess().getIndexesAssignment_0_0_0_5_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0_5__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_0_0_0_5__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_0_0_0_5__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getRightSquareBracketKeyword_0_0_0_5_2()); }
	']'
	{ after(grammarAccess.getXAssignmentAccess().getRightSquareBracketKeyword_0_0_0_5_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_1__0__Impl
	rule__XAssignment__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_1_0()); }
	(rule__XAssignment__Group_1_0__0)
	{ after(grammarAccess.getXAssignmentAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getValueAssignment_1_1()); }
	(rule__XAssignment__ValueAssignment_1_1)
	{ after(grammarAccess.getXAssignmentAccess().getValueAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_1_0_0()); }
	(rule__XAssignment__Group_1_0_0__0)
	{ after(grammarAccess.getXAssignmentAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_1_0_0__0__Impl
	rule__XAssignment__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getXAssignmentAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXAssignmentAccess().getXAssignmentAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_1_0_0__1__Impl
	rule__XAssignment__Group_1_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XAssignment__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXAssignmentAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_1_0_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_1_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getOpSingleAssignParserRuleCall_1_0_0_2()); }
	ruleOpSingleAssign
	{ after(grammarAccess.getXAssignmentAccess().getOpSingleAssignParserRuleCall_1_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2__0__Impl
	rule__XAssignment__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getXOrExpressionParserRuleCall_2_0()); }
	ruleXOrExpression
	{ after(grammarAccess.getXAssignmentAccess().getXOrExpressionParserRuleCall_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getAlternatives_2_1()); }
	(rule__XAssignment__Alternatives_2_1)?
	{ after(grammarAccess.getXAssignmentAccess().getAlternatives_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_2_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_0__0__Impl
	rule__XAssignment__Group_2_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_2_1_0_0()); }
	(rule__XAssignment__Group_2_1_0_0__0)
	{ after(grammarAccess.getXAssignmentAccess().getGroup_2_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_0__1__Impl
	rule__XAssignment__Group_2_1_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getThenAssignment_2_1_0_1()); }
	(rule__XAssignment__ThenAssignment_2_1_0_1)
	{ after(grammarAccess.getXAssignmentAccess().getThenAssignment_2_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_0__2__Impl
	rule__XAssignment__Group_2_1_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getColonKeyword_2_1_0_2()); }
	':'
	{ after(grammarAccess.getXAssignmentAccess().getColonKeyword_2_1_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_0__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getElseAssignment_2_1_0_3()); }
	(rule__XAssignment__ElseAssignment_2_1_0_3)
	{ after(grammarAccess.getXAssignmentAccess().getElseAssignment_2_1_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_2_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_0_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_2_1_0_0_0()); }
	(rule__XAssignment__Group_2_1_0_0_0__0)
	{ after(grammarAccess.getXAssignmentAccess().getGroup_2_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_2_1_0_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_0_0_0__0__Impl
	rule__XAssignment__Group_2_1_0_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getXJConditionalExpressionIfAction_2_1_0_0_0_0()); }
	()
	{ after(grammarAccess.getXAssignmentAccess().getXJConditionalExpressionIfAction_2_1_0_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_0_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_0_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getQuestionMarkKeyword_2_1_0_0_0_1()); }
	'?'
	{ after(grammarAccess.getXAssignmentAccess().getQuestionMarkKeyword_2_1_0_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_2_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_1__0__Impl
	rule__XAssignment__Group_2_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_2_1_1_0()); }
	(rule__XAssignment__Group_2_1_1_0__0)
	{ after(grammarAccess.getXAssignmentAccess().getGroup_2_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getRightOperandAssignment_2_1_1_1()); }
	(rule__XAssignment__RightOperandAssignment_2_1_1_1)
	{ after(grammarAccess.getXAssignmentAccess().getRightOperandAssignment_2_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_2_1_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getGroup_2_1_1_0_0()); }
	(rule__XAssignment__Group_2_1_1_0_0__0)
	{ after(grammarAccess.getXAssignmentAccess().getGroup_2_1_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAssignment__Group_2_1_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_1_0_0__0__Impl
	rule__XAssignment__Group_2_1_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getXBinaryOperationLeftOperandAction_2_1_1_0_0_0()); }
	()
	{ after(grammarAccess.getXAssignmentAccess().getXBinaryOperationLeftOperandAction_2_1_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAssignment__Group_2_1_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__Group_2_1_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAssignmentAccess().getFeatureAssignment_2_1_1_0_0_1()); }
	(rule__XAssignment__FeatureAssignment_2_1_1_0_0_1)
	{ after(grammarAccess.getXAssignmentAccess().getFeatureAssignment_2_1_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__OpMultiAssign__Group_8__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpMultiAssign__Group_8__0__Impl
	rule__OpMultiAssign__Group_8__1
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_8__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpMultiAssignAccess().getLessThanSignKeyword_8_0()); }
	'<'
	{ after(grammarAccess.getOpMultiAssignAccess().getLessThanSignKeyword_8_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_8__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpMultiAssign__Group_8__1__Impl
	rule__OpMultiAssign__Group_8__2
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_8__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpMultiAssignAccess().getLessThanSignKeyword_8_1()); }
	'<'
	{ after(grammarAccess.getOpMultiAssignAccess().getLessThanSignKeyword_8_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_8__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpMultiAssign__Group_8__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_8__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpMultiAssignAccess().getEqualsSignKeyword_8_2()); }
	'='
	{ after(grammarAccess.getOpMultiAssignAccess().getEqualsSignKeyword_8_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__OpMultiAssign__Group_9__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpMultiAssign__Group_9__0__Impl
	rule__OpMultiAssign__Group_9__1
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_9__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpMultiAssignAccess().getGreaterThanSignKeyword_9_0()); }
	'>'
	{ after(grammarAccess.getOpMultiAssignAccess().getGreaterThanSignKeyword_9_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_9__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpMultiAssign__Group_9__1__Impl
	rule__OpMultiAssign__Group_9__2
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_9__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpMultiAssignAccess().getGreaterThanSignKeyword_9_1()); }
	('>')?
	{ after(grammarAccess.getOpMultiAssignAccess().getGreaterThanSignKeyword_9_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_9__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpMultiAssign__Group_9__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__OpMultiAssign__Group_9__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpMultiAssignAccess().getGreaterThanSignEqualsSignKeyword_9_2()); }
	'>='
	{ after(grammarAccess.getOpMultiAssignAccess().getGreaterThanSignEqualsSignKeyword_9_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJFeatureCallWithArrayAccess__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group__0__Impl
	rule__XJFeatureCallWithArrayAccess__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getXFeatureCallParserRuleCall_0()); }
	ruleXFeatureCall
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getXFeatureCallParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getGroup_1()); }
	(rule__XJFeatureCallWithArrayAccess__Group_1__0)?
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJFeatureCallWithArrayAccess__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getGroup_1_0()); }
	(rule__XJFeatureCallWithArrayAccess__Group_1_0__0)
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJFeatureCallWithArrayAccess__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1_0__0__Impl
	rule__XJFeatureCallWithArrayAccess__Group_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getXJArrayAccessExpressionArrayAction_1_0_0()); }
	()
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getXJArrayAccessExpressionArrayAction_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1_0__1__Impl
	rule__XJFeatureCallWithArrayAccess__Group_1_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getLeftSquareBracketKeyword_1_0_1()); }
	'['
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getLeftSquareBracketKeyword_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1_0__2__Impl
	rule__XJFeatureCallWithArrayAccess__Group_1_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getIndexesAssignment_1_0_2()); }
	(rule__XJFeatureCallWithArrayAccess__IndexesAssignment_1_0_2)
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getIndexesAssignment_1_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1_0__3__Impl
	rule__XJFeatureCallWithArrayAccess__Group_1_0__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getRightSquareBracketKeyword_1_0_3()); }
	']'
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getRightSquareBracketKeyword_1_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1_0__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getGroup_1_0_4()); }
	(rule__XJFeatureCallWithArrayAccess__Group_1_0_4__0)*
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getGroup_1_0_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJFeatureCallWithArrayAccess__Group_1_0_4__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1_0_4__0__Impl
	rule__XJFeatureCallWithArrayAccess__Group_1_0_4__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0_4__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getLeftSquareBracketKeyword_1_0_4_0()); }
	('[')
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getLeftSquareBracketKeyword_1_0_4_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0_4__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1_0_4__1__Impl
	rule__XJFeatureCallWithArrayAccess__Group_1_0_4__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0_4__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getIndexesAssignment_1_0_4_1()); }
	(rule__XJFeatureCallWithArrayAccess__IndexesAssignment_1_0_4_1)
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getIndexesAssignment_1_0_4_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0_4__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJFeatureCallWithArrayAccess__Group_1_0_4__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__Group_1_0_4__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getRightSquareBracketKeyword_1_0_4_2()); }
	']'
	{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getRightSquareBracketKeyword_1_0_4_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XFeatureCall__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group__0__Impl
	rule__XFeatureCall__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getXFeatureCallAction_0()); }
	()
	{ after(grammarAccess.getXFeatureCallAccess().getXFeatureCallAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group__1__Impl
	rule__XFeatureCall__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getFeatureAssignment_1()); }
	(rule__XFeatureCall__FeatureAssignment_1)
	{ after(grammarAccess.getXFeatureCallAccess().getFeatureAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getGroup_2()); }
	(rule__XFeatureCall__Group_2__0)?
	{ after(grammarAccess.getXFeatureCallAccess().getGroup_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XFeatureCall__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group_2__0__Impl
	rule__XFeatureCall__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getExplicitOperationCallAssignment_2_0()); }
	(rule__XFeatureCall__ExplicitOperationCallAssignment_2_0)
	{ after(grammarAccess.getXFeatureCallAccess().getExplicitOperationCallAssignment_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group_2__1__Impl
	rule__XFeatureCall__Group_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getGroup_2_1()); }
	(rule__XFeatureCall__Group_2_1__0)?
	{ after(grammarAccess.getXFeatureCallAccess().getGroup_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group_2__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getRightParenthesisKeyword_2_2()); }
	')'
	{ after(grammarAccess.getXFeatureCallAccess().getRightParenthesisKeyword_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XFeatureCall__Group_2_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group_2_1__0__Impl
	rule__XFeatureCall__Group_2_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsAssignment_2_1_0()); }
	(rule__XFeatureCall__FeatureCallArgumentsAssignment_2_1_0)
	{ after(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsAssignment_2_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group_2_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getGroup_2_1_1()); }
	(rule__XFeatureCall__Group_2_1_1__0)*
	{ after(grammarAccess.getXFeatureCallAccess().getGroup_2_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XFeatureCall__Group_2_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group_2_1_1__0__Impl
	rule__XFeatureCall__Group_2_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getCommaKeyword_2_1_1_0()); }
	','
	{ after(grammarAccess.getXFeatureCallAccess().getCommaKeyword_2_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XFeatureCall__Group_2_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__Group_2_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsAssignment_2_1_1_1()); }
	(rule__XFeatureCall__FeatureCallArgumentsAssignment_2_1_1_1)
	{ after(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsAssignment_2_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XConstructorCall__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group__0__Impl
	rule__XConstructorCall__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getXJConstructorCallAction_0()); }
	()
	{ after(grammarAccess.getXConstructorCallAccess().getXJConstructorCallAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group__1__Impl
	rule__XConstructorCall__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getNewKeyword_1()); }
	'new'
	{ after(grammarAccess.getXConstructorCallAccess().getNewKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group__2__Impl
	rule__XConstructorCall__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getConstructorAssignment_2()); }
	(rule__XConstructorCall__ConstructorAssignment_2)
	{ after(grammarAccess.getXConstructorCallAccess().getConstructorAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group__3__Impl
	rule__XConstructorCall__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getGroup_3()); }
	(rule__XConstructorCall__Group_3__0)?
	{ after(grammarAccess.getXConstructorCallAccess().getGroup_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getGroup_4()); }
	(rule__XConstructorCall__Group_4__0)?
	{ after(grammarAccess.getXConstructorCallAccess().getGroup_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XConstructorCall__Group_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_3__0__Impl
	rule__XConstructorCall__Group_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getExplicitTypeArgumentsAssignment_3_0()); }
	(rule__XConstructorCall__ExplicitTypeArgumentsAssignment_3_0)
	{ after(grammarAccess.getXConstructorCallAccess().getExplicitTypeArgumentsAssignment_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_3__1__Impl
	rule__XConstructorCall__Group_3__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getGroup_3_1()); }
	(rule__XConstructorCall__Group_3_1__0)?
	{ after(grammarAccess.getXConstructorCallAccess().getGroup_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_3__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getGreaterThanSignKeyword_3_2()); }
	'>'
	{ after(grammarAccess.getXConstructorCallAccess().getGreaterThanSignKeyword_3_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XConstructorCall__Group_3_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_3_1__0__Impl
	rule__XConstructorCall__Group_3_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getTypeArgumentsAssignment_3_1_0()); }
	(rule__XConstructorCall__TypeArgumentsAssignment_3_1_0)
	{ after(grammarAccess.getXConstructorCallAccess().getTypeArgumentsAssignment_3_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_3_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getGroup_3_1_1()); }
	(rule__XConstructorCall__Group_3_1_1__0)*
	{ after(grammarAccess.getXConstructorCallAccess().getGroup_3_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XConstructorCall__Group_3_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_3_1_1__0__Impl
	rule__XConstructorCall__Group_3_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getCommaKeyword_3_1_1_0()); }
	','
	{ after(grammarAccess.getXConstructorCallAccess().getCommaKeyword_3_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_3_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_3_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getTypeArgumentsAssignment_3_1_1_1()); }
	(rule__XConstructorCall__TypeArgumentsAssignment_3_1_1_1)
	{ after(grammarAccess.getXConstructorCallAccess().getTypeArgumentsAssignment_3_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XConstructorCall__Group_4__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_4__0__Impl
	rule__XConstructorCall__Group_4__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getExplicitConstructorCallAssignment_4_0()); }
	(rule__XConstructorCall__ExplicitConstructorCallAssignment_4_0)
	{ after(grammarAccess.getXConstructorCallAccess().getExplicitConstructorCallAssignment_4_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_4__1__Impl
	rule__XConstructorCall__Group_4__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getGroup_4_1()); }
	(rule__XConstructorCall__Group_4_1__0)?
	{ after(grammarAccess.getXConstructorCallAccess().getGroup_4_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_4__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getRightParenthesisKeyword_4_2()); }
	')'
	{ after(grammarAccess.getXConstructorCallAccess().getRightParenthesisKeyword_4_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XConstructorCall__Group_4_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_4_1__0__Impl
	rule__XConstructorCall__Group_4_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getArgumentsAssignment_4_1_0()); }
	(rule__XConstructorCall__ArgumentsAssignment_4_1_0)
	{ after(grammarAccess.getXConstructorCallAccess().getArgumentsAssignment_4_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_4_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getGroup_4_1_1()); }
	(rule__XConstructorCall__Group_4_1_1__0)*
	{ after(grammarAccess.getXConstructorCallAccess().getGroup_4_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XConstructorCall__Group_4_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_4_1_1__0__Impl
	rule__XConstructorCall__Group_4_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getCommaKeyword_4_1_1_0()); }
	','
	{ after(grammarAccess.getXConstructorCallAccess().getCommaKeyword_4_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XConstructorCall__Group_4_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__Group_4_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXConstructorCallAccess().getArgumentsAssignment_4_1_1_1()); }
	(rule__XConstructorCall__ArgumentsAssignment_4_1_1_1)
	{ after(grammarAccess.getXConstructorCallAccess().getArgumentsAssignment_4_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJArrayConstructorCall__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group__0__Impl
	rule__XJArrayConstructorCall__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getGroup_0()); }
	(rule__XJArrayConstructorCall__Group_0__0)
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getGroup_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group__1__Impl
	rule__XJArrayConstructorCall__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getIndexesAssignment_1()); }
	(rule__XJArrayConstructorCall__IndexesAssignment_1)?
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getIndexesAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group__2__Impl
	rule__XJArrayConstructorCall__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getRightSquareBracketKeyword_2()); }
	']'
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getRightSquareBracketKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group__3__Impl
	rule__XJArrayConstructorCall__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getGroup_3()); }
	(rule__XJArrayConstructorCall__Group_3__0)*
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getGroup_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getArrayLiteralAssignment_4()); }
	(rule__XJArrayConstructorCall__ArrayLiteralAssignment_4)?
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getArrayLiteralAssignment_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJArrayConstructorCall__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getGroup_0_0()); }
	(rule__XJArrayConstructorCall__Group_0_0__0)
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJArrayConstructorCall__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group_0_0__0__Impl
	rule__XJArrayConstructorCall__Group_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getXJArrayConstructorCallAction_0_0_0()); }
	()
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getXJArrayConstructorCallAction_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group_0_0__1__Impl
	rule__XJArrayConstructorCall__Group_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getNewKeyword_0_0_1()); }
	'new'
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getNewKeyword_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group_0_0__2__Impl
	rule__XJArrayConstructorCall__Group_0_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getTypeAssignment_0_0_2()); }
	(rule__XJArrayConstructorCall__TypeAssignment_0_0_2)
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getTypeAssignment_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_0_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group_0_0__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_0_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getDimensionsAssignment_0_0_3()); }
	(rule__XJArrayConstructorCall__DimensionsAssignment_0_0_3)
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getDimensionsAssignment_0_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJArrayConstructorCall__Group_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group_3__0__Impl
	rule__XJArrayConstructorCall__Group_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getDimensionsAssignment_3_0()); }
	(rule__XJArrayConstructorCall__DimensionsAssignment_3_0)
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getDimensionsAssignment_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group_3__1__Impl
	rule__XJArrayConstructorCall__Group_3__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getIndexesAssignment_3_1()); }
	(rule__XJArrayConstructorCall__IndexesAssignment_3_1)?
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getIndexesAssignment_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_3__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayConstructorCall__Group_3__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__Group_3__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayConstructorCallAccess().getRightSquareBracketKeyword_3_2()); }
	']'
	{ after(grammarAccess.getXJArrayConstructorCallAccess().getRightSquareBracketKeyword_3_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJArrayDimension__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayDimension__Group__0__Impl
	rule__XJArrayDimension__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayDimension__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayDimensionAccess().getXJArrayDimensionAction_0()); }
	()
	{ after(grammarAccess.getXJArrayDimensionAccess().getXJArrayDimensionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayDimension__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayDimension__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayDimension__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayDimensionAccess().getLeftSquareBracketKeyword_1()); }
	'['
	{ after(grammarAccess.getXJArrayDimensionAccess().getLeftSquareBracketKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XCastedExpression__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCastedExpression__Group_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCastedExpressionAccess().getGroup_0_0()); }
	(rule__XCastedExpression__Group_0_0__0)
	{ after(grammarAccess.getXCastedExpressionAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XCastedExpression__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCastedExpression__Group_0_0__0__Impl
	rule__XCastedExpression__Group_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCastedExpressionAccess().getXCastedExpressionAction_0_0_0()); }
	()
	{ after(grammarAccess.getXCastedExpressionAccess().getXCastedExpressionAction_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCastedExpression__Group_0_0__1__Impl
	rule__XCastedExpression__Group_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCastedExpressionAccess().getLeftParenthesisKeyword_0_0_1()); }
	'('
	{ after(grammarAccess.getXCastedExpressionAccess().getLeftParenthesisKeyword_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCastedExpression__Group_0_0__2__Impl
	rule__XCastedExpression__Group_0_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCastedExpressionAccess().getTypeAssignment_0_0_2()); }
	(rule__XCastedExpression__TypeAssignment_0_0_2)
	{ after(grammarAccess.getXCastedExpressionAccess().getTypeAssignment_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCastedExpression__Group_0_0__3__Impl
	rule__XCastedExpression__Group_0_0__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCastedExpressionAccess().getRightParenthesisKeyword_0_0_3()); }
	')'
	{ after(grammarAccess.getXCastedExpressionAccess().getRightParenthesisKeyword_0_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCastedExpression__Group_0_0__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__Group_0_0__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCastedExpressionAccess().getTargetAssignment_0_0_4()); }
	(rule__XCastedExpression__TargetAssignment_0_0_4)
	{ after(grammarAccess.getXCastedExpressionAccess().getTargetAssignment_0_0_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_0__0__Impl
	rule__XPostfixOperation__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getGroup_0_0()); }
	(rule__XPostfixOperation__Group_0_0__0)
	{ after(grammarAccess.getXPostfixOperationAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getOperandAssignment_0_1()); }
	(rule__XPostfixOperation__OperandAssignment_0_1)
	{ after(grammarAccess.getXPostfixOperationAccess().getOperandAssignment_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_0_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getGroup_0_0_0()); }
	(rule__XPostfixOperation__Group_0_0_0__0)
	{ after(grammarAccess.getXPostfixOperationAccess().getGroup_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_0_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_0_0_0__0__Impl
	rule__XPostfixOperation__Group_0_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_0_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getXJPrefixOperationAction_0_0_0_0()); }
	()
	{ after(grammarAccess.getXPostfixOperationAccess().getXJPrefixOperationAction_0_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_0_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_0_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_0_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getFeatureAssignment_0_0_0_1()); }
	(rule__XPostfixOperation__FeatureAssignment_0_0_0_1)
	{ after(grammarAccess.getXPostfixOperationAccess().getFeatureAssignment_0_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1__0__Impl
	rule__XPostfixOperation__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getXMemberFeatureCallParserRuleCall_1_0()); }
	ruleXMemberFeatureCall
	{ after(grammarAccess.getXPostfixOperationAccess().getXMemberFeatureCallParserRuleCall_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getAlternatives_1_1()); }
	(rule__XPostfixOperation__Alternatives_1_1)?
	{ after(grammarAccess.getXPostfixOperationAccess().getAlternatives_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_1_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_0_0()); }
	(rule__XPostfixOperation__Group_1_1_0_0__0)
	{ after(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_1_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_0_0__0__Impl
	rule__XPostfixOperation__Group_1_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getXPostfixOperationOperandAction_1_1_0_0_0()); }
	()
	{ after(grammarAccess.getXPostfixOperationAccess().getXPostfixOperationOperandAction_1_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getFeatureAssignment_1_1_0_0_1()); }
	(rule__XPostfixOperation__FeatureAssignment_1_1_0_0_1)
	{ after(grammarAccess.getXPostfixOperationAccess().getFeatureAssignment_1_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_1_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_1_0()); }
	(rule__XPostfixOperation__Group_1_1_1_0__0)
	{ after(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_1_1_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1_0__0__Impl
	rule__XPostfixOperation__Group_1_1_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getXJArrayAccessExpressionArrayAction_1_1_1_0_0()); }
	()
	{ after(grammarAccess.getXPostfixOperationAccess().getXJArrayAccessExpressionArrayAction_1_1_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1_0__1__Impl
	rule__XPostfixOperation__Group_1_1_1_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getLeftSquareBracketKeyword_1_1_1_0_1()); }
	'['
	{ after(grammarAccess.getXPostfixOperationAccess().getLeftSquareBracketKeyword_1_1_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1_0__2__Impl
	rule__XPostfixOperation__Group_1_1_1_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getIndexesAssignment_1_1_1_0_2()); }
	(rule__XPostfixOperation__IndexesAssignment_1_1_1_0_2)
	{ after(grammarAccess.getXPostfixOperationAccess().getIndexesAssignment_1_1_1_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1_0__3__Impl
	rule__XPostfixOperation__Group_1_1_1_0__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getRightSquareBracketKeyword_1_1_1_0_3()); }
	']'
	{ after(grammarAccess.getXPostfixOperationAccess().getRightSquareBracketKeyword_1_1_1_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1_0__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_1_0_4()); }
	(rule__XPostfixOperation__Group_1_1_1_0_4__0)*
	{ after(grammarAccess.getXPostfixOperationAccess().getGroup_1_1_1_0_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XPostfixOperation__Group_1_1_1_0_4__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1_0_4__0__Impl
	rule__XPostfixOperation__Group_1_1_1_0_4__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0_4__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getLeftSquareBracketKeyword_1_1_1_0_4_0()); }
	('[')
	{ after(grammarAccess.getXPostfixOperationAccess().getLeftSquareBracketKeyword_1_1_1_0_4_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0_4__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1_0_4__1__Impl
	rule__XPostfixOperation__Group_1_1_1_0_4__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0_4__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getIndexesAssignment_1_1_1_0_4_1()); }
	(rule__XPostfixOperation__IndexesAssignment_1_1_1_0_4_1)
	{ after(grammarAccess.getXPostfixOperationAccess().getIndexesAssignment_1_1_1_0_4_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0_4__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XPostfixOperation__Group_1_1_1_0_4__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__Group_1_1_1_0_4__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXPostfixOperationAccess().getRightSquareBracketKeyword_1_1_1_0_4_2()); }
	']'
	{ after(grammarAccess.getXPostfixOperationAccess().getRightSquareBracketKeyword_1_1_1_0_4_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group__0__Impl
	rule__XMemberFeatureCall__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getXPrimaryExpressionParserRuleCall_0()); }
	ruleXPrimaryExpression
	{ after(grammarAccess.getXMemberFeatureCallAccess().getXPrimaryExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getAlternatives_1()); }
	(rule__XMemberFeatureCall__Alternatives_1)*
	{ after(grammarAccess.getXMemberFeatureCallAccess().getAlternatives_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_0_0()); }
	(rule__XMemberFeatureCall__Group_1_0_0__0)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_0_0__0__Impl
	rule__XMemberFeatureCall__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getXJClassObjectTypeExpressionAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXMemberFeatureCallAccess().getXJClassObjectTypeExpressionAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_0_0__1__Impl
	rule__XMemberFeatureCall__Group_1_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getArrayDimensionsAssignment_1_0_0_1()); }
	(rule__XMemberFeatureCall__ArrayDimensionsAssignment_1_0_0_1)*
	{ after(grammarAccess.getXMemberFeatureCallAccess().getArrayDimensionsAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_0_0__2__Impl
	rule__XMemberFeatureCall__Group_1_0_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getFullStopKeyword_1_0_0_2()); }
	'.'
	{ after(grammarAccess.getXMemberFeatureCallAccess().getFullStopKeyword_1_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_0_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_0_0__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_0_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getClassKeyword_1_0_0_3()); }
	'class'
	{ after(grammarAccess.getXMemberFeatureCallAccess().getClassKeyword_1_0_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_1__0__Impl
	rule__XMemberFeatureCall__Group_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_0()); }
	(rule__XMemberFeatureCall__Group_1_1_0__0)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getValueAssignment_1_1_1()); }
	(rule__XMemberFeatureCall__ValueAssignment_1_1_1)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getValueAssignment_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_0_0()); }
	(rule__XMemberFeatureCall__Group_1_1_0_0__0)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_1_0_0__0__Impl
	rule__XMemberFeatureCall__Group_1_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getXAssignmentAssignableAction_1_1_0_0_0()); }
	()
	{ after(grammarAccess.getXMemberFeatureCallAccess().getXAssignmentAssignableAction_1_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_1_0_0__1__Impl
	rule__XMemberFeatureCall__Group_1_1_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getFullStopKeyword_1_1_0_0_1()); }
	'.'
	{ after(grammarAccess.getXMemberFeatureCallAccess().getFullStopKeyword_1_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_1_0_0__2__Impl
	rule__XMemberFeatureCall__Group_1_1_0_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getFeatureAssignment_1_1_0_0_2()); }
	(rule__XMemberFeatureCall__FeatureAssignment_1_1_0_0_2)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getFeatureAssignment_1_1_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1_0_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_1_0_0__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_1_0_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getOpSingleAssignParserRuleCall_1_1_0_0_3()); }
	ruleOpSingleAssign
	{ after(grammarAccess.getXMemberFeatureCallAccess().getOpSingleAssignParserRuleCall_1_1_0_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2__0__Impl
	rule__XMemberFeatureCall__Group_1_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_0()); }
	(rule__XMemberFeatureCall__Group_1_2_0__0)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2__1__Impl
	rule__XMemberFeatureCall__Group_1_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getFeatureAssignment_1_2_1()); }
	(rule__XMemberFeatureCall__FeatureAssignment_1_2_1)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getFeatureAssignment_1_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_2()); }
	(rule__XMemberFeatureCall__Group_1_2_2__0)?
	{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_2_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_0_0()); }
	(rule__XMemberFeatureCall__Group_1_2_0_0__0)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_2_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_0_0__0__Impl
	rule__XMemberFeatureCall__Group_1_2_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getXMemberFeatureCallMemberCallTargetAction_1_2_0_0_0()); }
	()
	{ after(grammarAccess.getXMemberFeatureCallAccess().getXMemberFeatureCallMemberCallTargetAction_1_2_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getFullStopKeyword_1_2_0_0_1()); }
	'.'
	{ after(grammarAccess.getXMemberFeatureCallAccess().getFullStopKeyword_1_2_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_2_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_2__0__Impl
	rule__XMemberFeatureCall__Group_1_2_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getExplicitOperationCallAssignment_1_2_2_0()); }
	(rule__XMemberFeatureCall__ExplicitOperationCallAssignment_1_2_2_0)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getExplicitOperationCallAssignment_1_2_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_2__1__Impl
	rule__XMemberFeatureCall__Group_1_2_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_2_1()); }
	(rule__XMemberFeatureCall__Group_1_2_2_1__0)?
	{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_2__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getRightParenthesisKeyword_1_2_2_2()); }
	')'
	{ after(grammarAccess.getXMemberFeatureCallAccess().getRightParenthesisKeyword_1_2_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_2_2_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_2_1__0__Impl
	rule__XMemberFeatureCall__Group_1_2_2_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsAssignment_1_2_2_1_0()); }
	(rule__XMemberFeatureCall__MemberCallArgumentsAssignment_1_2_2_1_0)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsAssignment_1_2_2_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_2_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_2_1_1()); }
	(rule__XMemberFeatureCall__Group_1_2_2_1_1__0)*
	{ after(grammarAccess.getXMemberFeatureCallAccess().getGroup_1_2_2_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMemberFeatureCall__Group_1_2_2_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_2_1_1__0__Impl
	rule__XMemberFeatureCall__Group_1_2_2_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getCommaKeyword_1_2_2_1_1_0()); }
	','
	{ after(grammarAccess.getXMemberFeatureCallAccess().getCommaKeyword_1_2_2_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMemberFeatureCall__Group_1_2_2_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__Group_1_2_2_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsAssignment_1_2_2_1_1_1()); }
	(rule__XMemberFeatureCall__MemberCallArgumentsAssignment_1_2_2_1_1_1)
	{ after(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsAssignment_1_2_2_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XForLoopExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group__0__Impl
	rule__XForLoopExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getGroup_0()); }
	(rule__XForLoopExpression__Group_0__0)
	{ after(grammarAccess.getXForLoopExpressionAccess().getGroup_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group__1__Impl
	rule__XForLoopExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getForExpressionAssignment_1()); }
	(rule__XForLoopExpression__ForExpressionAssignment_1)
	{ after(grammarAccess.getXForLoopExpressionAccess().getForExpressionAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group__2__Impl
	rule__XForLoopExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getRightParenthesisKeyword_2()); }
	')'
	{ after(grammarAccess.getXForLoopExpressionAccess().getRightParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getEachExpressionAssignment_3()); }
	(rule__XForLoopExpression__EachExpressionAssignment_3)
	{ after(grammarAccess.getXForLoopExpressionAccess().getEachExpressionAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XForLoopExpression__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getGroup_0_0()); }
	(rule__XForLoopExpression__Group_0_0__0)
	{ after(grammarAccess.getXForLoopExpressionAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XForLoopExpression__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group_0_0__0__Impl
	rule__XForLoopExpression__Group_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getXForLoopExpressionAction_0_0_0()); }
	()
	{ after(grammarAccess.getXForLoopExpressionAccess().getXForLoopExpressionAction_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group_0_0__1__Impl
	rule__XForLoopExpression__Group_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getForKeyword_0_0_1()); }
	'for'
	{ after(grammarAccess.getXForLoopExpressionAccess().getForKeyword_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group_0_0__2__Impl
	rule__XForLoopExpression__Group_0_0__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getLeftParenthesisKeyword_0_0_2()); }
	'('
	{ after(grammarAccess.getXForLoopExpressionAccess().getLeftParenthesisKeyword_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group_0_0__3__Impl
	rule__XForLoopExpression__Group_0_0__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getDeclaredParamAssignment_0_0_3()); }
	(rule__XForLoopExpression__DeclaredParamAssignment_0_0_3)
	{ after(grammarAccess.getXForLoopExpressionAccess().getDeclaredParamAssignment_0_0_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XForLoopExpression__Group_0_0__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__Group_0_0__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXForLoopExpressionAccess().getColonKeyword_0_0_4()); }
	':'
	{ after(grammarAccess.getXForLoopExpressionAccess().getColonKeyword_0_0_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBasicForLoopExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__0__Impl
	rule__XBasicForLoopExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getXBasicForLoopExpressionAction_0()); }
	()
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getXBasicForLoopExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__1__Impl
	rule__XBasicForLoopExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getForKeyword_1()); }
	'for'
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getForKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__2__Impl
	rule__XBasicForLoopExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getLeftParenthesisKeyword_2()); }
	'('
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getLeftParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__3__Impl
	rule__XBasicForLoopExpression__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getGroup_3()); }
	(rule__XBasicForLoopExpression__Group_3__0)?
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getGroup_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__4__Impl
	rule__XBasicForLoopExpression__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getSemicolonKeyword_4()); }
	';'
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getSemicolonKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__5__Impl
	rule__XBasicForLoopExpression__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getExpressionAssignment_5()); }
	(rule__XBasicForLoopExpression__ExpressionAssignment_5)?
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getExpressionAssignment_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__6__Impl
	rule__XBasicForLoopExpression__Group__7
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getSemicolonKeyword_6()); }
	';'
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getSemicolonKeyword_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__7__Impl
	rule__XBasicForLoopExpression__Group__8
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getGroup_7()); }
	(rule__XBasicForLoopExpression__Group_7__0)?
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getGroup_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__8
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__8__Impl
	rule__XBasicForLoopExpression__Group__9
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__8__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getRightParenthesisKeyword_8()); }
	')'
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getRightParenthesisKeyword_8()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__9
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group__9__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group__9__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getEachExpressionAssignment_9()); }
	(rule__XBasicForLoopExpression__EachExpressionAssignment_9)
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getEachExpressionAssignment_9()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBasicForLoopExpression__Group_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group_3__0__Impl
	rule__XBasicForLoopExpression__Group_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getInitExpressionsAssignment_3_0()); }
	(rule__XBasicForLoopExpression__InitExpressionsAssignment_3_0)
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getInitExpressionsAssignment_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getGroup_3_1()); }
	(rule__XBasicForLoopExpression__Group_3_1__0)*
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getGroup_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBasicForLoopExpression__Group_3_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group_3_1__0__Impl
	rule__XBasicForLoopExpression__Group_3_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_3_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getCommaKeyword_3_1_0()); }
	','
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getCommaKeyword_3_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_3_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group_3_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_3_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getInitExpressionsAssignment_3_1_1()); }
	(rule__XBasicForLoopExpression__InitExpressionsAssignment_3_1_1)
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getInitExpressionsAssignment_3_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBasicForLoopExpression__Group_7__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group_7__0__Impl
	rule__XBasicForLoopExpression__Group_7__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_7__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getUpdateExpressionsAssignment_7_0()); }
	(rule__XBasicForLoopExpression__UpdateExpressionsAssignment_7_0)
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getUpdateExpressionsAssignment_7_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_7__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group_7__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_7__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getGroup_7_1()); }
	(rule__XBasicForLoopExpression__Group_7_1__0)*
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getGroup_7_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBasicForLoopExpression__Group_7_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group_7_1__0__Impl
	rule__XBasicForLoopExpression__Group_7_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_7_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getCommaKeyword_7_1_0()); }
	','
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getCommaKeyword_7_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_7_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBasicForLoopExpression__Group_7_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__Group_7_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBasicForLoopExpressionAccess().getUpdateExpressionsAssignment_7_1_1()); }
	(rule__XBasicForLoopExpression__UpdateExpressionsAssignment_7_1_1)
	{ after(grammarAccess.getXBasicForLoopExpressionAccess().getUpdateExpressionsAssignment_7_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XIfExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group__0__Impl
	rule__XIfExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getXIfExpressionAction_0()); }
	()
	{ after(grammarAccess.getXIfExpressionAccess().getXIfExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group__1__Impl
	rule__XIfExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getIfKeyword_1()); }
	'if'
	{ after(grammarAccess.getXIfExpressionAccess().getIfKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group__2__Impl
	rule__XIfExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getLeftParenthesisKeyword_2()); }
	'('
	{ after(grammarAccess.getXIfExpressionAccess().getLeftParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group__3__Impl
	rule__XIfExpression__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getIfAssignment_3()); }
	(rule__XIfExpression__IfAssignment_3)
	{ after(grammarAccess.getXIfExpressionAccess().getIfAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group__4__Impl
	rule__XIfExpression__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getRightParenthesisKeyword_4()); }
	')'
	{ after(grammarAccess.getXIfExpressionAccess().getRightParenthesisKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group__5__Impl
	rule__XIfExpression__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getThenAssignment_5()); }
	(rule__XIfExpression__ThenAssignment_5)
	{ after(grammarAccess.getXIfExpressionAccess().getThenAssignment_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group__6__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getGroup_6()); }
	(rule__XIfExpression__Group_6__0)?
	{ after(grammarAccess.getXIfExpressionAccess().getGroup_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XIfExpression__Group_6__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group_6__0__Impl
	rule__XIfExpression__Group_6__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group_6__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getElseKeyword_6_0()); }
	('else')
	{ after(grammarAccess.getXIfExpressionAccess().getElseKeyword_6_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group_6__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XIfExpression__Group_6__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__Group_6__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXIfExpressionAccess().getElseAssignment_6_1()); }
	(rule__XIfExpression__ElseAssignment_6_1)
	{ after(grammarAccess.getXIfExpressionAccess().getElseAssignment_6_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XWhileExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XWhileExpression__Group__0__Impl
	rule__XWhileExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXWhileExpressionAccess().getXWhileExpressionAction_0()); }
	()
	{ after(grammarAccess.getXWhileExpressionAccess().getXWhileExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XWhileExpression__Group__1__Impl
	rule__XWhileExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXWhileExpressionAccess().getWhileKeyword_1()); }
	'while'
	{ after(grammarAccess.getXWhileExpressionAccess().getWhileKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XWhileExpression__Group__2__Impl
	rule__XWhileExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXWhileExpressionAccess().getLeftParenthesisKeyword_2()); }
	'('
	{ after(grammarAccess.getXWhileExpressionAccess().getLeftParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XWhileExpression__Group__3__Impl
	rule__XWhileExpression__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXWhileExpressionAccess().getPredicateAssignment_3()); }
	(rule__XWhileExpression__PredicateAssignment_3)
	{ after(grammarAccess.getXWhileExpressionAccess().getPredicateAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XWhileExpression__Group__4__Impl
	rule__XWhileExpression__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXWhileExpressionAccess().getRightParenthesisKeyword_4()); }
	')'
	{ after(grammarAccess.getXWhileExpressionAccess().getRightParenthesisKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XWhileExpression__Group__5__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXWhileExpressionAccess().getBodyAssignment_5()); }
	(rule__XWhileExpression__BodyAssignment_5)
	{ after(grammarAccess.getXWhileExpressionAccess().getBodyAssignment_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XDoWhileExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XDoWhileExpression__Group__0__Impl
	rule__XDoWhileExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXDoWhileExpressionAccess().getXDoWhileExpressionAction_0()); }
	()
	{ after(grammarAccess.getXDoWhileExpressionAccess().getXDoWhileExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XDoWhileExpression__Group__1__Impl
	rule__XDoWhileExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXDoWhileExpressionAccess().getDoKeyword_1()); }
	'do'
	{ after(grammarAccess.getXDoWhileExpressionAccess().getDoKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XDoWhileExpression__Group__2__Impl
	rule__XDoWhileExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXDoWhileExpressionAccess().getBodyAssignment_2()); }
	(rule__XDoWhileExpression__BodyAssignment_2)
	{ after(grammarAccess.getXDoWhileExpressionAccess().getBodyAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XDoWhileExpression__Group__3__Impl
	rule__XDoWhileExpression__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXDoWhileExpressionAccess().getWhileKeyword_3()); }
	'while'
	{ after(grammarAccess.getXDoWhileExpressionAccess().getWhileKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XDoWhileExpression__Group__4__Impl
	rule__XDoWhileExpression__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXDoWhileExpressionAccess().getLeftParenthesisKeyword_4()); }
	'('
	{ after(grammarAccess.getXDoWhileExpressionAccess().getLeftParenthesisKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XDoWhileExpression__Group__5__Impl
	rule__XDoWhileExpression__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXDoWhileExpressionAccess().getPredicateAssignment_5()); }
	(rule__XDoWhileExpression__PredicateAssignment_5)
	{ after(grammarAccess.getXDoWhileExpressionAccess().getPredicateAssignment_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XDoWhileExpression__Group__6__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXDoWhileExpressionAccess().getRightParenthesisKeyword_6()); }
	')'
	{ after(grammarAccess.getXDoWhileExpressionAccess().getRightParenthesisKeyword_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XSwitchExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__0__Impl
	rule__XSwitchExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getXSwitchExpressionAction_0()); }
	()
	{ after(grammarAccess.getXSwitchExpressionAccess().getXSwitchExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__1__Impl
	rule__XSwitchExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getSwitchKeyword_1()); }
	'switch'
	{ after(grammarAccess.getXSwitchExpressionAccess().getSwitchKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__2__Impl
	rule__XSwitchExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getLeftParenthesisKeyword_2()); }
	'('
	{ after(grammarAccess.getXSwitchExpressionAccess().getLeftParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__3__Impl
	rule__XSwitchExpression__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getSwitchAssignment_3()); }
	(rule__XSwitchExpression__SwitchAssignment_3)
	{ after(grammarAccess.getXSwitchExpressionAccess().getSwitchAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__4__Impl
	rule__XSwitchExpression__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getRightParenthesisKeyword_4()); }
	')'
	{ after(grammarAccess.getXSwitchExpressionAccess().getRightParenthesisKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__5__Impl
	rule__XSwitchExpression__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getLeftCurlyBracketKeyword_5()); }
	'{'
	{ after(grammarAccess.getXSwitchExpressionAccess().getLeftCurlyBracketKeyword_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__6__Impl
	rule__XSwitchExpression__Group__7
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getCasesAssignment_6()); }
	(rule__XSwitchExpression__CasesAssignment_6)*
	{ after(grammarAccess.getXSwitchExpressionAccess().getCasesAssignment_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__7
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__7__Impl
	rule__XSwitchExpression__Group__8
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__7__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getGroup_7()); }
	(rule__XSwitchExpression__Group_7__0)?
	{ after(grammarAccess.getXSwitchExpressionAccess().getGroup_7()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__8
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group__8__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group__8__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getRightCurlyBracketKeyword_8()); }
	'}'
	{ after(grammarAccess.getXSwitchExpressionAccess().getRightCurlyBracketKeyword_8()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XSwitchExpression__Group_7__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group_7__0__Impl
	rule__XSwitchExpression__Group_7__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group_7__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getDefaultKeyword_7_0()); }
	'default'
	{ after(grammarAccess.getXSwitchExpressionAccess().getDefaultKeyword_7_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group_7__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group_7__1__Impl
	rule__XSwitchExpression__Group_7__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group_7__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getColonKeyword_7_1()); }
	':'
	{ after(grammarAccess.getXSwitchExpressionAccess().getColonKeyword_7_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group_7__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSwitchExpression__Group_7__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__Group_7__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSwitchExpressionAccess().getDefaultAssignment_7_2()); }
	(rule__XSwitchExpression__DefaultAssignment_7_2)
	{ after(grammarAccess.getXSwitchExpressionAccess().getDefaultAssignment_7_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XCasePart__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCasePart__Group__0__Impl
	rule__XCasePart__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCasePartAccess().getXCasePartAction_0()); }
	()
	{ after(grammarAccess.getXCasePartAccess().getXCasePartAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCasePart__Group__1__Impl
	rule__XCasePart__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCasePartAccess().getCaseKeyword_1()); }
	'case'
	{ after(grammarAccess.getXCasePartAccess().getCaseKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCasePart__Group__2__Impl
	rule__XCasePart__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCasePartAccess().getCaseAssignment_2()); }
	(rule__XCasePart__CaseAssignment_2)
	{ after(grammarAccess.getXCasePartAccess().getCaseAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCasePart__Group__3__Impl
	rule__XCasePart__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCasePartAccess().getColonKeyword_3()); }
	':'
	{ after(grammarAccess.getXCasePartAccess().getColonKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCasePart__Group__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCasePartAccess().getThenAssignment_4()); }
	(rule__XCasePart__ThenAssignment_4)
	{ after(grammarAccess.getXCasePartAccess().getThenAssignment_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJSwitchStatements__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJSwitchStatements__Group__0__Impl
	rule__XJSwitchStatements__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSwitchStatements__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJSwitchStatementsAccess().getXJSwitchStatementsAction_0()); }
	()
	{ after(grammarAccess.getXJSwitchStatementsAccess().getXJSwitchStatementsAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSwitchStatements__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJSwitchStatements__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSwitchStatements__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJSwitchStatementsAccess().getExpressionsAssignment_1()); }
	(rule__XJSwitchStatements__ExpressionsAssignment_1)*
	{ after(grammarAccess.getXJSwitchStatementsAccess().getExpressionsAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJTryWithResourcesStatement__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group__0__Impl
	rule__XJTryWithResourcesStatement__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getGroup_0()); }
	(rule__XJTryWithResourcesStatement__Group_0__0)
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getGroup_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group__1__Impl
	rule__XJTryWithResourcesStatement__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getResourcesAssignment_1()); }
	(rule__XJTryWithResourcesStatement__ResourcesAssignment_1)*
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getResourcesAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group__2__Impl
	rule__XJTryWithResourcesStatement__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getRightParenthesisKeyword_2()); }
	')'
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getRightParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group__3__Impl
	rule__XJTryWithResourcesStatement__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getExpressionAssignment_3()); }
	(rule__XJTryWithResourcesStatement__ExpressionAssignment_3)
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getExpressionAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group__4__Impl
	rule__XJTryWithResourcesStatement__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getCatchClausesAssignment_4()); }
	(rule__XJTryWithResourcesStatement__CatchClausesAssignment_4)*
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getCatchClausesAssignment_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group__5__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getGroup_5()); }
	(rule__XJTryWithResourcesStatement__Group_5__0)?
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getGroup_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJTryWithResourcesStatement__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getGroup_0_0()); }
	(rule__XJTryWithResourcesStatement__Group_0_0__0)
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJTryWithResourcesStatement__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group_0_0__0__Impl
	rule__XJTryWithResourcesStatement__Group_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getXJTryWithResourcesStatementAction_0_0_0()); }
	()
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getXJTryWithResourcesStatementAction_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group_0_0__1__Impl
	rule__XJTryWithResourcesStatement__Group_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getTryKeyword_0_0_1()); }
	'try'
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getTryKeyword_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group_0_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getOpenParenthesisAssignment_0_0_2()); }
	(rule__XJTryWithResourcesStatement__OpenParenthesisAssignment_0_0_2)
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getOpenParenthesisAssignment_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJTryWithResourcesStatement__Group_5__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group_5__0__Impl
	rule__XJTryWithResourcesStatement__Group_5__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_5__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getFinallyKeyword_5_0()); }
	'finally'
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getFinallyKeyword_5_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_5__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesStatement__Group_5__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__Group_5__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getFinallyExpressionAssignment_5_1()); }
	(rule__XJTryWithResourcesStatement__FinallyExpressionAssignment_5_1)
	{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getFinallyExpressionAssignment_5_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJTryWithResourcesVariableDeclaration__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesVariableDeclaration__Group__0__Impl
	rule__XJTryWithResourcesVariableDeclaration__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getXJTryWithResourcesVariableDeclarationAction_0()); }
	()
	{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getXJTryWithResourcesVariableDeclarationAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesVariableDeclaration__Group__1__Impl
	rule__XJTryWithResourcesVariableDeclaration__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getFinalAssignment_1()); }
	(rule__XJTryWithResourcesVariableDeclaration__FinalAssignment_1)?
	{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getFinalAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesVariableDeclaration__Group__2__Impl
	rule__XJTryWithResourcesVariableDeclaration__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getTypeAssignment_2()); }
	(rule__XJTryWithResourcesVariableDeclaration__TypeAssignment_2)
	{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getTypeAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesVariableDeclaration__Group__3__Impl
	rule__XJTryWithResourcesVariableDeclaration__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getNameAssignment_3()); }
	(rule__XJTryWithResourcesVariableDeclaration__NameAssignment_3)
	{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getNameAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesVariableDeclaration__Group__4__Impl
	rule__XJTryWithResourcesVariableDeclaration__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getEqualsSignKeyword_4()); }
	'='
	{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getEqualsSignKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesVariableDeclaration__Group__5__Impl
	rule__XJTryWithResourcesVariableDeclaration__Group__6
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getRightAssignment_5()); }
	(rule__XJTryWithResourcesVariableDeclaration__RightAssignment_5)
	{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getRightAssignment_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__6
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJTryWithResourcesVariableDeclaration__Group__6__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__Group__6__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getSemicolonAssignment_6()); }
	(rule__XJTryWithResourcesVariableDeclaration__SemicolonAssignment_6)?
	{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getSemicolonAssignment_6()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XTryCatchFinallyExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group__0__Impl
	rule__XTryCatchFinallyExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getXTryCatchFinallyExpressionAction_0()); }
	()
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getXTryCatchFinallyExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group__1__Impl
	rule__XTryCatchFinallyExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getTryKeyword_1()); }
	'try'
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getTryKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group__2__Impl
	rule__XTryCatchFinallyExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getExpressionAssignment_2()); }
	(rule__XTryCatchFinallyExpression__ExpressionAssignment_2)
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getExpressionAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getAlternatives_3()); }
	(rule__XTryCatchFinallyExpression__Alternatives_3)
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getAlternatives_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XTryCatchFinallyExpression__Group_3_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group_3_0__0__Impl
	rule__XTryCatchFinallyExpression__Group_3_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getCatchClausesAssignment_3_0_0()); }
		(rule__XTryCatchFinallyExpression__CatchClausesAssignment_3_0_0)
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getCatchClausesAssignment_3_0_0()); }
	)
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getCatchClausesAssignment_3_0_0()); }
		(rule__XTryCatchFinallyExpression__CatchClausesAssignment_3_0_0)*
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getCatchClausesAssignment_3_0_0()); }
	)
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group_3_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_0_1()); }
	(rule__XTryCatchFinallyExpression__Group_3_0_1__0)?
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getGroup_3_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XTryCatchFinallyExpression__Group_3_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group_3_0_1__0__Impl
	rule__XTryCatchFinallyExpression__Group_3_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyKeyword_3_0_1_0()); }
	('finally')
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyKeyword_3_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group_3_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionAssignment_3_0_1_1()); }
	(rule__XTryCatchFinallyExpression__FinallyExpressionAssignment_3_0_1_1)
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionAssignment_3_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XTryCatchFinallyExpression__Group_3_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group_3_1__0__Impl
	rule__XTryCatchFinallyExpression__Group_3_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyKeyword_3_1_0()); }
	'finally'
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyKeyword_3_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XTryCatchFinallyExpression__Group_3_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__Group_3_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionAssignment_3_1_1()); }
	(rule__XTryCatchFinallyExpression__FinallyExpressionAssignment_3_1_1)
	{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionAssignment_3_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XCatchClause__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCatchClause__Group__0__Impl
	rule__XCatchClause__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCatchClauseAccess().getXCatchClauseAction_0()); }
	()
	{ after(grammarAccess.getXCatchClauseAccess().getXCatchClauseAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCatchClause__Group__1__Impl
	rule__XCatchClause__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCatchClauseAccess().getCatchKeyword_1()); }
	('catch')
	{ after(grammarAccess.getXCatchClauseAccess().getCatchKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCatchClause__Group__2__Impl
	rule__XCatchClause__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCatchClauseAccess().getLeftParenthesisKeyword_2()); }
	'('
	{ after(grammarAccess.getXCatchClauseAccess().getLeftParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCatchClause__Group__3__Impl
	rule__XCatchClause__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCatchClauseAccess().getDeclaredParamAssignment_3()); }
	(rule__XCatchClause__DeclaredParamAssignment_3)
	{ after(grammarAccess.getXCatchClauseAccess().getDeclaredParamAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCatchClause__Group__4__Impl
	rule__XCatchClause__Group__5
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCatchClauseAccess().getRightParenthesisKeyword_4()); }
	')'
	{ after(grammarAccess.getXCatchClauseAccess().getRightParenthesisKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__5
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCatchClause__Group__5__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__Group__5__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCatchClauseAccess().getExpressionAssignment_5()); }
	(rule__XCatchClause__ExpressionAssignment_5)
	{ after(grammarAccess.getXCatchClauseAccess().getExpressionAssignment_5()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XSynchronizedExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSynchronizedExpression__Group__0__Impl
	rule__XSynchronizedExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSynchronizedExpressionAccess().getGroup_0()); }
	(rule__XSynchronizedExpression__Group_0__0)
	{ after(grammarAccess.getXSynchronizedExpressionAccess().getGroup_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSynchronizedExpression__Group__1__Impl
	rule__XSynchronizedExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSynchronizedExpressionAccess().getParamAssignment_1()); }
	(rule__XSynchronizedExpression__ParamAssignment_1)
	{ after(grammarAccess.getXSynchronizedExpressionAccess().getParamAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSynchronizedExpression__Group__2__Impl
	rule__XSynchronizedExpression__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSynchronizedExpressionAccess().getRightParenthesisKeyword_2()); }
	')'
	{ after(grammarAccess.getXSynchronizedExpressionAccess().getRightParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSynchronizedExpression__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSynchronizedExpressionAccess().getExpressionAssignment_3()); }
	(rule__XSynchronizedExpression__ExpressionAssignment_3)
	{ after(grammarAccess.getXSynchronizedExpressionAccess().getExpressionAssignment_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XSynchronizedExpression__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSynchronizedExpression__Group_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSynchronizedExpressionAccess().getGroup_0_0()); }
	(rule__XSynchronizedExpression__Group_0_0__0)
	{ after(grammarAccess.getXSynchronizedExpressionAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XSynchronizedExpression__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSynchronizedExpression__Group_0_0__0__Impl
	rule__XSynchronizedExpression__Group_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSynchronizedExpressionAccess().getXSynchronizedExpressionAction_0_0_0()); }
	()
	{ after(grammarAccess.getXSynchronizedExpressionAccess().getXSynchronizedExpressionAction_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSynchronizedExpression__Group_0_0__1__Impl
	rule__XSynchronizedExpression__Group_0_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSynchronizedExpressionAccess().getSynchronizedKeyword_0_0_1()); }
	'synchronized'
	{ after(grammarAccess.getXSynchronizedExpressionAccess().getSynchronizedKeyword_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group_0_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSynchronizedExpression__Group_0_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__Group_0_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSynchronizedExpressionAccess().getLeftParenthesisKeyword_0_0_2()); }
	'('
	{ after(grammarAccess.getXSynchronizedExpressionAccess().getLeftParenthesisKeyword_0_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJArrayLiteral__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayLiteral__Group__0__Impl
	rule__XJArrayLiteral__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayLiteralAccess().getXJArrayLiteralAction_0()); }
	()
	{ after(grammarAccess.getXJArrayLiteralAccess().getXJArrayLiteralAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayLiteral__Group__1__Impl
	rule__XJArrayLiteral__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayLiteralAccess().getLeftCurlyBracketKeyword_1()); }
	'{'
	{ after(grammarAccess.getXJArrayLiteralAccess().getLeftCurlyBracketKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayLiteral__Group__2__Impl
	rule__XJArrayLiteral__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayLiteralAccess().getGroup_2()); }
	(rule__XJArrayLiteral__Group_2__0)?
	{ after(grammarAccess.getXJArrayLiteralAccess().getGroup_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayLiteral__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayLiteralAccess().getRightCurlyBracketKeyword_3()); }
	'}'
	{ after(grammarAccess.getXJArrayLiteralAccess().getRightCurlyBracketKeyword_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJArrayLiteral__Group_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayLiteral__Group_2__0__Impl
	rule__XJArrayLiteral__Group_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayLiteralAccess().getElementsAssignment_2_0()); }
	(rule__XJArrayLiteral__ElementsAssignment_2_0)
	{ after(grammarAccess.getXJArrayLiteralAccess().getElementsAssignment_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayLiteral__Group_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayLiteralAccess().getGroup_2_1()); }
	(rule__XJArrayLiteral__Group_2_1__0)*
	{ after(grammarAccess.getXJArrayLiteralAccess().getGroup_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XJArrayLiteral__Group_2_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayLiteral__Group_2_1__0__Impl
	rule__XJArrayLiteral__Group_2_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group_2_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayLiteralAccess().getCommaKeyword_2_1_0()); }
	','
	{ after(grammarAccess.getXJArrayLiteralAccess().getCommaKeyword_2_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group_2_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XJArrayLiteral__Group_2_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__Group_2_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXJArrayLiteralAccess().getElementsAssignment_2_1_1()); }
	(rule__XJArrayLiteral__ElementsAssignment_2_1_1)
	{ after(grammarAccess.getXJArrayLiteralAccess().getElementsAssignment_2_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XCharLiteral__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCharLiteral__Group__0__Impl
	rule__XCharLiteral__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XCharLiteral__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCharLiteralAccess().getXJCharLiteralAction_0()); }
	()
	{ after(grammarAccess.getXCharLiteralAccess().getXJCharLiteralAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCharLiteral__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XCharLiteral__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XCharLiteral__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXCharLiteralAccess().getValueAssignment_1()); }
	(rule__XCharLiteral__ValueAssignment_1)
	{ after(grammarAccess.getXCharLiteralAccess().getValueAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XStringLiteral__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XStringLiteral__Group__0__Impl
	rule__XStringLiteral__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XStringLiteral__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXStringLiteralAccess().getXStringLiteralAction_0()); }
	()
	{ after(grammarAccess.getXStringLiteralAccess().getXStringLiteralAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XStringLiteral__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XStringLiteral__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XStringLiteral__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXStringLiteralAccess().getValueAssignment_1()); }
	(rule__XStringLiteral__ValueAssignment_1)
	{ after(grammarAccess.getXStringLiteralAccess().getValueAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__OpOther__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpOther__Group_0__0__Impl
	rule__OpOther__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpOtherAccess().getGreaterThanSignKeyword_0_0()); }
	'>'
	{ after(grammarAccess.getOpOtherAccess().getGreaterThanSignKeyword_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpOther__Group_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpOtherAccess().getAlternatives_0_1()); }
	(rule__OpOther__Alternatives_0_1)
	{ after(grammarAccess.getOpOtherAccess().getAlternatives_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__OpOther__Group_0_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpOther__Group_0_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Group_0_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpOtherAccess().getGroup_0_1_0_0()); }
	(rule__OpOther__Group_0_1_0_0__0)
	{ after(grammarAccess.getOpOtherAccess().getGroup_0_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__OpOther__Group_0_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpOther__Group_0_1_0_0__0__Impl
	rule__OpOther__Group_0_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Group_0_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpOtherAccess().getGreaterThanSignKeyword_0_1_0_0_0()); }
	'>'
	{ after(grammarAccess.getOpOtherAccess().getGreaterThanSignKeyword_0_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Group_0_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpOther__Group_0_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__OpOther__Group_0_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpOtherAccess().getGreaterThanSignKeyword_0_1_0_0_1()); }
	'>'
	{ after(grammarAccess.getOpOtherAccess().getGreaterThanSignKeyword_0_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAndExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAndExpression__Group__0__Impl
	rule__XAndExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAndExpressionAccess().getXBitwiseInclusiveOrExpressionParserRuleCall_0()); }
	ruleXBitwiseInclusiveOrExpression
	{ after(grammarAccess.getXAndExpressionAccess().getXBitwiseInclusiveOrExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAndExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAndExpressionAccess().getGroup_1()); }
	(rule__XAndExpression__Group_1__0)*
	{ after(grammarAccess.getXAndExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAndExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAndExpression__Group_1__0__Impl
	rule__XAndExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAndExpressionAccess().getGroup_1_0()); }
	(rule__XAndExpression__Group_1_0__0)
	{ after(grammarAccess.getXAndExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAndExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAndExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XAndExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXAndExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAndExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAndExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAndExpressionAccess().getGroup_1_0_0()); }
	(rule__XAndExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXAndExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAndExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAndExpression__Group_1_0_0__0__Impl
	rule__XAndExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAndExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXAndExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAndExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAndExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XAndExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXAndExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseInclusiveOrExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseInclusiveOrExpression__Group__0__Impl
	rule__XBitwiseInclusiveOrExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getXBitwiseExclusiveOrExpressionParserRuleCall_0()); }
	ruleXBitwiseExclusiveOrExpression
	{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getXBitwiseExclusiveOrExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseInclusiveOrExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getGroup_1()); }
	(rule__XBitwiseInclusiveOrExpression__Group_1__0)*
	{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseInclusiveOrExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseInclusiveOrExpression__Group_1__0__Impl
	rule__XBitwiseInclusiveOrExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getGroup_1_0()); }
	(rule__XBitwiseInclusiveOrExpression__Group_1_0__0)
	{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseInclusiveOrExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XBitwiseInclusiveOrExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseInclusiveOrExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseInclusiveOrExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getGroup_1_0_0()); }
	(rule__XBitwiseInclusiveOrExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseInclusiveOrExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseInclusiveOrExpression__Group_1_0_0__0__Impl
	rule__XBitwiseInclusiveOrExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseInclusiveOrExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XBitwiseInclusiveOrExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseExclusiveOrExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseExclusiveOrExpression__Group__0__Impl
	rule__XBitwiseExclusiveOrExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getXBitwiseAndExpressionParserRuleCall_0()); }
	ruleXBitwiseAndExpression
	{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getXBitwiseAndExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseExclusiveOrExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getGroup_1()); }
	(rule__XBitwiseExclusiveOrExpression__Group_1__0)*
	{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseExclusiveOrExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseExclusiveOrExpression__Group_1__0__Impl
	rule__XBitwiseExclusiveOrExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getGroup_1_0()); }
	(rule__XBitwiseExclusiveOrExpression__Group_1_0__0)
	{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseExclusiveOrExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XBitwiseExclusiveOrExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseExclusiveOrExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseExclusiveOrExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getGroup_1_0_0()); }
	(rule__XBitwiseExclusiveOrExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseExclusiveOrExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseExclusiveOrExpression__Group_1_0_0__0__Impl
	rule__XBitwiseExclusiveOrExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseExclusiveOrExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XBitwiseExclusiveOrExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseAndExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseAndExpression__Group__0__Impl
	rule__XBitwiseAndExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseAndExpressionAccess().getXEqualityExpressionParserRuleCall_0()); }
	ruleXEqualityExpression
	{ after(grammarAccess.getXBitwiseAndExpressionAccess().getXEqualityExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseAndExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseAndExpressionAccess().getGroup_1()); }
	(rule__XBitwiseAndExpression__Group_1__0)*
	{ after(grammarAccess.getXBitwiseAndExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseAndExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseAndExpression__Group_1__0__Impl
	rule__XBitwiseAndExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseAndExpressionAccess().getGroup_1_0()); }
	(rule__XBitwiseAndExpression__Group_1_0__0)
	{ after(grammarAccess.getXBitwiseAndExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseAndExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseAndExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XBitwiseAndExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXBitwiseAndExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseAndExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseAndExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseAndExpressionAccess().getGroup_1_0_0()); }
	(rule__XBitwiseAndExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXBitwiseAndExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBitwiseAndExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseAndExpression__Group_1_0_0__0__Impl
	rule__XBitwiseAndExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseAndExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXBitwiseAndExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBitwiseAndExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBitwiseAndExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XBitwiseAndExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXBitwiseAndExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValueOrCommaList__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0__0__Impl
	rule__XAnnotationElementValueOrCommaList__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0_0()); }
	(rule__XAnnotationElementValueOrCommaList__Group_0_0__0)
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0__1__Impl
	rule__XAnnotationElementValueOrCommaList__Group_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0_1()); }
	(rule__XAnnotationElementValueOrCommaList__Group_0_1__0)?
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getRightCurlyBracketKeyword_0_2()); }
	'}'
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getRightCurlyBracketKeyword_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValueOrCommaList__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0_0_0()); }
	(rule__XAnnotationElementValueOrCommaList__Group_0_0_0__0)
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValueOrCommaList__Group_0_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0_0_0__0__Impl
	rule__XAnnotationElementValueOrCommaList__Group_0_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getXJArrayLiteralAction_0_0_0_0()); }
	()
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getXJArrayLiteralAction_0_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getLeftCurlyBracketKeyword_0_0_0_1()); }
	'{'
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getLeftCurlyBracketKeyword_0_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValueOrCommaList__Group_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0_1__0__Impl
	rule__XAnnotationElementValueOrCommaList__Group_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getElementsAssignment_0_1_0()); }
	(rule__XAnnotationElementValueOrCommaList__ElementsAssignment_0_1_0)
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getElementsAssignment_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0_1_1()); }
	(rule__XAnnotationElementValueOrCommaList__Group_0_1_1__0)*
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getGroup_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValueOrCommaList__Group_0_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0_1_1__0__Impl
	rule__XAnnotationElementValueOrCommaList__Group_0_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getCommaKeyword_0_1_1_0()); }
	','
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getCommaKeyword_0_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValueOrCommaList__Group_0_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__Group_0_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getElementsAssignment_0_1_1_1()); }
	(rule__XAnnotationElementValueOrCommaList__ElementsAssignment_0_1_1_1)
	{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getElementsAssignment_0_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValue__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0__0__Impl
	rule__XAnnotationElementValue__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getGroup_0_0()); }
	(rule__XAnnotationElementValue__Group_0_0__0)
	{ after(grammarAccess.getXAnnotationElementValueAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0__1__Impl
	rule__XAnnotationElementValue__Group_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getGroup_0_1()); }
	(rule__XAnnotationElementValue__Group_0_1__0)?
	{ after(grammarAccess.getXAnnotationElementValueAccess().getGroup_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getRightCurlyBracketKeyword_0_2()); }
	'}'
	{ after(grammarAccess.getXAnnotationElementValueAccess().getRightCurlyBracketKeyword_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValue__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getGroup_0_0_0()); }
	(rule__XAnnotationElementValue__Group_0_0_0__0)
	{ after(grammarAccess.getXAnnotationElementValueAccess().getGroup_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValue__Group_0_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0_0_0__0__Impl
	rule__XAnnotationElementValue__Group_0_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getXJArrayLiteralAction_0_0_0_0()); }
	()
	{ after(grammarAccess.getXAnnotationElementValueAccess().getXJArrayLiteralAction_0_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getLeftCurlyBracketKeyword_0_0_0_1()); }
	'{'
	{ after(grammarAccess.getXAnnotationElementValueAccess().getLeftCurlyBracketKeyword_0_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValue__Group_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0_1__0__Impl
	rule__XAnnotationElementValue__Group_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getElementsAssignment_0_1_0()); }
	(rule__XAnnotationElementValue__ElementsAssignment_0_1_0)
	{ after(grammarAccess.getXAnnotationElementValueAccess().getElementsAssignment_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getGroup_0_1_1()); }
	(rule__XAnnotationElementValue__Group_0_1_1__0)*
	{ after(grammarAccess.getXAnnotationElementValueAccess().getGroup_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValue__Group_0_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0_1_1__0__Impl
	rule__XAnnotationElementValue__Group_0_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getCommaKeyword_0_1_1_0()); }
	','
	{ after(grammarAccess.getXAnnotationElementValueAccess().getCommaKeyword_0_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValue__Group_0_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__Group_0_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValueAccess().getElementsAssignment_0_1_1_1()); }
	(rule__XAnnotationElementValue__ElementsAssignment_0_1_1_1)
	{ after(grammarAccess.getXAnnotationElementValueAccess().getElementsAssignment_0_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotation__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group__0__Impl
	rule__XAnnotation__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getXAnnotationAction_0()); }
	()
	{ after(grammarAccess.getXAnnotationAccess().getXAnnotationAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group__1__Impl
	rule__XAnnotation__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getCommercialAtKeyword_1()); }
	'@'
	{ after(grammarAccess.getXAnnotationAccess().getCommercialAtKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group__2__Impl
	rule__XAnnotation__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getAnnotationTypeAssignment_2()); }
	(rule__XAnnotation__AnnotationTypeAssignment_2)
	{ after(grammarAccess.getXAnnotationAccess().getAnnotationTypeAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getGroup_3()); }
	(rule__XAnnotation__Group_3__0)?
	{ after(grammarAccess.getXAnnotationAccess().getGroup_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotation__Group_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group_3__0__Impl
	rule__XAnnotation__Group_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getLeftParenthesisKeyword_3_0()); }
	('(')
	{ after(grammarAccess.getXAnnotationAccess().getLeftParenthesisKeyword_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group_3__1__Impl
	rule__XAnnotation__Group_3__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getAlternatives_3_1()); }
	(rule__XAnnotation__Alternatives_3_1)?
	{ after(grammarAccess.getXAnnotationAccess().getAlternatives_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group_3__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getRightParenthesisKeyword_3_2()); }
	')'
	{ after(grammarAccess.getXAnnotationAccess().getRightParenthesisKeyword_3_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotation__Group_3_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group_3_1_0__0__Impl
	rule__XAnnotation__Group_3_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getElementValuePairsAssignment_3_1_0_0()); }
	(rule__XAnnotation__ElementValuePairsAssignment_3_1_0_0)
	{ after(grammarAccess.getXAnnotationAccess().getElementValuePairsAssignment_3_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group_3_1_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getGroup_3_1_0_1()); }
	(rule__XAnnotation__Group_3_1_0_1__0)*
	{ after(grammarAccess.getXAnnotationAccess().getGroup_3_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotation__Group_3_1_0_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group_3_1_0_1__0__Impl
	rule__XAnnotation__Group_3_1_0_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3_1_0_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getCommaKeyword_3_1_0_1_0()); }
	','
	{ after(grammarAccess.getXAnnotationAccess().getCommaKeyword_3_1_0_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3_1_0_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotation__Group_3_1_0_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__Group_3_1_0_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationAccess().getElementValuePairsAssignment_3_1_0_1_1()); }
	(rule__XAnnotation__ElementValuePairsAssignment_3_1_0_1_1)
	{ after(grammarAccess.getXAnnotationAccess().getElementValuePairsAssignment_3_1_0_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValuePair__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValuePair__Group__0__Impl
	rule__XAnnotationElementValuePair__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValuePairAccess().getGroup_0()); }
	(rule__XAnnotationElementValuePair__Group_0__0)
	{ after(grammarAccess.getXAnnotationElementValuePairAccess().getGroup_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValuePair__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValuePairAccess().getValueAssignment_1()); }
	(rule__XAnnotationElementValuePair__ValueAssignment_1)
	{ after(grammarAccess.getXAnnotationElementValuePairAccess().getValueAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValuePair__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValuePair__Group_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValuePairAccess().getGroup_0_0()); }
	(rule__XAnnotationElementValuePair__Group_0_0__0)
	{ after(grammarAccess.getXAnnotationElementValuePairAccess().getGroup_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAnnotationElementValuePair__Group_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValuePair__Group_0_0__0__Impl
	rule__XAnnotationElementValuePair__Group_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__Group_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValuePairAccess().getElementAssignment_0_0_0()); }
	(rule__XAnnotationElementValuePair__ElementAssignment_0_0_0)
	{ after(grammarAccess.getXAnnotationElementValuePairAccess().getElementAssignment_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__Group_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAnnotationElementValuePair__Group_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__Group_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAnnotationElementValuePairAccess().getEqualsSignKeyword_0_0_1()); }
	'='
	{ after(grammarAccess.getXAnnotationElementValuePairAccess().getEqualsSignKeyword_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XOrExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOrExpression__Group__0__Impl
	rule__XOrExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOrExpressionAccess().getXAndExpressionParserRuleCall_0()); }
	ruleXAndExpression
	{ after(grammarAccess.getXOrExpressionAccess().getXAndExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOrExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOrExpressionAccess().getGroup_1()); }
	(rule__XOrExpression__Group_1__0)*
	{ after(grammarAccess.getXOrExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XOrExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOrExpression__Group_1__0__Impl
	rule__XOrExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOrExpressionAccess().getGroup_1_0()); }
	(rule__XOrExpression__Group_1_0__0)
	{ after(grammarAccess.getXOrExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOrExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOrExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XOrExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXOrExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XOrExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOrExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOrExpressionAccess().getGroup_1_0_0()); }
	(rule__XOrExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXOrExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XOrExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOrExpression__Group_1_0_0__0__Impl
	rule__XOrExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOrExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXOrExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOrExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOrExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XOrExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXOrExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XEqualityExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XEqualityExpression__Group__0__Impl
	rule__XEqualityExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXEqualityExpressionAccess().getXRelationalExpressionParserRuleCall_0()); }
	ruleXRelationalExpression
	{ after(grammarAccess.getXEqualityExpressionAccess().getXRelationalExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XEqualityExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXEqualityExpressionAccess().getGroup_1()); }
	(rule__XEqualityExpression__Group_1__0)*
	{ after(grammarAccess.getXEqualityExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XEqualityExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XEqualityExpression__Group_1__0__Impl
	rule__XEqualityExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXEqualityExpressionAccess().getGroup_1_0()); }
	(rule__XEqualityExpression__Group_1_0__0)
	{ after(grammarAccess.getXEqualityExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XEqualityExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXEqualityExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XEqualityExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXEqualityExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XEqualityExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XEqualityExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXEqualityExpressionAccess().getGroup_1_0_0()); }
	(rule__XEqualityExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXEqualityExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XEqualityExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XEqualityExpression__Group_1_0_0__0__Impl
	rule__XEqualityExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXEqualityExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXEqualityExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XEqualityExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXEqualityExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XEqualityExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXEqualityExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XRelationalExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group__0__Impl
	rule__XRelationalExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getXOtherOperatorExpressionParserRuleCall_0()); }
	ruleXOtherOperatorExpression
	{ after(grammarAccess.getXRelationalExpressionAccess().getXOtherOperatorExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getAlternatives_1()); }
	(rule__XRelationalExpression__Alternatives_1)*
	{ after(grammarAccess.getXRelationalExpressionAccess().getAlternatives_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XRelationalExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_0__0__Impl
	rule__XRelationalExpression__Group_1_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0_0()); }
	(rule__XRelationalExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getTypeAssignment_1_0_1()); }
	(rule__XRelationalExpression__TypeAssignment_1_0_1)
	{ after(grammarAccess.getXRelationalExpressionAccess().getTypeAssignment_1_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XRelationalExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_0_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0_0_0()); }
	(rule__XRelationalExpression__Group_1_0_0_0__0)
	{ after(grammarAccess.getXRelationalExpressionAccess().getGroup_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XRelationalExpression__Group_1_0_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_0_0_0__0__Impl
	rule__XRelationalExpression__Group_1_0_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_0_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getXInstanceOfExpressionExpressionAction_1_0_0_0_0()); }
	()
	{ after(grammarAccess.getXRelationalExpressionAccess().getXInstanceOfExpressionExpressionAction_1_0_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_0_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_0_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_0_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getInstanceofKeyword_1_0_0_0_1()); }
	'instanceof'
	{ after(grammarAccess.getXRelationalExpressionAccess().getInstanceofKeyword_1_0_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XRelationalExpression__Group_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_1__0__Impl
	rule__XRelationalExpression__Group_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1_0()); }
	(rule__XRelationalExpression__Group_1_1_0__0)
	{ after(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getRightOperandAssignment_1_1_1()); }
	(rule__XRelationalExpression__RightOperandAssignment_1_1_1)
	{ after(grammarAccess.getXRelationalExpressionAccess().getRightOperandAssignment_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XRelationalExpression__Group_1_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1_0_0()); }
	(rule__XRelationalExpression__Group_1_1_0_0__0)
	{ after(grammarAccess.getXRelationalExpressionAccess().getGroup_1_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XRelationalExpression__Group_1_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_1_0_0__0__Impl
	rule__XRelationalExpression__Group_1_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getXBinaryOperationLeftOperandAction_1_1_0_0_0()); }
	()
	{ after(grammarAccess.getXRelationalExpressionAccess().getXBinaryOperationLeftOperandAction_1_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XRelationalExpression__Group_1_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__Group_1_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXRelationalExpressionAccess().getFeatureAssignment_1_1_0_0_1()); }
	(rule__XRelationalExpression__FeatureAssignment_1_1_0_0_1)
	{ after(grammarAccess.getXRelationalExpressionAccess().getFeatureAssignment_1_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__OpCompare__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpCompare__Group_1__0__Impl
	rule__OpCompare__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__OpCompare__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpCompareAccess().getLessThanSignKeyword_1_0()); }
	'<'
	{ after(grammarAccess.getOpCompareAccess().getLessThanSignKeyword_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__OpCompare__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__OpCompare__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__OpCompare__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getOpCompareAccess().getEqualsSignKeyword_1_1()); }
	'='
	{ after(grammarAccess.getOpCompareAccess().getEqualsSignKeyword_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XOtherOperatorExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOtherOperatorExpression__Group__0__Impl
	rule__XOtherOperatorExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOtherOperatorExpressionAccess().getXAdditiveExpressionParserRuleCall_0()); }
	ruleXAdditiveExpression
	{ after(grammarAccess.getXOtherOperatorExpressionAccess().getXAdditiveExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOtherOperatorExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1()); }
	(rule__XOtherOperatorExpression__Group_1__0)*
	{ after(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XOtherOperatorExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOtherOperatorExpression__Group_1__0__Impl
	rule__XOtherOperatorExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1_0()); }
	(rule__XOtherOperatorExpression__Group_1_0__0)
	{ after(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOtherOperatorExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOtherOperatorExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XOtherOperatorExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXOtherOperatorExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XOtherOperatorExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOtherOperatorExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1_0_0()); }
	(rule__XOtherOperatorExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXOtherOperatorExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XOtherOperatorExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOtherOperatorExpression__Group_1_0_0__0__Impl
	rule__XOtherOperatorExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOtherOperatorExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXOtherOperatorExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XOtherOperatorExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXOtherOperatorExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XOtherOperatorExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXOtherOperatorExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAdditiveExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAdditiveExpression__Group__0__Impl
	rule__XAdditiveExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAdditiveExpressionAccess().getXMultiplicativeExpressionParserRuleCall_0()); }
	ruleXMultiplicativeExpression
	{ after(grammarAccess.getXAdditiveExpressionAccess().getXMultiplicativeExpressionParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAdditiveExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAdditiveExpressionAccess().getGroup_1()); }
	(rule__XAdditiveExpression__Group_1__0)*
	{ after(grammarAccess.getXAdditiveExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAdditiveExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAdditiveExpression__Group_1__0__Impl
	rule__XAdditiveExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAdditiveExpressionAccess().getGroup_1_0()); }
	(rule__XAdditiveExpression__Group_1_0__0)
	{ after(grammarAccess.getXAdditiveExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAdditiveExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAdditiveExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XAdditiveExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXAdditiveExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAdditiveExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAdditiveExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAdditiveExpressionAccess().getGroup_1_0_0()); }
	(rule__XAdditiveExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXAdditiveExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XAdditiveExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAdditiveExpression__Group_1_0_0__0__Impl
	rule__XAdditiveExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAdditiveExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXAdditiveExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XAdditiveExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXAdditiveExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XAdditiveExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXAdditiveExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMultiplicativeExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMultiplicativeExpression__Group__0__Impl
	rule__XMultiplicativeExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMultiplicativeExpressionAccess().getXUnaryOperationParserRuleCall_0()); }
	ruleXUnaryOperation
	{ after(grammarAccess.getXMultiplicativeExpressionAccess().getXUnaryOperationParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMultiplicativeExpression__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1()); }
	(rule__XMultiplicativeExpression__Group_1__0)*
	{ after(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMultiplicativeExpression__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMultiplicativeExpression__Group_1__0__Impl
	rule__XMultiplicativeExpression__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1_0()); }
	(rule__XMultiplicativeExpression__Group_1_0__0)
	{ after(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMultiplicativeExpression__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMultiplicativeExpressionAccess().getRightOperandAssignment_1_1()); }
	(rule__XMultiplicativeExpression__RightOperandAssignment_1_1)
	{ after(grammarAccess.getXMultiplicativeExpressionAccess().getRightOperandAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMultiplicativeExpression__Group_1_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMultiplicativeExpression__Group_1_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group_1_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1_0_0()); }
	(rule__XMultiplicativeExpression__Group_1_0_0__0)
	{ after(grammarAccess.getXMultiplicativeExpressionAccess().getGroup_1_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XMultiplicativeExpression__Group_1_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMultiplicativeExpression__Group_1_0_0__0__Impl
	rule__XMultiplicativeExpression__Group_1_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group_1_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMultiplicativeExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
	()
	{ after(grammarAccess.getXMultiplicativeExpressionAccess().getXBinaryOperationLeftOperandAction_1_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group_1_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XMultiplicativeExpression__Group_1_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__Group_1_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXMultiplicativeExpressionAccess().getFeatureAssignment_1_0_0_1()); }
	(rule__XMultiplicativeExpression__FeatureAssignment_1_0_0_1)
	{ after(grammarAccess.getXMultiplicativeExpressionAccess().getFeatureAssignment_1_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XUnaryOperation__Group_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XUnaryOperation__Group_0__0__Impl
	rule__XUnaryOperation__Group_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XUnaryOperation__Group_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXUnaryOperationAccess().getXUnaryOperationAction_0_0()); }
	()
	{ after(grammarAccess.getXUnaryOperationAccess().getXUnaryOperationAction_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XUnaryOperation__Group_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XUnaryOperation__Group_0__1__Impl
	rule__XUnaryOperation__Group_0__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XUnaryOperation__Group_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXUnaryOperationAccess().getFeatureAssignment_0_1()); }
	(rule__XUnaryOperation__FeatureAssignment_0_1)
	{ after(grammarAccess.getXUnaryOperationAccess().getFeatureAssignment_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XUnaryOperation__Group_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XUnaryOperation__Group_0__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XUnaryOperation__Group_0__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXUnaryOperationAccess().getOperandAssignment_0_2()); }
	(rule__XUnaryOperation__OperandAssignment_0_2)
	{ after(grammarAccess.getXUnaryOperationAccess().getOperandAssignment_0_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XSetLiteral__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group__0__Impl
	rule__XSetLiteral__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getXSetLiteralAction_0()); }
	()
	{ after(grammarAccess.getXSetLiteralAccess().getXSetLiteralAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group__1__Impl
	rule__XSetLiteral__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getNumberSignKeyword_1()); }
	'#'
	{ after(grammarAccess.getXSetLiteralAccess().getNumberSignKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group__2__Impl
	rule__XSetLiteral__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getLeftCurlyBracketKeyword_2()); }
	'{'
	{ after(grammarAccess.getXSetLiteralAccess().getLeftCurlyBracketKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group__3__Impl
	rule__XSetLiteral__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getGroup_3()); }
	(rule__XSetLiteral__Group_3__0)?
	{ after(grammarAccess.getXSetLiteralAccess().getGroup_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getRightCurlyBracketKeyword_4()); }
	'}'
	{ after(grammarAccess.getXSetLiteralAccess().getRightCurlyBracketKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XSetLiteral__Group_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group_3__0__Impl
	rule__XSetLiteral__Group_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getElementsAssignment_3_0()); }
	(rule__XSetLiteral__ElementsAssignment_3_0)
	{ after(grammarAccess.getXSetLiteralAccess().getElementsAssignment_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getGroup_3_1()); }
	(rule__XSetLiteral__Group_3_1__0)*
	{ after(grammarAccess.getXSetLiteralAccess().getGroup_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XSetLiteral__Group_3_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group_3_1__0__Impl
	rule__XSetLiteral__Group_3_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group_3_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getCommaKeyword_3_1_0()); }
	','
	{ after(grammarAccess.getXSetLiteralAccess().getCommaKeyword_3_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group_3_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XSetLiteral__Group_3_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__Group_3_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXSetLiteralAccess().getElementsAssignment_3_1_1()); }
	(rule__XSetLiteral__ElementsAssignment_3_1_1)
	{ after(grammarAccess.getXSetLiteralAccess().getElementsAssignment_3_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XListLiteral__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group__0__Impl
	rule__XListLiteral__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getXListLiteralAction_0()); }
	()
	{ after(grammarAccess.getXListLiteralAccess().getXListLiteralAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group__1__Impl
	rule__XListLiteral__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getNumberSignKeyword_1()); }
	'#'
	{ after(grammarAccess.getXListLiteralAccess().getNumberSignKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group__2__Impl
	rule__XListLiteral__Group__3
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getLeftSquareBracketKeyword_2()); }
	'['
	{ after(grammarAccess.getXListLiteralAccess().getLeftSquareBracketKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group__3__Impl
	rule__XListLiteral__Group__4
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getGroup_3()); }
	(rule__XListLiteral__Group_3__0)?
	{ after(grammarAccess.getXListLiteralAccess().getGroup_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getRightSquareBracketKeyword_4()); }
	']'
	{ after(grammarAccess.getXListLiteralAccess().getRightSquareBracketKeyword_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XListLiteral__Group_3__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group_3__0__Impl
	rule__XListLiteral__Group_3__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group_3__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getElementsAssignment_3_0()); }
	(rule__XListLiteral__ElementsAssignment_3_0)
	{ after(grammarAccess.getXListLiteralAccess().getElementsAssignment_3_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group_3__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group_3__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group_3__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getGroup_3_1()); }
	(rule__XListLiteral__Group_3_1__0)*
	{ after(grammarAccess.getXListLiteralAccess().getGroup_3_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XListLiteral__Group_3_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group_3_1__0__Impl
	rule__XListLiteral__Group_3_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group_3_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getCommaKeyword_3_1_0()); }
	','
	{ after(grammarAccess.getXListLiteralAccess().getCommaKeyword_3_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group_3_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XListLiteral__Group_3_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__Group_3_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXListLiteralAccess().getElementsAssignment_3_1_1()); }
	(rule__XListLiteral__ElementsAssignment_3_1_1)
	{ after(grammarAccess.getXListLiteralAccess().getElementsAssignment_3_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XExpressionInClosure__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XExpressionInClosure__Group__0__Impl
	rule__XExpressionInClosure__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XExpressionInClosure__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXExpressionInClosureAccess().getXBlockExpressionAction_0()); }
	()
	{ after(grammarAccess.getXExpressionInClosureAccess().getXBlockExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XExpressionInClosure__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XExpressionInClosure__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XExpressionInClosure__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXExpressionInClosureAccess().getGroup_1()); }
	(rule__XExpressionInClosure__Group_1__0)*
	{ after(grammarAccess.getXExpressionInClosureAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XExpressionInClosure__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XExpressionInClosure__Group_1__0__Impl
	rule__XExpressionInClosure__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XExpressionInClosure__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXExpressionInClosureAccess().getExpressionsAssignment_1_0()); }
	(rule__XExpressionInClosure__ExpressionsAssignment_1_0)
	{ after(grammarAccess.getXExpressionInClosureAccess().getExpressionsAssignment_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XExpressionInClosure__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XExpressionInClosure__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XExpressionInClosure__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXExpressionInClosureAccess().getSemicolonKeyword_1_1()); }
	(';')?
	{ after(grammarAccess.getXExpressionInClosureAccess().getSemicolonKeyword_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XParenthesizedExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XParenthesizedExpression__Group__0__Impl
	rule__XParenthesizedExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XParenthesizedExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXParenthesizedExpressionAccess().getLeftParenthesisKeyword_0()); }
	'('
	{ after(grammarAccess.getXParenthesizedExpressionAccess().getLeftParenthesisKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XParenthesizedExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XParenthesizedExpression__Group__1__Impl
	rule__XParenthesizedExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XParenthesizedExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXParenthesizedExpressionAccess().getXExpressionParserRuleCall_1()); }
	ruleXExpression
	{ after(grammarAccess.getXParenthesizedExpressionAccess().getXExpressionParserRuleCall_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XParenthesizedExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XParenthesizedExpression__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XParenthesizedExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXParenthesizedExpressionAccess().getRightParenthesisKeyword_2()); }
	')'
	{ after(grammarAccess.getXParenthesizedExpressionAccess().getRightParenthesisKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmFormalParameter__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmFormalParameter__Group__0__Impl
	rule__JvmFormalParameter__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmFormalParameter__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmFormalParameterAccess().getParameterTypeAssignment_0()); }
	(rule__JvmFormalParameter__ParameterTypeAssignment_0)?
	{ after(grammarAccess.getJvmFormalParameterAccess().getParameterTypeAssignment_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmFormalParameter__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmFormalParameter__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmFormalParameter__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmFormalParameterAccess().getNameAssignment_1()); }
	(rule__JvmFormalParameter__NameAssignment_1)
	{ after(grammarAccess.getJvmFormalParameterAccess().getNameAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XBooleanLiteral__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBooleanLiteral__Group__0__Impl
	rule__XBooleanLiteral__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XBooleanLiteral__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBooleanLiteralAccess().getXBooleanLiteralAction_0()); }
	()
	{ after(grammarAccess.getXBooleanLiteralAccess().getXBooleanLiteralAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBooleanLiteral__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XBooleanLiteral__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XBooleanLiteral__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXBooleanLiteralAccess().getAlternatives_1()); }
	(rule__XBooleanLiteral__Alternatives_1)
	{ after(grammarAccess.getXBooleanLiteralAccess().getAlternatives_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XNullLiteral__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XNullLiteral__Group__0__Impl
	rule__XNullLiteral__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XNullLiteral__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXNullLiteralAccess().getXNullLiteralAction_0()); }
	()
	{ after(grammarAccess.getXNullLiteralAccess().getXNullLiteralAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XNullLiteral__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XNullLiteral__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XNullLiteral__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXNullLiteralAccess().getNullKeyword_1()); }
	'null'
	{ after(grammarAccess.getXNullLiteralAccess().getNullKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XNumberLiteral__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XNumberLiteral__Group__0__Impl
	rule__XNumberLiteral__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XNumberLiteral__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXNumberLiteralAccess().getXNumberLiteralAction_0()); }
	()
	{ after(grammarAccess.getXNumberLiteralAccess().getXNumberLiteralAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XNumberLiteral__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XNumberLiteral__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XNumberLiteral__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXNumberLiteralAccess().getValueAssignment_1()); }
	(rule__XNumberLiteral__ValueAssignment_1)
	{ after(grammarAccess.getXNumberLiteralAccess().getValueAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XThrowExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XThrowExpression__Group__0__Impl
	rule__XThrowExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XThrowExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXThrowExpressionAccess().getXThrowExpressionAction_0()); }
	()
	{ after(grammarAccess.getXThrowExpressionAccess().getXThrowExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XThrowExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XThrowExpression__Group__1__Impl
	rule__XThrowExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XThrowExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXThrowExpressionAccess().getThrowKeyword_1()); }
	'throw'
	{ after(grammarAccess.getXThrowExpressionAccess().getThrowKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XThrowExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XThrowExpression__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XThrowExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXThrowExpressionAccess().getExpressionAssignment_2()); }
	(rule__XThrowExpression__ExpressionAssignment_2)
	{ after(grammarAccess.getXThrowExpressionAccess().getExpressionAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__XReturnExpression__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XReturnExpression__Group__0__Impl
	rule__XReturnExpression__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__XReturnExpression__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXReturnExpressionAccess().getXReturnExpressionAction_0()); }
	()
	{ after(grammarAccess.getXReturnExpressionAccess().getXReturnExpressionAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XReturnExpression__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XReturnExpression__Group__1__Impl
	rule__XReturnExpression__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__XReturnExpression__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXReturnExpressionAccess().getReturnKeyword_1()); }
	'return'
	{ after(grammarAccess.getXReturnExpressionAccess().getReturnKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__XReturnExpression__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__XReturnExpression__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__XReturnExpression__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getXReturnExpressionAccess().getExpressionAssignment_2()); }
	(rule__XReturnExpression__ExpressionAssignment_2)?
	{ after(grammarAccess.getXReturnExpressionAccess().getExpressionAssignment_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__QualifiedName__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedName__Group__0__Impl
	rule__QualifiedName__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedName__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameAccess().getValidIDParserRuleCall_0()); }
	ruleValidID
	{ after(grammarAccess.getQualifiedNameAccess().getValidIDParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedName__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedName__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedName__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameAccess().getGroup_1()); }
	(rule__QualifiedName__Group_1__0)*
	{ after(grammarAccess.getQualifiedNameAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__QualifiedName__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedName__Group_1__0__Impl
	rule__QualifiedName__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedName__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameAccess().getFullStopKeyword_1_0()); }
	('.')
	{ after(grammarAccess.getQualifiedNameAccess().getFullStopKeyword_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedName__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedName__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedName__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameAccess().getValidIDParserRuleCall_1_1()); }
	ruleValidID
	{ after(grammarAccess.getQualifiedNameAccess().getValidIDParserRuleCall_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Number__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Number__Group_1__0__Impl
	rule__Number__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getNumberAccess().getAlternatives_1_0()); }
	(rule__Number__Alternatives_1_0)
	{ after(grammarAccess.getNumberAccess().getAlternatives_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Number__Group_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getNumberAccess().getGroup_1_1()); }
	(rule__Number__Group_1_1__0)?
	{ after(grammarAccess.getNumberAccess().getGroup_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__Number__Group_1_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Number__Group_1_1__0__Impl
	rule__Number__Group_1_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Group_1_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getNumberAccess().getFullStopKeyword_1_1_0()); }
	'.'
	{ after(grammarAccess.getNumberAccess().getFullStopKeyword_1_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Group_1_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__Number__Group_1_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__Number__Group_1_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getNumberAccess().getAlternatives_1_1_1()); }
	(rule__Number__Alternatives_1_1_1)
	{ after(grammarAccess.getNumberAccess().getAlternatives_1_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__ArrayBrackets__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__ArrayBrackets__Group__0__Impl
	rule__ArrayBrackets__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__ArrayBrackets__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getArrayBracketsAccess().getLeftSquareBracketKeyword_0()); }
	'['
	{ after(grammarAccess.getArrayBracketsAccess().getLeftSquareBracketKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__ArrayBrackets__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__ArrayBrackets__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__ArrayBrackets__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getArrayBracketsAccess().getRightSquareBracketKeyword_1()); }
	']'
	{ after(grammarAccess.getArrayBracketsAccess().getRightSquareBracketKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmParameterizedTypeReference__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group__0__Impl
	rule__JvmParameterizedTypeReference__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeAssignment_0()); }
	(rule__JvmParameterizedTypeReference__TypeAssignment_0)
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeAssignment_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1()); }
	(rule__JvmParameterizedTypeReference__Group_1__0)?
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmParameterizedTypeReference__Group_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1__0__Impl
	rule__JvmParameterizedTypeReference__Group_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getLessThanSignKeyword_1_0()); }
	('<')
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getLessThanSignKeyword_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1__1__Impl
	rule__JvmParameterizedTypeReference__Group_1__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_1()); }
	(rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_1)
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1__2__Impl
	rule__JvmParameterizedTypeReference__Group_1__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_2()); }
	(rule__JvmParameterizedTypeReference__Group_1_2__0)*
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1__3__Impl
	rule__JvmParameterizedTypeReference__Group_1__4
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGreaterThanSignKeyword_1_3()); }
	'>'
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGreaterThanSignKeyword_1_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__4
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1__4__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1__4__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4()); }
	(rule__JvmParameterizedTypeReference__Group_1_4__0)*
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmParameterizedTypeReference__Group_1_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_2__0__Impl
	rule__JvmParameterizedTypeReference__Group_1_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getCommaKeyword_1_2_0()); }
	','
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getCommaKeyword_1_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_2_1()); }
	(rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_2_1)
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmParameterizedTypeReference__Group_1_4__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4__0__Impl
	rule__JvmParameterizedTypeReference__Group_1_4__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4_0()); }
	(rule__JvmParameterizedTypeReference__Group_1_4_0__0)
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4__1__Impl
	rule__JvmParameterizedTypeReference__Group_1_4__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeAssignment_1_4_1()); }
	(rule__JvmParameterizedTypeReference__TypeAssignment_1_4_1)
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeAssignment_1_4_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4_2()); }
	(rule__JvmParameterizedTypeReference__Group_1_4_2__0)?
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmParameterizedTypeReference__Group_1_4_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_0__0__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4_0_0()); }
	(rule__JvmParameterizedTypeReference__Group_1_4_0_0__0)
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmParameterizedTypeReference__Group_1_4_0_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_0_0__0__Impl
	rule__JvmParameterizedTypeReference__Group_1_4_0_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_0_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getJvmInnerTypeReferenceOuterAction_1_4_0_0_0()); }
	()
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getJvmInnerTypeReferenceOuterAction_1_4_0_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_0_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_0_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_0_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getFullStopKeyword_1_4_0_0_1()); }
	'.'
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getFullStopKeyword_1_4_0_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmParameterizedTypeReference__Group_1_4_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_2__0__Impl
	rule__JvmParameterizedTypeReference__Group_1_4_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getLessThanSignKeyword_1_4_2_0()); }
	('<')
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getLessThanSignKeyword_1_4_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_2__1__Impl
	rule__JvmParameterizedTypeReference__Group_1_4_2__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_4_2_1()); }
	(rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_4_2_1)
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_4_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_2__2__Impl
	rule__JvmParameterizedTypeReference__Group_1_4_2__3
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4_2_2()); }
	(rule__JvmParameterizedTypeReference__Group_1_4_2_2__0)*
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGroup_1_4_2_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_2__3__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2__3__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGreaterThanSignKeyword_1_4_2_3()); }
	'>'
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getGreaterThanSignKeyword_1_4_2_3()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmParameterizedTypeReference__Group_1_4_2_2__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_2_2__0__Impl
	rule__JvmParameterizedTypeReference__Group_1_4_2_2__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2_2__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getCommaKeyword_1_4_2_2_0()); }
	','
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getCommaKeyword_1_4_2_2_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2_2__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmParameterizedTypeReference__Group_1_4_2_2__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__Group_1_4_2_2__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_4_2_2_1()); }
	(rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_4_2_2_1)
	{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsAssignment_1_4_2_2_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmWildcardTypeReference__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmWildcardTypeReference__Group__0__Impl
	rule__JvmWildcardTypeReference__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getJvmWildcardTypeReferenceAction_0()); }
	()
	{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getJvmWildcardTypeReferenceAction_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmWildcardTypeReference__Group__1__Impl
	rule__JvmWildcardTypeReference__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getQuestionMarkKeyword_1()); }
	'?'
	{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getQuestionMarkKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmWildcardTypeReference__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getAlternatives_2()); }
	(rule__JvmWildcardTypeReference__Alternatives_2)?
	{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getAlternatives_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmWildcardTypeReference__Group_2_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmWildcardTypeReference__Group_2_0__0__Impl
	rule__JvmWildcardTypeReference__Group_2_0__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group_2_0__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_0_0()); }
	(rule__JvmWildcardTypeReference__ConstraintsAssignment_2_0_0)
	{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_0_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group_2_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmWildcardTypeReference__Group_2_0__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group_2_0__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_0_1()); }
	(rule__JvmWildcardTypeReference__ConstraintsAssignment_2_0_1)*
	{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_0_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmWildcardTypeReference__Group_2_1__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmWildcardTypeReference__Group_2_1__0__Impl
	rule__JvmWildcardTypeReference__Group_2_1__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group_2_1__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_1_0()); }
	(rule__JvmWildcardTypeReference__ConstraintsAssignment_2_1_0)
	{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_1_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group_2_1__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmWildcardTypeReference__Group_2_1__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__Group_2_1__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_1_1()); }
	(rule__JvmWildcardTypeReference__ConstraintsAssignment_2_1_1)*
	{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsAssignment_2_1_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmUpperBound__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmUpperBound__Group__0__Impl
	rule__JvmUpperBound__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmUpperBound__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmUpperBoundAccess().getExtendsKeyword_0()); }
	'extends'
	{ after(grammarAccess.getJvmUpperBoundAccess().getExtendsKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmUpperBound__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmUpperBound__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmUpperBound__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmUpperBoundAccess().getTypeReferenceAssignment_1()); }
	(rule__JvmUpperBound__TypeReferenceAssignment_1)
	{ after(grammarAccess.getJvmUpperBoundAccess().getTypeReferenceAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmUpperBoundAnded__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmUpperBoundAnded__Group__0__Impl
	rule__JvmUpperBoundAnded__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmUpperBoundAnded__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmUpperBoundAndedAccess().getAmpersandKeyword_0()); }
	'&'
	{ after(grammarAccess.getJvmUpperBoundAndedAccess().getAmpersandKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmUpperBoundAnded__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmUpperBoundAnded__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmUpperBoundAnded__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmUpperBoundAndedAccess().getTypeReferenceAssignment_1()); }
	(rule__JvmUpperBoundAnded__TypeReferenceAssignment_1)
	{ after(grammarAccess.getJvmUpperBoundAndedAccess().getTypeReferenceAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmLowerBound__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmLowerBound__Group__0__Impl
	rule__JvmLowerBound__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmLowerBound__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmLowerBoundAccess().getSuperKeyword_0()); }
	'super'
	{ after(grammarAccess.getJvmLowerBoundAccess().getSuperKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmLowerBound__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmLowerBound__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmLowerBound__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmLowerBoundAccess().getTypeReferenceAssignment_1()); }
	(rule__JvmLowerBound__TypeReferenceAssignment_1)
	{ after(grammarAccess.getJvmLowerBoundAccess().getTypeReferenceAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__JvmLowerBoundAnded__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmLowerBoundAnded__Group__0__Impl
	rule__JvmLowerBoundAnded__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmLowerBoundAnded__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmLowerBoundAndedAccess().getAmpersandKeyword_0()); }
	'&'
	{ after(grammarAccess.getJvmLowerBoundAndedAccess().getAmpersandKeyword_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmLowerBoundAnded__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__JvmLowerBoundAnded__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmLowerBoundAnded__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getJvmLowerBoundAndedAccess().getTypeReferenceAssignment_1()); }
	(rule__JvmLowerBoundAnded__TypeReferenceAssignment_1)
	{ after(grammarAccess.getJvmLowerBoundAndedAccess().getTypeReferenceAssignment_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__QualifiedNameWithWildcard__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedNameWithWildcard__Group__0__Impl
	rule__QualifiedNameWithWildcard__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedNameWithWildcard__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameWithWildcardAccess().getQualifiedNameParserRuleCall_0()); }
	ruleQualifiedName
	{ after(grammarAccess.getQualifiedNameWithWildcardAccess().getQualifiedNameParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedNameWithWildcard__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedNameWithWildcard__Group__1__Impl
	rule__QualifiedNameWithWildcard__Group__2
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedNameWithWildcard__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameWithWildcardAccess().getFullStopKeyword_1()); }
	'.'
	{ after(grammarAccess.getQualifiedNameWithWildcardAccess().getFullStopKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedNameWithWildcard__Group__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedNameWithWildcard__Group__2__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedNameWithWildcard__Group__2__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameWithWildcardAccess().getAsteriskKeyword_2()); }
	'*'
	{ after(grammarAccess.getQualifiedNameWithWildcardAccess().getAsteriskKeyword_2()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__QualifiedNameInStaticImport__Group__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedNameInStaticImport__Group__0__Impl
	rule__QualifiedNameInStaticImport__Group__1
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedNameInStaticImport__Group__0__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameInStaticImportAccess().getValidIDParserRuleCall_0()); }
	ruleValidID
	{ after(grammarAccess.getQualifiedNameInStaticImportAccess().getValidIDParserRuleCall_0()); }
)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedNameInStaticImport__Group__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__QualifiedNameInStaticImport__Group__1__Impl
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedNameInStaticImport__Group__1__Impl
	@init {
		int stackSize = keepStackSize();
	}
:
(
	{ before(grammarAccess.getQualifiedNameInStaticImportAccess().getFullStopKeyword_1()); }
	'.'
	{ after(grammarAccess.getQualifiedNameInStaticImportAccess().getFullStopKeyword_1()); }
)
;
finally {
	restoreStackSize(stackSize);
}


rule__CbCProblem__UnorderedGroup_0
	@init {
		int stackSize = keepStackSize();
		getUnorderedGroupHelper().enter(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0());
	}
:
	rule__CbCProblem__UnorderedGroup_0__0
	{getUnorderedGroupHelper().canLeave(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0())}?
;
finally {
	getUnorderedGroupHelper().leave(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0());
	restoreStackSize(stackSize);
}

rule__CbCProblem__UnorderedGroup_0__Impl
	@init {
		int stackSize = keepStackSize();
		boolean selected = false;
	}
:
		(
		( 
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0(), 0)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0(), 0);
				}
				{
					selected = true;
				}
				(
					{ before(grammarAccess.getCbCProblemAccess().getCbcformulaAssignment_0_0()); }
					(rule__CbCProblem__CbcformulaAssignment_0_0)
					{ after(grammarAccess.getCbCProblemAccess().getCbcformulaAssignment_0_0()); }
				)
			)
		)|
		( 
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0(), 1)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0(), 1);
				}
				{
					selected = true;
				}
				(
					{ before(grammarAccess.getCbCProblemAccess().getGlobalconditionAssignment_0_1()); }
					(rule__CbCProblem__GlobalconditionAssignment_0_1)
					{ after(grammarAccess.getCbCProblemAccess().getGlobalconditionAssignment_0_1()); }
				)
			)
		)|
		( 
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0(), 2)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0(), 2);
				}
				{
					selected = true;
				}
				(
					{ before(grammarAccess.getCbCProblemAccess().getJavaVariableAssignment_0_2()); }
					(rule__CbCProblem__JavaVariableAssignment_0_2)
					{ after(grammarAccess.getCbCProblemAccess().getJavaVariableAssignment_0_2()); }
				)
			)
		)|
		( 
			{getUnorderedGroupHelper().canSelect(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0(), 3)}?=>(
				{
					getUnorderedGroupHelper().select(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0(), 3);
				}
				{
					selected = true;
				}
				(
					{ before(grammarAccess.getCbCProblemAccess().getRenamingAssignment_0_3()); }
					(rule__CbCProblem__RenamingAssignment_0_3)
					{ after(grammarAccess.getCbCProblemAccess().getRenamingAssignment_0_3()); }
				)
			)
		)
		)
;
finally {
	if (selected)
		getUnorderedGroupHelper().returnFromSelection(grammarAccess.getCbCProblemAccess().getUnorderedGroup_0());
	restoreStackSize(stackSize);
}

rule__CbCProblem__UnorderedGroup_0__0
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCProblem__UnorderedGroup_0__Impl
	rule__CbCProblem__UnorderedGroup_0__1?
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCProblem__UnorderedGroup_0__1
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCProblem__UnorderedGroup_0__Impl
	rule__CbCProblem__UnorderedGroup_0__2?
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCProblem__UnorderedGroup_0__2
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCProblem__UnorderedGroup_0__Impl
	rule__CbCProblem__UnorderedGroup_0__3?
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCProblem__UnorderedGroup_0__3
	@init {
		int stackSize = keepStackSize();
	}
:
	rule__CbCProblem__UnorderedGroup_0__Impl
;
finally {
	restoreStackSize(stackSize);
}


rule__CbCProblem__CbcformulaAssignment_0_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCProblemAccess().getCbcformulaCbCFormulaParserRuleCall_0_0_0()); }
		ruleCbCFormula
		{ after(grammarAccess.getCbCProblemAccess().getCbcformulaCbCFormulaParserRuleCall_0_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCProblem__GlobalconditionAssignment_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCProblemAccess().getGlobalconditionGlobalConditionsParserRuleCall_0_1_0()); }
		ruleGlobalConditions
		{ after(grammarAccess.getCbCProblemAccess().getGlobalconditionGlobalConditionsParserRuleCall_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCProblem__JavaVariableAssignment_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCProblemAccess().getJavaVariableJavaVariablesParserRuleCall_0_2_0()); }
		ruleJavaVariables
		{ after(grammarAccess.getCbCProblemAccess().getJavaVariableJavaVariablesParserRuleCall_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCProblem__RenamingAssignment_0_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCProblemAccess().getRenamingRenamingParserRuleCall_0_3_0()); }
		ruleRenaming
		{ after(grammarAccess.getCbCProblemAccess().getRenamingRenamingParserRuleCall_0_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__NameAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCFormulaAccess().getNameEStringParserRuleCall_1_0()); }
		ruleEString
		{ after(grammarAccess.getCbCFormulaAccess().getNameEStringParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__PreConditionAssignment_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCFormulaAccess().getPreConditionConditionParserRuleCall_4_0()); }
		ruleCondition
		{ after(grammarAccess.getCbCFormulaAccess().getPreConditionConditionParserRuleCall_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__StatementAssignment_7
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCFormulaAccess().getStatementAbstractStatementParserRuleCall_7_0()); }
		ruleAbstractStatement
		{ after(grammarAccess.getCbCFormulaAccess().getStatementAbstractStatementParserRuleCall_7_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CbCFormula__PostConditionAssignment_11
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCbCFormulaAccess().getPostConditionConditionParserRuleCall_11_0()); }
		ruleCondition
		{ after(grammarAccess.getCbCFormulaAccess().getPostConditionConditionParserRuleCall_11_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__AbstractStatement_Impl__NameAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getAbstractStatement_ImplAccess().getNameCodeStringParserRuleCall_1_0()); }
		ruleCodeString
		{ after(grammarAccess.getAbstractStatement_ImplAccess().getNameCodeStringParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__MethodStatement__NameAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getMethodStatementAccess().getNameEStringParserRuleCall_1_0()); }
		ruleEString
		{ after(grammarAccess.getMethodStatementAccess().getNameEStringParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__ReturnStatement__NameAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getReturnStatementAccess().getNameCodeStringParserRuleCall_2_0()); }
		ruleCodeString
		{ after(grammarAccess.getReturnStatementAccess().getNameCodeStringParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__WeakPreConditionAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getStrengthWeakStatementAccess().getWeakPreConditionConditionParserRuleCall_3_0()); }
		ruleCondition
		{ after(grammarAccess.getStrengthWeakStatementAccess().getWeakPreConditionConditionParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__NameAssignment_6
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getStrengthWeakStatementAccess().getNameCodeStringParserRuleCall_6_0()); }
		ruleCodeString
		{ after(grammarAccess.getStrengthWeakStatementAccess().getNameCodeStringParserRuleCall_6_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__StrengthWeakStatement__StrongPostConditionAssignment_10
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getStrengthWeakStatementAccess().getStrongPostConditionConditionParserRuleCall_10_0()); }
		ruleCondition
		{ after(grammarAccess.getStrengthWeakStatementAccess().getStrongPostConditionConditionParserRuleCall_10_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Condition__ConditionAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getConditionAccess().getConditionExpressionParserRuleCall_1_0()); }
		ruleExpression
		{ after(grammarAccess.getConditionAccess().getConditionExpressionParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SkipStatement__NameAssignment
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSkipStatementAccess().getNameSemicolonKeyword_0()); }
		(
			{ before(grammarAccess.getSkipStatementAccess().getNameSemicolonKeyword_0()); }
			';'
			{ after(grammarAccess.getSkipStatementAccess().getNameSemicolonKeyword_0()); }
		)
		{ after(grammarAccess.getSkipStatementAccess().getNameSemicolonKeyword_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__FirstStatementAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCompositionStatementAccess().getFirstStatementAbstractStatementParserRuleCall_2_0()); }
		ruleAbstractStatement
		{ after(grammarAccess.getCompositionStatementAccess().getFirstStatementAbstractStatementParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__IntermediateConditionAssignment_6
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCompositionStatementAccess().getIntermediateConditionConditionParserRuleCall_6_0()); }
		ruleCondition
		{ after(grammarAccess.getCompositionStatementAccess().getIntermediateConditionConditionParserRuleCall_6_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__CompositionStatement__SecondStatementAssignment_9
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getCompositionStatementAccess().getSecondStatementAbstractStatementParserRuleCall_9_0()); }
		ruleAbstractStatement
		{ after(grammarAccess.getCompositionStatementAccess().getSecondStatementAbstractStatementParserRuleCall_9_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaStatement__StatementAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJavaStatementAccess().getStatementXJStatementOrBlockParserRuleCall_1_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getJavaStatementAccess().getStatementXJStatementOrBlockParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLWhileExpression__AnnotationAssignment_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLWhileExpressionAccess().getAnnotationLoopInvariantAnnotationStatementParserRuleCall_1_0_0()); }
		ruleLoopInvariantAnnotationStatement
		{ after(grammarAccess.getJMLWhileExpressionAccess().getAnnotationLoopInvariantAnnotationStatementParserRuleCall_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLWhileExpression__WhileAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLWhileExpressionAccess().getWhileXWhileExpressionParserRuleCall_1_1_0()); }
		ruleXWhileExpression
		{ after(grammarAccess.getJMLWhileExpressionAccess().getWhileXWhileExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__RequiresAssignment_1_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLAnnotationAccess().getRequiresJMLExpressionParserRuleCall_1_0_2_0()); }
		ruleJMLExpression
		{ after(grammarAccess.getJMLAnnotationAccess().getRequiresJMLExpressionParserRuleCall_1_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAnnotation__EnsuresAssignment_1_1_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLAnnotationAccess().getEnsuresJMLExpressionParserRuleCall_1_1_2_0()); }
		ruleJMLExpression
		{ after(grammarAccess.getJMLAnnotationAccess().getEnsuresJMLExpressionParserRuleCall_1_1_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__LoopInvariantAssignment_1_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getLoopInvariantJMLExpressionParserRuleCall_1_0_2_0()); }
		ruleJMLExpression
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getLoopInvariantJMLExpressionParserRuleCall_1_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__AssignableAssignment_1_1_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableVariableOrMethodNameParserRuleCall_1_1_2_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableVariableOrMethodNameParserRuleCall_1_1_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__AssignableAssignment_1_1_3_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableVariableOrMethodNameParserRuleCall_1_1_3_1_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getAssignableVariableOrMethodNameParserRuleCall_1_1_3_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotation__DecreasesAssignment_1_2_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getLoopInvariantAnnotationAccess().getDecreasesJMLExpressionParserRuleCall_1_2_2_0()); }
		ruleJMLExpression
		{ after(grammarAccess.getLoopInvariantAnnotationAccess().getDecreasesJMLExpressionParserRuleCall_1_2_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__LoopInvariantAnnotationStatement__JmlAnnotationAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getLoopInvariantAnnotationStatementAccess().getJmlAnnotationLoopInvariantAnnotationParserRuleCall_1_0()); }
		ruleLoopInvariantAnnotation
		{ after(grammarAccess.getLoopInvariantAnnotationStatementAccess().getJmlAnnotationLoopInvariantAnnotationParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineBlockStatement__ReferencesAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getInlineBlockStatementAccess().getReferencesBlockStatementCrossReference_2_0()); }
		(
			{ before(grammarAccess.getInlineBlockStatementAccess().getReferencesBlockStatementIDTerminalRuleCall_2_0_1()); }
			RULE_ID
			{ after(grammarAccess.getInlineBlockStatementAccess().getReferencesBlockStatementIDTerminalRuleCall_2_0_1()); }
		)
		{ after(grammarAccess.getInlineBlockStatementAccess().getReferencesBlockStatementCrossReference_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__NameAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getBlockStatementAccess().getNameEStringParserRuleCall_2_0()); }
		ruleEString
		{ after(grammarAccess.getBlockStatementAccess().getNameEStringParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__JmlAnnotationAssignment_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getBlockStatementAccess().getJmlAnnotationJMLAnnotationParserRuleCall_4_0()); }
		ruleJMLAnnotation
		{ after(grammarAccess.getBlockStatementAccess().getJmlAnnotationJMLAnnotationParserRuleCall_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__JavaStatementAssignment_5_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getBlockStatementAccess().getJavaStatementJavaStatementParserRuleCall_5_1_0_0()); }
		ruleJavaStatement
		{ after(grammarAccess.getBlockStatementAccess().getJavaStatementJavaStatementParserRuleCall_5_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__BlockStatement__CbcStatementAssignment_5_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getBlockStatementAccess().getCbcStatementAbstractStatementParserRuleCall_5_1_1_1_0()); }
		ruleAbstractStatement
		{ after(grammarAccess.getBlockStatementAccess().getCbcStatementAbstractStatementParserRuleCall_5_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__JmlAnnotationAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getInlineJavaBlockStatementAccess().getJmlAnnotationJMLAnnotationParserRuleCall_1_0()); }
		ruleJMLAnnotation
		{ after(grammarAccess.getInlineJavaBlockStatementAccess().getJmlAnnotationJMLAnnotationParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__InlineJavaBlockStatement__ReferencesAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getInlineJavaBlockStatementAccess().getReferencesBlockStatementCrossReference_3_0()); }
		(
			{ before(grammarAccess.getInlineJavaBlockStatementAccess().getReferencesBlockStatementIDTerminalRuleCall_3_0_1()); }
			RULE_ID
			{ after(grammarAccess.getInlineJavaBlockStatementAccess().getReferencesBlockStatementIDTerminalRuleCall_3_0_1()); }
		)
		{ after(grammarAccess.getInlineJavaBlockStatementAccess().getReferencesBlockStatementCrossReference_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__GuardsAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSelectionStatementAccess().getGuardsConditionParserRuleCall_3_0()); }
		ruleCondition
		{ after(grammarAccess.getSelectionStatementAccess().getGuardsConditionParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__CommandsAssignment_7
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSelectionStatementAccess().getCommandsAbstractStatementParserRuleCall_7_0()); }
		ruleAbstractStatement
		{ after(grammarAccess.getSelectionStatementAccess().getCommandsAbstractStatementParserRuleCall_7_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__GuardsAssignment_9_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSelectionStatementAccess().getGuardsConditionParserRuleCall_9_2_0()); }
		ruleCondition
		{ after(grammarAccess.getSelectionStatementAccess().getGuardsConditionParserRuleCall_9_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SelectionStatement__CommandsAssignment_9_6
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSelectionStatementAccess().getCommandsAbstractStatementParserRuleCall_9_6_0()); }
		ruleAbstractStatement
		{ after(grammarAccess.getSelectionStatementAccess().getCommandsAbstractStatementParserRuleCall_9_6_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__GuardAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSmallRepetitionStatementAccess().getGuardConditionParserRuleCall_3_0()); }
		ruleCondition
		{ after(grammarAccess.getSmallRepetitionStatementAccess().getGuardConditionParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__InvariantAssignment_8
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSmallRepetitionStatementAccess().getInvariantConditionParserRuleCall_8_0()); }
		ruleCondition
		{ after(grammarAccess.getSmallRepetitionStatementAccess().getInvariantConditionParserRuleCall_8_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__VariantAssignment_12
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSmallRepetitionStatementAccess().getVariantVariantParserRuleCall_12_0()); }
		ruleVariant
		{ after(grammarAccess.getSmallRepetitionStatementAccess().getVariantVariantParserRuleCall_12_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__SmallRepetitionStatement__LoopStatementAssignment_15
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getSmallRepetitionStatementAccess().getLoopStatementAbstractStatementParserRuleCall_15_0()); }
		ruleAbstractStatement
		{ after(grammarAccess.getSmallRepetitionStatementAccess().getLoopStatementAbstractStatementParserRuleCall_15_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Variant__VarAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariantAccess().getVarMultiMathOperationParserRuleCall_1_0()); }
		ruleMultiMathOperation
		{ after(grammarAccess.getVariantAccess().getVarMultiMathOperationParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__VariablesAssignment_2_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJavaVariablesAccess().getVariablesJavaVariableParserRuleCall_2_2_0()); }
		ruleJavaVariable
		{ after(grammarAccess.getJavaVariablesAccess().getVariablesJavaVariableParserRuleCall_2_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariables__VariablesAssignment_2_3_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJavaVariablesAccess().getVariablesJavaVariableParserRuleCall_2_3_1_0()); }
		ruleJavaVariable
		{ after(grammarAccess.getJavaVariablesAccess().getVariablesJavaVariableParserRuleCall_2_3_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariable__TypeAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJavaVariableAccess().getTypeJvmTypeReferenceParserRuleCall_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJavaVariableAccess().getTypeJvmTypeReferenceParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JavaVariable__VarAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJavaVariableAccess().getVarVariableOrMethodNameParserRuleCall_2_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getJavaVariableAccess().getVarVariableOrMethodNameParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__ConditionsAssignment_2_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getGlobalConditionsAccess().getConditionsConditionParserRuleCall_2_2_0()); }
		ruleCondition
		{ after(grammarAccess.getGlobalConditionsAccess().getConditionsConditionParserRuleCall_2_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__GlobalConditions__ConditionsAssignment_2_3_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getGlobalConditionsAccess().getConditionsConditionParserRuleCall_2_3_1_0()); }
		ruleCondition
		{ after(grammarAccess.getGlobalConditionsAccess().getConditionsConditionParserRuleCall_2_3_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__RenameAssignment_2_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getRenamingAccess().getRenameRenameParserRuleCall_2_2_0()); }
		ruleRename
		{ after(grammarAccess.getRenamingAccess().getRenameRenameParserRuleCall_2_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Renaming__RenameAssignment_2_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getRenamingAccess().getRenameRenameParserRuleCall_2_3_0()); }
		ruleRename
		{ after(grammarAccess.getRenamingAccess().getRenameRenameParserRuleCall_2_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__TypeAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getRenameAccess().getTypeEStringParserRuleCall_3_0()); }
		ruleEString
		{ after(grammarAccess.getRenameAccess().getTypeEStringParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__FunctionAssignment_5
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getRenameAccess().getFunctionEStringParserRuleCall_5_0()); }
		ruleEString
		{ after(grammarAccess.getRenameAccess().getFunctionEStringParserRuleCall_5_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Rename__NewNameAssignment_7
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getRenameAccess().getNewNameEStringParserRuleCall_7_0()); }
		ruleEString
		{ after(grammarAccess.getRenameAccess().getNewNameEStringParserRuleCall_7_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__NameAssignment_0_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableOrMethodNameAccess().getNameIDTerminalRuleCall_0_0_0()); }
		RULE_ID
		{ after(grammarAccess.getVariableOrMethodNameAccess().getNameIDTerminalRuleCall_0_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__VariableOrMethodName__NameAssignment_1_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getVariableOrMethodNameAccess().getNameIDTerminalRuleCall_1_1_0_0()); }
		RULE_ID
		{ after(grammarAccess.getVariableOrMethodNameAccess().getNameIDTerminalRuleCall_1_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Implication__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getImplicationAccess().getRightConcatParserRuleCall_1_1_0()); }
		ruleConcat
		{ after(grammarAccess.getImplicationAccess().getRightConcatParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Concat__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getConcatAccess().getRightForeachParserRuleCall_1_1_0()); }
		ruleForeach
		{ after(grammarAccess.getConcatAccess().getRightForeachParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__ForeachAssignment_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getForeachAccess().getForeachForallKeyword_1_0_0()); }
		(
			{ before(grammarAccess.getForeachAccess().getForeachForallKeyword_1_0_0()); }
			'(\\forall'
			{ after(grammarAccess.getForeachAccess().getForeachForallKeyword_1_0_0()); }
		)
		{ after(grammarAccess.getForeachAccess().getForeachForallKeyword_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__TypeAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getForeachAccess().getTypeJvmTypeReferenceParserRuleCall_1_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getForeachAccess().getTypeJvmTypeReferenceParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__VarAssignment_1_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getForeachAccess().getVarVariableOrMethodNameParserRuleCall_1_2_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getForeachAccess().getVarVariableOrMethodNameParserRuleCall_1_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Foreach__RightAssignment_1_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getForeachAccess().getRightExistsParserRuleCall_1_4_0()); }
		ruleExists
		{ after(grammarAccess.getForeachAccess().getRightExistsParserRuleCall_1_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__ExistsAssignment_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getExistsAccess().getExistsExistsKeyword_1_0_0()); }
		(
			{ before(grammarAccess.getExistsAccess().getExistsExistsKeyword_1_0_0()); }
			'(\\exists'
			{ after(grammarAccess.getExistsAccess().getExistsExistsKeyword_1_0_0()); }
		)
		{ after(grammarAccess.getExistsAccess().getExistsExistsKeyword_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__TypeAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getExistsAccess().getTypeJvmTypeReferenceParserRuleCall_1_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getExistsAccess().getTypeJvmTypeReferenceParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__VarAssignment_1_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getExistsAccess().getVarVariableOrMethodNameParserRuleCall_1_2_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getExistsAccess().getVarVariableOrMethodNameParserRuleCall_1_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Exists__RightAssignment_1_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getExistsAccess().getRightRelationParserRuleCall_1_4_0()); }
		ruleRelation
		{ after(grammarAccess.getExistsAccess().getRightRelationParserRuleCall_1_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__Relation__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getRelationAccess().getRightMultiMathOperationParserRuleCall_1_1_0()); }
		ruleMultiMathOperation
		{ after(grammarAccess.getRelationAccess().getRightMultiMathOperationParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__MultiMathOperation__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getMultiMathOperationAccess().getRightAddMathOperationParserRuleCall_1_1_0()); }
		ruleAddMathOperation
		{ after(grammarAccess.getMultiMathOperationAccess().getRightAddMathOperationParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__AddMathOperation__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getAddMathOperationAccess().getRightQualifiedExpressionParserRuleCall_1_1_0()); }
		ruleQualifiedExpression
		{ after(grammarAccess.getAddMathOperationAccess().getRightQualifiedExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__QualifiedExpression__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getQualifiedExpressionAccess().getRightPrimaryExpressionParserRuleCall_1_1_0()); }
		rulePrimaryExpression
		{ after(grammarAccess.getQualifiedExpressionAccess().getRightPrimaryExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__IsTrueAssignment_0_1_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getIsTrueTrueKeyword_0_1_0_1_0()); }
		(
			{ before(grammarAccess.getPrimaryExpressionAccess().getIsTrueTrueKeyword_0_1_0_1_0()); }
			'true'
			{ after(grammarAccess.getPrimaryExpressionAccess().getIsTrueTrueKeyword_0_1_0_1_0()); }
		)
		{ after(grammarAccess.getPrimaryExpressionAccess().getIsTrueTrueKeyword_0_1_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__FuncAssignment_2_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getFuncVariableOrMethodNameParserRuleCall_2_2_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getPrimaryExpressionAccess().getFuncVariableOrMethodNameParserRuleCall_2_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__ArgsAssignment_2_3_0_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getArgsExpressionParserRuleCall_2_3_0_1_0_0()); }
		ruleExpression
		{ after(grammarAccess.getPrimaryExpressionAccess().getArgsExpressionParserRuleCall_2_3_0_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__ArgsAssignment_2_3_0_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getArgsExpressionParserRuleCall_2_3_0_1_1_1_0()); }
		ruleExpression
		{ after(grammarAccess.getPrimaryExpressionAccess().getArgsExpressionParserRuleCall_2_3_0_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__PrimaryExpression__ArgsAssignment_2_3_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getPrimaryExpressionAccess().getArgsExpressionParserRuleCall_2_3_1_1_0()); }
		ruleExpression
		{ after(grammarAccess.getPrimaryExpressionAccess().getArgsExpressionParserRuleCall_2_3_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLImplication__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLImplicationAccess().getRightJMLConcatParserRuleCall_1_1_0()); }
		ruleJMLConcat
		{ after(grammarAccess.getJMLImplicationAccess().getRightJMLConcatParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLConcat__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLConcatAccess().getRightJMLForeachParserRuleCall_1_1_0()); }
		ruleJMLForeach
		{ after(grammarAccess.getJMLConcatAccess().getRightJMLForeachParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__ForeachAssignment_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLForeachAccess().getForeachForallKeyword_1_0_0()); }
		(
			{ before(grammarAccess.getJMLForeachAccess().getForeachForallKeyword_1_0_0()); }
			'(\\forall'
			{ after(grammarAccess.getJMLForeachAccess().getForeachForallKeyword_1_0_0()); }
		)
		{ after(grammarAccess.getJMLForeachAccess().getForeachForallKeyword_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__TypeAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLForeachAccess().getTypeJvmTypeReferenceParserRuleCall_1_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJMLForeachAccess().getTypeJvmTypeReferenceParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__VarAssignment_1_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLForeachAccess().getVarVariableOrMethodNameParserRuleCall_1_2_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getJMLForeachAccess().getVarVariableOrMethodNameParserRuleCall_1_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLForeach__RightAssignment_1_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLForeachAccess().getRightJMLExistsParserRuleCall_1_4_0()); }
		ruleJMLExists
		{ after(grammarAccess.getJMLForeachAccess().getRightJMLExistsParserRuleCall_1_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__ExistsAssignment_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLExistsAccess().getExistsExistsKeyword_1_0_0()); }
		(
			{ before(grammarAccess.getJMLExistsAccess().getExistsExistsKeyword_1_0_0()); }
			'(\\exists'
			{ after(grammarAccess.getJMLExistsAccess().getExistsExistsKeyword_1_0_0()); }
		)
		{ after(grammarAccess.getJMLExistsAccess().getExistsExistsKeyword_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__TypeAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLExistsAccess().getTypeJvmTypeReferenceParserRuleCall_1_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJMLExistsAccess().getTypeJvmTypeReferenceParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__VarAssignment_1_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLExistsAccess().getVarVariableOrMethodNameParserRuleCall_1_2_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getJMLExistsAccess().getVarVariableOrMethodNameParserRuleCall_1_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLExists__RightAssignment_1_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLExistsAccess().getRightJMLRelationParserRuleCall_1_4_0()); }
		ruleJMLRelation
		{ after(grammarAccess.getJMLExistsAccess().getRightJMLRelationParserRuleCall_1_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLRelation__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLRelationAccess().getRightJMLMultiMathOperationParserRuleCall_1_1_0()); }
		ruleJMLMultiMathOperation
		{ after(grammarAccess.getJMLRelationAccess().getRightJMLMultiMathOperationParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLMultiMathOperation__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLMultiMathOperationAccess().getRightJMLAddMathOperationParserRuleCall_1_1_0()); }
		ruleJMLAddMathOperation
		{ after(grammarAccess.getJMLMultiMathOperationAccess().getRightJMLAddMathOperationParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLAddMathOperation__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLAddMathOperationAccess().getRightJMLQualifiedExpressionParserRuleCall_1_1_0()); }
		ruleJMLQualifiedExpression
		{ after(grammarAccess.getJMLAddMathOperationAccess().getRightJMLQualifiedExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLQualifiedExpression__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLQualifiedExpressionAccess().getRightJMLPrimaryExpressionParserRuleCall_1_1_0()); }
		ruleJMLPrimaryExpression
		{ after(grammarAccess.getJMLQualifiedExpressionAccess().getRightJMLPrimaryExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__IsTrueAssignment_0_1_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getIsTrueTrueKeyword_0_1_0_1_0()); }
		(
			{ before(grammarAccess.getJMLPrimaryExpressionAccess().getIsTrueTrueKeyword_0_1_0_1_0()); }
			'true'
			{ after(grammarAccess.getJMLPrimaryExpressionAccess().getIsTrueTrueKeyword_0_1_0_1_0()); }
		)
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getIsTrueTrueKeyword_0_1_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__FuncAssignment_2_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getFuncVariableOrMethodNameParserRuleCall_2_2_0()); }
		ruleVariableOrMethodName
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getFuncVariableOrMethodNameParserRuleCall_2_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__ArgsAssignment_2_3_0_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getArgsJMLExpressionParserRuleCall_2_3_0_1_0_0()); }
		ruleJMLExpression
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getArgsJMLExpressionParserRuleCall_2_3_0_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__ArgsAssignment_2_3_0_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getArgsJMLExpressionParserRuleCall_2_3_0_1_1_1_0()); }
		ruleJMLExpression
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getArgsJMLExpressionParserRuleCall_2_3_0_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JMLPrimaryExpression__ArgsAssignment_2_3_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJMLPrimaryExpressionAccess().getArgsJMLExpressionParserRuleCall_2_3_1_1_0()); }
		ruleJMLExpression
		{ after(grammarAccess.getJMLPrimaryExpressionAccess().getArgsJMLExpressionParserRuleCall_2_3_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__StaticAssignment_1_0_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getStaticStaticKeyword_1_0_0_0()); }
		(
			{ before(grammarAccess.getXImportDeclarationAccess().getStaticStaticKeyword_1_0_0_0()); }
			'static'
			{ after(grammarAccess.getXImportDeclarationAccess().getStaticStaticKeyword_1_0_0_0()); }
		)
		{ after(grammarAccess.getXImportDeclarationAccess().getStaticStaticKeyword_1_0_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__ImportedTypeAssignment_1_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getImportedTypeJvmDeclaredTypeCrossReference_1_0_1_0()); }
		(
			{ before(grammarAccess.getXImportDeclarationAccess().getImportedTypeJvmDeclaredTypeQualifiedNameInStaticImportParserRuleCall_1_0_1_0_1()); }
			ruleQualifiedNameInStaticImport
			{ after(grammarAccess.getXImportDeclarationAccess().getImportedTypeJvmDeclaredTypeQualifiedNameInStaticImportParserRuleCall_1_0_1_0_1()); }
		)
		{ after(grammarAccess.getXImportDeclarationAccess().getImportedTypeJvmDeclaredTypeCrossReference_1_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__WildcardAssignment_1_0_2_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getWildcardAsteriskKeyword_1_0_2_0_0()); }
		(
			{ before(grammarAccess.getXImportDeclarationAccess().getWildcardAsteriskKeyword_1_0_2_0_0()); }
			'*'
			{ after(grammarAccess.getXImportDeclarationAccess().getWildcardAsteriskKeyword_1_0_2_0_0()); }
		)
		{ after(grammarAccess.getXImportDeclarationAccess().getWildcardAsteriskKeyword_1_0_2_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__MemberNameAssignment_1_0_2_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getMemberNameValidIDParserRuleCall_1_0_2_1_0()); }
		ruleValidID
		{ after(grammarAccess.getXImportDeclarationAccess().getMemberNameValidIDParserRuleCall_1_0_2_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__ImportedTypeAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getImportedTypeJvmDeclaredTypeCrossReference_1_1_0()); }
		(
			{ before(grammarAccess.getXImportDeclarationAccess().getImportedTypeJvmDeclaredTypeQualifiedNameParserRuleCall_1_1_0_1()); }
			ruleQualifiedName
			{ after(grammarAccess.getXImportDeclarationAccess().getImportedTypeJvmDeclaredTypeQualifiedNameParserRuleCall_1_1_0_1()); }
		)
		{ after(grammarAccess.getXImportDeclarationAccess().getImportedTypeJvmDeclaredTypeCrossReference_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XImportDeclaration__ImportedNamespaceAssignment_1_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXImportDeclarationAccess().getImportedNamespaceQualifiedNameWithWildcardParserRuleCall_1_2_0()); }
		ruleQualifiedNameWithWildcard
		{ after(grammarAccess.getXImportDeclarationAccess().getImportedNamespaceQualifiedNameWithWildcardParserRuleCall_1_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__FinalAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getFullJvmFormalParameterAccess().getFinalFinalKeyword_1_0()); }
		(
			{ before(grammarAccess.getFullJvmFormalParameterAccess().getFinalFinalKeyword_1_0()); }
			'final'
			{ after(grammarAccess.getFullJvmFormalParameterAccess().getFinalFinalKeyword_1_0()); }
		)
		{ after(grammarAccess.getFullJvmFormalParameterAccess().getFinalFinalKeyword_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__ParameterTypeAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getFullJvmFormalParameterAccess().getParameterTypeJvmTypeReferenceParserRuleCall_2_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getFullJvmFormalParameterAccess().getParameterTypeJvmTypeReferenceParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__VarArgsAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getFullJvmFormalParameterAccess().getVarArgsFullStopFullStopFullStopKeyword_3_0()); }
		(
			{ before(grammarAccess.getFullJvmFormalParameterAccess().getVarArgsFullStopFullStopFullStopKeyword_3_0()); }
			'...'
			{ after(grammarAccess.getFullJvmFormalParameterAccess().getVarArgsFullStopFullStopFullStopKeyword_3_0()); }
		)
		{ after(grammarAccess.getFullJvmFormalParameterAccess().getVarArgsFullStopFullStopFullStopKeyword_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__FullJvmFormalParameter__NameAssignment_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getFullJvmFormalParameterAccess().getNameValidIDParserRuleCall_4_0()); }
		ruleValidID
		{ after(grammarAccess.getFullJvmFormalParameterAccess().getNameValidIDParserRuleCall_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSemicolonStatement__ExpressionAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJSemicolonStatementAccess().getExpressionAlternatives_1_0()); }
		(rule__XJSemicolonStatement__ExpressionAlternatives_1_0)
		{ after(grammarAccess.getXJSemicolonStatementAccess().getExpressionAlternatives_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSemicolonStatement__SemicolonAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJSemicolonStatementAccess().getSemicolonSemicolonKeyword_2_0()); }
		(
			{ before(grammarAccess.getXJSemicolonStatementAccess().getSemicolonSemicolonKeyword_2_0()); }
			';'
			{ after(grammarAccess.getXJSemicolonStatementAccess().getSemicolonSemicolonKeyword_2_0()); }
		)
		{ after(grammarAccess.getXJSemicolonStatementAccess().getSemicolonSemicolonKeyword_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJEmptyStatement__SemicolonAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJEmptyStatementAccess().getSemicolonSemicolonKeyword_1_0()); }
		(
			{ before(grammarAccess.getXJEmptyStatementAccess().getSemicolonSemicolonKeyword_1_0()); }
			';'
			{ after(grammarAccess.getXJEmptyStatementAccess().getSemicolonSemicolonKeyword_1_0()); }
		)
		{ after(grammarAccess.getXJEmptyStatementAccess().getSemicolonSemicolonKeyword_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBlockExpression__ExpressionsAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBlockExpressionAccess().getExpressionsXJStatementOrBlockParserRuleCall_2_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getXBlockExpressionAccess().getExpressionsXJStatementOrBlockParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__FinalAssignment_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXVariableDeclarationAccess().getFinalFinalKeyword_0_0_1_0()); }
		(
			{ before(grammarAccess.getXVariableDeclarationAccess().getFinalFinalKeyword_0_0_1_0()); }
			'final'
			{ after(grammarAccess.getXVariableDeclarationAccess().getFinalFinalKeyword_0_0_1_0()); }
		)
		{ after(grammarAccess.getXVariableDeclarationAccess().getFinalFinalKeyword_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__TypeAssignment_0_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXVariableDeclarationAccess().getTypeJvmTypeReferenceParserRuleCall_0_0_2_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getXVariableDeclarationAccess().getTypeJvmTypeReferenceParserRuleCall_0_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__NameAssignment_0_0_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXVariableDeclarationAccess().getNameValidIDParserRuleCall_0_0_3_0()); }
		ruleValidID
		{ after(grammarAccess.getXVariableDeclarationAccess().getNameValidIDParserRuleCall_0_0_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__RightAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXVariableDeclarationAccess().getRightXExpressionParserRuleCall_1_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXVariableDeclarationAccess().getRightXExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XVariableDeclaration__AdditionalVariablesAssignment_2_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXVariableDeclarationAccess().getAdditionalVariablesXJAdditionalXVariableDeclarationParserRuleCall_2_1_0()); }
		ruleXJAdditionalXVariableDeclaration
		{ after(grammarAccess.getXVariableDeclarationAccess().getAdditionalVariablesXJAdditionalXVariableDeclarationParserRuleCall_2_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__NameAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getNameValidIDParserRuleCall_1_0()); }
		ruleValidID
		{ after(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getNameValidIDParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJAdditionalXVariableDeclaration__RightAssignment_2_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getRightXExpressionParserRuleCall_2_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXJAdditionalXVariableDeclarationAccess().getRightXExpressionParserRuleCall_2_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__FeatureAssignment_0_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementCrossReference_0_0_0_1_0()); }
		(
			{ before(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementFeatureCallIDParserRuleCall_0_0_0_1_0_1()); }
			ruleFeatureCallID
			{ after(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementFeatureCallIDParserRuleCall_0_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementCrossReference_0_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__IndexesAssignment_0_0_0_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getIndexesXExpressionParserRuleCall_0_0_0_3_0()); }
		ruleXExpression
		{ after(grammarAccess.getXAssignmentAccess().getIndexesXExpressionParserRuleCall_0_0_0_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__IndexesAssignment_0_0_0_5_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getIndexesXExpressionParserRuleCall_0_0_0_5_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXAssignmentAccess().getIndexesXExpressionParserRuleCall_0_0_0_5_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__ValueAssignment_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getValueXAssignmentParserRuleCall_0_1_0()); }
		ruleXAssignment
		{ after(grammarAccess.getXAssignmentAccess().getValueXAssignmentParserRuleCall_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementFeatureCallIDParserRuleCall_1_0_0_1_0_1()); }
			ruleFeatureCallID
			{ after(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementFeatureCallIDParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__ValueAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getValueXAssignmentParserRuleCall_1_1_0()); }
		ruleXAssignment
		{ after(grammarAccess.getXAssignmentAccess().getValueXAssignmentParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__ThenAssignment_2_1_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getThenXExpressionParserRuleCall_2_1_0_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXAssignmentAccess().getThenXExpressionParserRuleCall_2_1_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__ElseAssignment_2_1_0_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getElseXExpressionParserRuleCall_2_1_0_3_0()); }
		ruleXExpression
		{ after(grammarAccess.getXAssignmentAccess().getElseXExpressionParserRuleCall_2_1_0_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__FeatureAssignment_2_1_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementCrossReference_2_1_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementOpMultiAssignParserRuleCall_2_1_1_0_0_1_0_1()); }
			ruleOpMultiAssign
			{ after(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementOpMultiAssignParserRuleCall_2_1_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXAssignmentAccess().getFeatureJvmIdentifiableElementCrossReference_2_1_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAssignment__RightOperandAssignment_2_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAssignmentAccess().getRightOperandXAssignmentParserRuleCall_2_1_1_1_0()); }
		ruleXAssignment
		{ after(grammarAccess.getXAssignmentAccess().getRightOperandXAssignmentParserRuleCall_2_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__IndexesAssignment_1_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getIndexesXExpressionParserRuleCall_1_0_2_0()); }
		ruleXExpression
		{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getIndexesXExpressionParserRuleCall_1_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJFeatureCallWithArrayAccess__IndexesAssignment_1_0_4_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getIndexesXExpressionParserRuleCall_1_0_4_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXJFeatureCallWithArrayAccessAccess().getIndexesXExpressionParserRuleCall_1_0_4_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__FeatureAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXFeatureCallAccess().getFeatureJvmIdentifiableElementCrossReference_1_0()); }
		(
			{ before(grammarAccess.getXFeatureCallAccess().getFeatureJvmIdentifiableElementIdOrSuperParserRuleCall_1_0_1()); }
			ruleIdOrSuper
			{ after(grammarAccess.getXFeatureCallAccess().getFeatureJvmIdentifiableElementIdOrSuperParserRuleCall_1_0_1()); }
		)
		{ after(grammarAccess.getXFeatureCallAccess().getFeatureJvmIdentifiableElementCrossReference_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__ExplicitOperationCallAssignment_2_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXFeatureCallAccess().getExplicitOperationCallLeftParenthesisKeyword_2_0_0()); }
		(
			{ before(grammarAccess.getXFeatureCallAccess().getExplicitOperationCallLeftParenthesisKeyword_2_0_0()); }
			'('
			{ after(grammarAccess.getXFeatureCallAccess().getExplicitOperationCallLeftParenthesisKeyword_2_0_0()); }
		)
		{ after(grammarAccess.getXFeatureCallAccess().getExplicitOperationCallLeftParenthesisKeyword_2_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__FeatureCallArgumentsAssignment_2_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsXExpressionParserRuleCall_2_1_0_0()); }
		ruleXExpression
		{ after(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsXExpressionParserRuleCall_2_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XFeatureCall__FeatureCallArgumentsAssignment_2_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsXExpressionParserRuleCall_2_1_1_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXFeatureCallAccess().getFeatureCallArgumentsXExpressionParserRuleCall_2_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__ConstructorAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXConstructorCallAccess().getConstructorJvmConstructorCrossReference_2_0()); }
		(
			{ before(grammarAccess.getXConstructorCallAccess().getConstructorJvmConstructorQualifiedNameParserRuleCall_2_0_1()); }
			ruleQualifiedName
			{ after(grammarAccess.getXConstructorCallAccess().getConstructorJvmConstructorQualifiedNameParserRuleCall_2_0_1()); }
		)
		{ after(grammarAccess.getXConstructorCallAccess().getConstructorJvmConstructorCrossReference_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__ExplicitTypeArgumentsAssignment_3_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXConstructorCallAccess().getExplicitTypeArgumentsLessThanSignKeyword_3_0_0()); }
		(
			{ before(grammarAccess.getXConstructorCallAccess().getExplicitTypeArgumentsLessThanSignKeyword_3_0_0()); }
			'<'
			{ after(grammarAccess.getXConstructorCallAccess().getExplicitTypeArgumentsLessThanSignKeyword_3_0_0()); }
		)
		{ after(grammarAccess.getXConstructorCallAccess().getExplicitTypeArgumentsLessThanSignKeyword_3_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__TypeArgumentsAssignment_3_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXConstructorCallAccess().getTypeArgumentsJvmArgumentTypeReferenceParserRuleCall_3_1_0_0()); }
		ruleJvmArgumentTypeReference
		{ after(grammarAccess.getXConstructorCallAccess().getTypeArgumentsJvmArgumentTypeReferenceParserRuleCall_3_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__TypeArgumentsAssignment_3_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXConstructorCallAccess().getTypeArgumentsJvmArgumentTypeReferenceParserRuleCall_3_1_1_1_0()); }
		ruleJvmArgumentTypeReference
		{ after(grammarAccess.getXConstructorCallAccess().getTypeArgumentsJvmArgumentTypeReferenceParserRuleCall_3_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__ExplicitConstructorCallAssignment_4_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXConstructorCallAccess().getExplicitConstructorCallLeftParenthesisKeyword_4_0_0()); }
		(
			{ before(grammarAccess.getXConstructorCallAccess().getExplicitConstructorCallLeftParenthesisKeyword_4_0_0()); }
			'('
			{ after(grammarAccess.getXConstructorCallAccess().getExplicitConstructorCallLeftParenthesisKeyword_4_0_0()); }
		)
		{ after(grammarAccess.getXConstructorCallAccess().getExplicitConstructorCallLeftParenthesisKeyword_4_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__ArgumentsAssignment_4_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXConstructorCallAccess().getArgumentsXExpressionParserRuleCall_4_1_0_0()); }
		ruleXExpression
		{ after(grammarAccess.getXConstructorCallAccess().getArgumentsXExpressionParserRuleCall_4_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XConstructorCall__ArgumentsAssignment_4_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXConstructorCallAccess().getArgumentsXExpressionParserRuleCall_4_1_1_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXConstructorCallAccess().getArgumentsXExpressionParserRuleCall_4_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__TypeAssignment_0_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJArrayConstructorCallAccess().getTypeJvmTypeCrossReference_0_0_2_0()); }
		(
			{ before(grammarAccess.getXJArrayConstructorCallAccess().getTypeJvmTypeQualifiedNameParserRuleCall_0_0_2_0_1()); }
			ruleQualifiedName
			{ after(grammarAccess.getXJArrayConstructorCallAccess().getTypeJvmTypeQualifiedNameParserRuleCall_0_0_2_0_1()); }
		)
		{ after(grammarAccess.getXJArrayConstructorCallAccess().getTypeJvmTypeCrossReference_0_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__DimensionsAssignment_0_0_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJArrayConstructorCallAccess().getDimensionsXJArrayDimensionParserRuleCall_0_0_3_0()); }
		ruleXJArrayDimension
		{ after(grammarAccess.getXJArrayConstructorCallAccess().getDimensionsXJArrayDimensionParserRuleCall_0_0_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__IndexesAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJArrayConstructorCallAccess().getIndexesXExpressionParserRuleCall_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXJArrayConstructorCallAccess().getIndexesXExpressionParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__DimensionsAssignment_3_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJArrayConstructorCallAccess().getDimensionsXJArrayDimensionParserRuleCall_3_0_0()); }
		ruleXJArrayDimension
		{ after(grammarAccess.getXJArrayConstructorCallAccess().getDimensionsXJArrayDimensionParserRuleCall_3_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__IndexesAssignment_3_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJArrayConstructorCallAccess().getIndexesXExpressionParserRuleCall_3_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXJArrayConstructorCallAccess().getIndexesXExpressionParserRuleCall_3_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayConstructorCall__ArrayLiteralAssignment_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJArrayConstructorCallAccess().getArrayLiteralXJArrayLiteralParserRuleCall_4_0()); }
		ruleXJArrayLiteral
		{ after(grammarAccess.getXJArrayConstructorCallAccess().getArrayLiteralXJArrayLiteralParserRuleCall_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__TypeAssignment_0_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXCastedExpressionAccess().getTypeJvmTypeReferenceParserRuleCall_0_0_2_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getXCastedExpressionAccess().getTypeJvmTypeReferenceParserRuleCall_0_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCastedExpression__TargetAssignment_0_0_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXCastedExpressionAccess().getTargetXExpressionParserRuleCall_0_0_4_0()); }
		ruleXExpression
		{ after(grammarAccess.getXCastedExpressionAccess().getTargetXExpressionParserRuleCall_0_0_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__FeatureAssignment_0_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getFeatureJvmIdentifiableElementCrossReference_0_0_0_1_0()); }
		(
			{ before(grammarAccess.getXPostfixOperationAccess().getFeatureJvmIdentifiableElementOpPostfixParserRuleCall_0_0_0_1_0_1()); }
			ruleOpPostfix
			{ after(grammarAccess.getXPostfixOperationAccess().getFeatureJvmIdentifiableElementOpPostfixParserRuleCall_0_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXPostfixOperationAccess().getFeatureJvmIdentifiableElementCrossReference_0_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__OperandAssignment_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getOperandXMemberFeatureCallParserRuleCall_0_1_0()); }
		ruleXMemberFeatureCall
		{ after(grammarAccess.getXPostfixOperationAccess().getOperandXMemberFeatureCallParserRuleCall_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__FeatureAssignment_1_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getFeatureJvmIdentifiableElementCrossReference_1_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXPostfixOperationAccess().getFeatureJvmIdentifiableElementOpPostfixParserRuleCall_1_1_0_0_1_0_1()); }
			ruleOpPostfix
			{ after(grammarAccess.getXPostfixOperationAccess().getFeatureJvmIdentifiableElementOpPostfixParserRuleCall_1_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXPostfixOperationAccess().getFeatureJvmIdentifiableElementCrossReference_1_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__IndexesAssignment_1_1_1_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getIndexesXExpressionParserRuleCall_1_1_1_0_2_0()); }
		ruleXExpression
		{ after(grammarAccess.getXPostfixOperationAccess().getIndexesXExpressionParserRuleCall_1_1_1_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XPostfixOperation__IndexesAssignment_1_1_1_0_4_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXPostfixOperationAccess().getIndexesXExpressionParserRuleCall_1_1_1_0_4_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXPostfixOperationAccess().getIndexesXExpressionParserRuleCall_1_1_1_0_4_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__ArrayDimensionsAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getArrayDimensionsArrayBracketsParserRuleCall_1_0_0_1_0()); }
		ruleArrayBrackets
		{ after(grammarAccess.getXMemberFeatureCallAccess().getArrayDimensionsArrayBracketsParserRuleCall_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__FeatureAssignment_1_1_0_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getFeatureJvmIdentifiableElementCrossReference_1_1_0_0_2_0()); }
		(
			{ before(grammarAccess.getXMemberFeatureCallAccess().getFeatureJvmIdentifiableElementFeatureCallIDParserRuleCall_1_1_0_0_2_0_1()); }
			ruleFeatureCallID
			{ after(grammarAccess.getXMemberFeatureCallAccess().getFeatureJvmIdentifiableElementFeatureCallIDParserRuleCall_1_1_0_0_2_0_1()); }
		)
		{ after(grammarAccess.getXMemberFeatureCallAccess().getFeatureJvmIdentifiableElementCrossReference_1_1_0_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__ValueAssignment_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getValueXAssignmentParserRuleCall_1_1_1_0()); }
		ruleXAssignment
		{ after(grammarAccess.getXMemberFeatureCallAccess().getValueXAssignmentParserRuleCall_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__FeatureAssignment_1_2_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getFeatureJvmIdentifiableElementCrossReference_1_2_1_0()); }
		(
			{ before(grammarAccess.getXMemberFeatureCallAccess().getFeatureJvmIdentifiableElementIdOrSuperParserRuleCall_1_2_1_0_1()); }
			ruleIdOrSuper
			{ after(grammarAccess.getXMemberFeatureCallAccess().getFeatureJvmIdentifiableElementIdOrSuperParserRuleCall_1_2_1_0_1()); }
		)
		{ after(grammarAccess.getXMemberFeatureCallAccess().getFeatureJvmIdentifiableElementCrossReference_1_2_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__ExplicitOperationCallAssignment_1_2_2_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getExplicitOperationCallLeftParenthesisKeyword_1_2_2_0_0()); }
		(
			{ before(grammarAccess.getXMemberFeatureCallAccess().getExplicitOperationCallLeftParenthesisKeyword_1_2_2_0_0()); }
			'('
			{ after(grammarAccess.getXMemberFeatureCallAccess().getExplicitOperationCallLeftParenthesisKeyword_1_2_2_0_0()); }
		)
		{ after(grammarAccess.getXMemberFeatureCallAccess().getExplicitOperationCallLeftParenthesisKeyword_1_2_2_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__MemberCallArgumentsAssignment_1_2_2_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsXExpressionParserRuleCall_1_2_2_1_0_0()); }
		ruleXExpression
		{ after(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsXExpressionParserRuleCall_1_2_2_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMemberFeatureCall__MemberCallArgumentsAssignment_1_2_2_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsXExpressionParserRuleCall_1_2_2_1_1_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXMemberFeatureCallAccess().getMemberCallArgumentsXExpressionParserRuleCall_1_2_2_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__DeclaredParamAssignment_0_0_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXForLoopExpressionAccess().getDeclaredParamFullJvmFormalParameterParserRuleCall_0_0_3_0()); }
		ruleFullJvmFormalParameter
		{ after(grammarAccess.getXForLoopExpressionAccess().getDeclaredParamFullJvmFormalParameterParserRuleCall_0_0_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__ForExpressionAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXForLoopExpressionAccess().getForExpressionXExpressionParserRuleCall_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXForLoopExpressionAccess().getForExpressionXExpressionParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XForLoopExpression__EachExpressionAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXForLoopExpressionAccess().getEachExpressionXJStatementOrBlockParserRuleCall_3_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getXForLoopExpressionAccess().getEachExpressionXJStatementOrBlockParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__InitExpressionsAssignment_3_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBasicForLoopExpressionAccess().getInitExpressionsXExpressionOrVarDeclarationParserRuleCall_3_0_0()); }
		ruleXExpressionOrVarDeclaration
		{ after(grammarAccess.getXBasicForLoopExpressionAccess().getInitExpressionsXExpressionOrVarDeclarationParserRuleCall_3_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__InitExpressionsAssignment_3_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBasicForLoopExpressionAccess().getInitExpressionsXExpressionOrVarDeclarationParserRuleCall_3_1_1_0()); }
		ruleXExpressionOrVarDeclaration
		{ after(grammarAccess.getXBasicForLoopExpressionAccess().getInitExpressionsXExpressionOrVarDeclarationParserRuleCall_3_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__ExpressionAssignment_5
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBasicForLoopExpressionAccess().getExpressionXExpressionParserRuleCall_5_0()); }
		ruleXExpression
		{ after(grammarAccess.getXBasicForLoopExpressionAccess().getExpressionXExpressionParserRuleCall_5_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__UpdateExpressionsAssignment_7_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBasicForLoopExpressionAccess().getUpdateExpressionsXExpressionParserRuleCall_7_0_0()); }
		ruleXExpression
		{ after(grammarAccess.getXBasicForLoopExpressionAccess().getUpdateExpressionsXExpressionParserRuleCall_7_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__UpdateExpressionsAssignment_7_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBasicForLoopExpressionAccess().getUpdateExpressionsXExpressionParserRuleCall_7_1_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXBasicForLoopExpressionAccess().getUpdateExpressionsXExpressionParserRuleCall_7_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBasicForLoopExpression__EachExpressionAssignment_9
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBasicForLoopExpressionAccess().getEachExpressionXJStatementOrBlockParserRuleCall_9_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getXBasicForLoopExpressionAccess().getEachExpressionXJStatementOrBlockParserRuleCall_9_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__IfAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXIfExpressionAccess().getIfXExpressionParserRuleCall_3_0()); }
		ruleXExpression
		{ after(grammarAccess.getXIfExpressionAccess().getIfXExpressionParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__ThenAssignment_5
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXIfExpressionAccess().getThenXJStatementOrBlockParserRuleCall_5_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getXIfExpressionAccess().getThenXJStatementOrBlockParserRuleCall_5_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XIfExpression__ElseAssignment_6_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXIfExpressionAccess().getElseXJStatementOrBlockParserRuleCall_6_1_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getXIfExpressionAccess().getElseXJStatementOrBlockParserRuleCall_6_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__PredicateAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXWhileExpressionAccess().getPredicateXExpressionParserRuleCall_3_0()); }
		ruleXExpression
		{ after(grammarAccess.getXWhileExpressionAccess().getPredicateXExpressionParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XWhileExpression__BodyAssignment_5
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXWhileExpressionAccess().getBodyXJStatementOrBlockParserRuleCall_5_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getXWhileExpressionAccess().getBodyXJStatementOrBlockParserRuleCall_5_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__BodyAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXDoWhileExpressionAccess().getBodyXJStatementOrBlockParserRuleCall_2_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getXDoWhileExpressionAccess().getBodyXJStatementOrBlockParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XDoWhileExpression__PredicateAssignment_5
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXDoWhileExpressionAccess().getPredicateXExpressionParserRuleCall_5_0()); }
		ruleXExpression
		{ after(grammarAccess.getXDoWhileExpressionAccess().getPredicateXExpressionParserRuleCall_5_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__SwitchAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXSwitchExpressionAccess().getSwitchXExpressionParserRuleCall_3_0()); }
		ruleXExpression
		{ after(grammarAccess.getXSwitchExpressionAccess().getSwitchXExpressionParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__CasesAssignment_6
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXSwitchExpressionAccess().getCasesXCasePartParserRuleCall_6_0()); }
		ruleXCasePart
		{ after(grammarAccess.getXSwitchExpressionAccess().getCasesXCasePartParserRuleCall_6_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSwitchExpression__DefaultAssignment_7_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXSwitchExpressionAccess().getDefaultXJSwitchStatementsParserRuleCall_7_2_0()); }
		ruleXJSwitchStatements
		{ after(grammarAccess.getXSwitchExpressionAccess().getDefaultXJSwitchStatementsParserRuleCall_7_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__CaseAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXCasePartAccess().getCaseXExpressionParserRuleCall_2_0()); }
		ruleXExpression
		{ after(grammarAccess.getXCasePartAccess().getCaseXExpressionParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCasePart__ThenAssignment_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXCasePartAccess().getThenXJSwitchStatementsParserRuleCall_4_0()); }
		ruleXJSwitchStatements
		{ after(grammarAccess.getXCasePartAccess().getThenXJSwitchStatementsParserRuleCall_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJSwitchStatements__ExpressionsAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJSwitchStatementsAccess().getExpressionsXJStatementOrBlockParserRuleCall_1_0()); }
		ruleXJStatementOrBlock
		{ after(grammarAccess.getXJSwitchStatementsAccess().getExpressionsXJStatementOrBlockParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__OpenParenthesisAssignment_0_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getOpenParenthesisLeftParenthesisKeyword_0_0_2_0()); }
		(
			{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getOpenParenthesisLeftParenthesisKeyword_0_0_2_0()); }
			'('
			{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getOpenParenthesisLeftParenthesisKeyword_0_0_2_0()); }
		)
		{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getOpenParenthesisLeftParenthesisKeyword_0_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__ResourcesAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getResourcesXJTryWithResourcesVariableDeclarationParserRuleCall_1_0()); }
		ruleXJTryWithResourcesVariableDeclaration
		{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getResourcesXJTryWithResourcesVariableDeclarationParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__ExpressionAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getExpressionXBlockExpressionParserRuleCall_3_0()); }
		ruleXBlockExpression
		{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getExpressionXBlockExpressionParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__CatchClausesAssignment_4
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getCatchClausesXCatchClauseParserRuleCall_4_0()); }
		ruleXCatchClause
		{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getCatchClausesXCatchClauseParserRuleCall_4_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesStatement__FinallyExpressionAssignment_5_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesStatementAccess().getFinallyExpressionXBlockExpressionParserRuleCall_5_1_0()); }
		ruleXBlockExpression
		{ after(grammarAccess.getXJTryWithResourcesStatementAccess().getFinallyExpressionXBlockExpressionParserRuleCall_5_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__FinalAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getFinalFinalKeyword_1_0()); }
		(
			{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getFinalFinalKeyword_1_0()); }
			'final'
			{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getFinalFinalKeyword_1_0()); }
		)
		{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getFinalFinalKeyword_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__TypeAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getTypeJvmTypeReferenceParserRuleCall_2_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getTypeJvmTypeReferenceParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__NameAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getNameValidIDParserRuleCall_3_0()); }
		ruleValidID
		{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getNameValidIDParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__RightAssignment_5
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getRightXExpressionParserRuleCall_5_0()); }
		ruleXExpression
		{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getRightXExpressionParserRuleCall_5_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJTryWithResourcesVariableDeclaration__SemicolonAssignment_6
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getSemicolonSemicolonKeyword_6_0()); }
		(
			{ before(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getSemicolonSemicolonKeyword_6_0()); }
			';'
			{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getSemicolonSemicolonKeyword_6_0()); }
		)
		{ after(grammarAccess.getXJTryWithResourcesVariableDeclarationAccess().getSemicolonSemicolonKeyword_6_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__ExpressionAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getExpressionXBlockExpressionParserRuleCall_2_0()); }
		ruleXBlockExpression
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getExpressionXBlockExpressionParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__CatchClausesAssignment_3_0_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getCatchClausesXCatchClauseParserRuleCall_3_0_0_0()); }
		ruleXCatchClause
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getCatchClausesXCatchClauseParserRuleCall_3_0_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__FinallyExpressionAssignment_3_0_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionXBlockExpressionParserRuleCall_3_0_1_1_0()); }
		ruleXBlockExpression
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionXBlockExpressionParserRuleCall_3_0_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XTryCatchFinallyExpression__FinallyExpressionAssignment_3_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionXBlockExpressionParserRuleCall_3_1_1_0()); }
		ruleXBlockExpression
		{ after(grammarAccess.getXTryCatchFinallyExpressionAccess().getFinallyExpressionXBlockExpressionParserRuleCall_3_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__DeclaredParamAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXCatchClauseAccess().getDeclaredParamFullJvmFormalParameterParserRuleCall_3_0()); }
		ruleFullJvmFormalParameter
		{ after(grammarAccess.getXCatchClauseAccess().getDeclaredParamFullJvmFormalParameterParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCatchClause__ExpressionAssignment_5
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXCatchClauseAccess().getExpressionXBlockExpressionParserRuleCall_5_0()); }
		ruleXBlockExpression
		{ after(grammarAccess.getXCatchClauseAccess().getExpressionXBlockExpressionParserRuleCall_5_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__ParamAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXSynchronizedExpressionAccess().getParamXExpressionParserRuleCall_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXSynchronizedExpressionAccess().getParamXExpressionParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSynchronizedExpression__ExpressionAssignment_3
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXSynchronizedExpressionAccess().getExpressionXBlockExpressionParserRuleCall_3_0()); }
		ruleXBlockExpression
		{ after(grammarAccess.getXSynchronizedExpressionAccess().getExpressionXBlockExpressionParserRuleCall_3_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__ElementsAssignment_2_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJArrayLiteralAccess().getElementsXExpressionParserRuleCall_2_0_0()); }
		ruleXExpression
		{ after(grammarAccess.getXJArrayLiteralAccess().getElementsXExpressionParserRuleCall_2_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XJArrayLiteral__ElementsAssignment_2_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXJArrayLiteralAccess().getElementsXExpressionParserRuleCall_2_1_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXJArrayLiteralAccess().getElementsXExpressionParserRuleCall_2_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XCharLiteral__ValueAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXCharLiteralAccess().getValueCHARACTERTerminalRuleCall_1_0()); }
		RULE_CHARACTER
		{ after(grammarAccess.getXCharLiteralAccess().getValueCHARACTERTerminalRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XStringLiteral__ValueAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXStringLiteralAccess().getValueSTRINGTerminalRuleCall_1_0()); }
		RULE_STRING
		{ after(grammarAccess.getXStringLiteralAccess().getValueSTRINGTerminalRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAndExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXAndExpressionAccess().getFeatureJvmIdentifiableElementOpAndParserRuleCall_1_0_0_1_0_1()); }
			ruleOpAnd
			{ after(grammarAccess.getXAndExpressionAccess().getFeatureJvmIdentifiableElementOpAndParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXAndExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAndExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAndExpressionAccess().getRightOperandXBitwiseInclusiveOrExpressionParserRuleCall_1_1_0()); }
		ruleXBitwiseInclusiveOrExpression
		{ after(grammarAccess.getXAndExpressionAccess().getRightOperandXBitwiseInclusiveOrExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getFeatureJvmIdentifiableElementOpInclusiveOrParserRuleCall_1_0_0_1_0_1()); }
			ruleOpInclusiveOr
			{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getFeatureJvmIdentifiableElementOpInclusiveOrParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseInclusiveOrExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getRightOperandXBitwiseExclusiveOrExpressionParserRuleCall_1_1_0()); }
		ruleXBitwiseExclusiveOrExpression
		{ after(grammarAccess.getXBitwiseInclusiveOrExpressionAccess().getRightOperandXBitwiseExclusiveOrExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getFeatureJvmIdentifiableElementOpExclusiveOrParserRuleCall_1_0_0_1_0_1()); }
			ruleOpExclusiveOr
			{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getFeatureJvmIdentifiableElementOpExclusiveOrParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseExclusiveOrExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getRightOperandXBitwiseAndExpressionParserRuleCall_1_1_0()); }
		ruleXBitwiseAndExpression
		{ after(grammarAccess.getXBitwiseExclusiveOrExpressionAccess().getRightOperandXBitwiseAndExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBitwiseAndExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXBitwiseAndExpressionAccess().getFeatureJvmIdentifiableElementOpBitwiseAndParserRuleCall_1_0_0_1_0_1()); }
			ruleOpBitwiseAnd
			{ after(grammarAccess.getXBitwiseAndExpressionAccess().getFeatureJvmIdentifiableElementOpBitwiseAndParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXBitwiseAndExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBitwiseAndExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBitwiseAndExpressionAccess().getRightOperandXEqualityExpressionParserRuleCall_1_1_0()); }
		ruleXEqualityExpression
		{ after(grammarAccess.getXBitwiseAndExpressionAccess().getRightOperandXEqualityExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__ElementsAssignment_0_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getElementsXAnnotationOrExpressionParserRuleCall_0_1_0_0()); }
		ruleXAnnotationOrExpression
		{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getElementsXAnnotationOrExpressionParserRuleCall_0_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValueOrCommaList__ElementsAssignment_0_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getElementsXAnnotationOrExpressionParserRuleCall_0_1_1_1_0()); }
		ruleXAnnotationOrExpression
		{ after(grammarAccess.getXAnnotationElementValueOrCommaListAccess().getElementsXAnnotationOrExpressionParserRuleCall_0_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__ElementsAssignment_0_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationElementValueAccess().getElementsXAnnotationOrExpressionParserRuleCall_0_1_0_0()); }
		ruleXAnnotationOrExpression
		{ after(grammarAccess.getXAnnotationElementValueAccess().getElementsXAnnotationOrExpressionParserRuleCall_0_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValue__ElementsAssignment_0_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationElementValueAccess().getElementsXAnnotationOrExpressionParserRuleCall_0_1_1_1_0()); }
		ruleXAnnotationOrExpression
		{ after(grammarAccess.getXAnnotationElementValueAccess().getElementsXAnnotationOrExpressionParserRuleCall_0_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__AnnotationTypeAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationAccess().getAnnotationTypeJvmAnnotationTypeCrossReference_2_0()); }
		(
			{ before(grammarAccess.getXAnnotationAccess().getAnnotationTypeJvmAnnotationTypeQualifiedNameParserRuleCall_2_0_1()); }
			ruleQualifiedName
			{ after(grammarAccess.getXAnnotationAccess().getAnnotationTypeJvmAnnotationTypeQualifiedNameParserRuleCall_2_0_1()); }
		)
		{ after(grammarAccess.getXAnnotationAccess().getAnnotationTypeJvmAnnotationTypeCrossReference_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__ElementValuePairsAssignment_3_1_0_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationAccess().getElementValuePairsXAnnotationElementValuePairParserRuleCall_3_1_0_0_0()); }
		ruleXAnnotationElementValuePair
		{ after(grammarAccess.getXAnnotationAccess().getElementValuePairsXAnnotationElementValuePairParserRuleCall_3_1_0_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__ElementValuePairsAssignment_3_1_0_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationAccess().getElementValuePairsXAnnotationElementValuePairParserRuleCall_3_1_0_1_1_0()); }
		ruleXAnnotationElementValuePair
		{ after(grammarAccess.getXAnnotationAccess().getElementValuePairsXAnnotationElementValuePairParserRuleCall_3_1_0_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotation__ValueAssignment_3_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationAccess().getValueXAnnotationElementValueOrCommaListParserRuleCall_3_1_1_0()); }
		ruleXAnnotationElementValueOrCommaList
		{ after(grammarAccess.getXAnnotationAccess().getValueXAnnotationElementValueOrCommaListParserRuleCall_3_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__ElementAssignment_0_0_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationElementValuePairAccess().getElementJvmOperationCrossReference_0_0_0_0()); }
		(
			{ before(grammarAccess.getXAnnotationElementValuePairAccess().getElementJvmOperationValidIDParserRuleCall_0_0_0_0_1()); }
			ruleValidID
			{ after(grammarAccess.getXAnnotationElementValuePairAccess().getElementJvmOperationValidIDParserRuleCall_0_0_0_0_1()); }
		)
		{ after(grammarAccess.getXAnnotationElementValuePairAccess().getElementJvmOperationCrossReference_0_0_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAnnotationElementValuePair__ValueAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAnnotationElementValuePairAccess().getValueXAnnotationElementValueParserRuleCall_1_0()); }
		ruleXAnnotationElementValue
		{ after(grammarAccess.getXAnnotationElementValuePairAccess().getValueXAnnotationElementValueParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXOrExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXOrExpressionAccess().getFeatureJvmIdentifiableElementOpOrParserRuleCall_1_0_0_1_0_1()); }
			ruleOpOr
			{ after(grammarAccess.getXOrExpressionAccess().getFeatureJvmIdentifiableElementOpOrParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXOrExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOrExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXOrExpressionAccess().getRightOperandXAndExpressionParserRuleCall_1_1_0()); }
		ruleXAndExpression
		{ after(grammarAccess.getXOrExpressionAccess().getRightOperandXAndExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXEqualityExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXEqualityExpressionAccess().getFeatureJvmIdentifiableElementOpEqualityParserRuleCall_1_0_0_1_0_1()); }
			ruleOpEquality
			{ after(grammarAccess.getXEqualityExpressionAccess().getFeatureJvmIdentifiableElementOpEqualityParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXEqualityExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XEqualityExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXEqualityExpressionAccess().getRightOperandXRelationalExpressionParserRuleCall_1_1_0()); }
		ruleXRelationalExpression
		{ after(grammarAccess.getXEqualityExpressionAccess().getRightOperandXRelationalExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__TypeAssignment_1_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXRelationalExpressionAccess().getTypeJvmTypeReferenceParserRuleCall_1_0_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getXRelationalExpressionAccess().getTypeJvmTypeReferenceParserRuleCall_1_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__FeatureAssignment_1_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXRelationalExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXRelationalExpressionAccess().getFeatureJvmIdentifiableElementOpCompareParserRuleCall_1_1_0_0_1_0_1()); }
			ruleOpCompare
			{ after(grammarAccess.getXRelationalExpressionAccess().getFeatureJvmIdentifiableElementOpCompareParserRuleCall_1_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXRelationalExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XRelationalExpression__RightOperandAssignment_1_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXRelationalExpressionAccess().getRightOperandXOtherOperatorExpressionParserRuleCall_1_1_1_0()); }
		ruleXOtherOperatorExpression
		{ after(grammarAccess.getXRelationalExpressionAccess().getRightOperandXOtherOperatorExpressionParserRuleCall_1_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXOtherOperatorExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXOtherOperatorExpressionAccess().getFeatureJvmIdentifiableElementOpOtherParserRuleCall_1_0_0_1_0_1()); }
			ruleOpOther
			{ after(grammarAccess.getXOtherOperatorExpressionAccess().getFeatureJvmIdentifiableElementOpOtherParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXOtherOperatorExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XOtherOperatorExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXOtherOperatorExpressionAccess().getRightOperandXAdditiveExpressionParserRuleCall_1_1_0()); }
		ruleXAdditiveExpression
		{ after(grammarAccess.getXOtherOperatorExpressionAccess().getRightOperandXAdditiveExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAdditiveExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXAdditiveExpressionAccess().getFeatureJvmIdentifiableElementOpAddParserRuleCall_1_0_0_1_0_1()); }
			ruleOpAdd
			{ after(grammarAccess.getXAdditiveExpressionAccess().getFeatureJvmIdentifiableElementOpAddParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXAdditiveExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XAdditiveExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXAdditiveExpressionAccess().getRightOperandXMultiplicativeExpressionParserRuleCall_1_1_0()); }
		ruleXMultiplicativeExpression
		{ after(grammarAccess.getXAdditiveExpressionAccess().getRightOperandXMultiplicativeExpressionParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__FeatureAssignment_1_0_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMultiplicativeExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
		(
			{ before(grammarAccess.getXMultiplicativeExpressionAccess().getFeatureJvmIdentifiableElementOpMultiParserRuleCall_1_0_0_1_0_1()); }
			ruleOpMulti
			{ after(grammarAccess.getXMultiplicativeExpressionAccess().getFeatureJvmIdentifiableElementOpMultiParserRuleCall_1_0_0_1_0_1()); }
		)
		{ after(grammarAccess.getXMultiplicativeExpressionAccess().getFeatureJvmIdentifiableElementCrossReference_1_0_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XMultiplicativeExpression__RightOperandAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXMultiplicativeExpressionAccess().getRightOperandXUnaryOperationParserRuleCall_1_1_0()); }
		ruleXUnaryOperation
		{ after(grammarAccess.getXMultiplicativeExpressionAccess().getRightOperandXUnaryOperationParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XUnaryOperation__FeatureAssignment_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXUnaryOperationAccess().getFeatureJvmIdentifiableElementCrossReference_0_1_0()); }
		(
			{ before(grammarAccess.getXUnaryOperationAccess().getFeatureJvmIdentifiableElementOpUnaryParserRuleCall_0_1_0_1()); }
			ruleOpUnary
			{ after(grammarAccess.getXUnaryOperationAccess().getFeatureJvmIdentifiableElementOpUnaryParserRuleCall_0_1_0_1()); }
		)
		{ after(grammarAccess.getXUnaryOperationAccess().getFeatureJvmIdentifiableElementCrossReference_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XUnaryOperation__OperandAssignment_0_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXUnaryOperationAccess().getOperandXUnaryOperationParserRuleCall_0_2_0()); }
		ruleXUnaryOperation
		{ after(grammarAccess.getXUnaryOperationAccess().getOperandXUnaryOperationParserRuleCall_0_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__ElementsAssignment_3_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXSetLiteralAccess().getElementsXExpressionParserRuleCall_3_0_0()); }
		ruleXExpression
		{ after(grammarAccess.getXSetLiteralAccess().getElementsXExpressionParserRuleCall_3_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XSetLiteral__ElementsAssignment_3_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXSetLiteralAccess().getElementsXExpressionParserRuleCall_3_1_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXSetLiteralAccess().getElementsXExpressionParserRuleCall_3_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__ElementsAssignment_3_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXListLiteralAccess().getElementsXExpressionParserRuleCall_3_0_0()); }
		ruleXExpression
		{ after(grammarAccess.getXListLiteralAccess().getElementsXExpressionParserRuleCall_3_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XListLiteral__ElementsAssignment_3_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXListLiteralAccess().getElementsXExpressionParserRuleCall_3_1_1_0()); }
		ruleXExpression
		{ after(grammarAccess.getXListLiteralAccess().getElementsXExpressionParserRuleCall_3_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XExpressionInClosure__ExpressionsAssignment_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXExpressionInClosureAccess().getExpressionsXExpressionOrVarDeclarationParserRuleCall_1_0_0()); }
		ruleXExpressionOrVarDeclaration
		{ after(grammarAccess.getXExpressionInClosureAccess().getExpressionsXExpressionOrVarDeclarationParserRuleCall_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmFormalParameter__ParameterTypeAssignment_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmFormalParameterAccess().getParameterTypeJvmTypeReferenceParserRuleCall_0_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJvmFormalParameterAccess().getParameterTypeJvmTypeReferenceParserRuleCall_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmFormalParameter__NameAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmFormalParameterAccess().getNameValidIDParserRuleCall_1_0()); }
		ruleValidID
		{ after(grammarAccess.getJvmFormalParameterAccess().getNameValidIDParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XBooleanLiteral__IsTrueAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXBooleanLiteralAccess().getIsTrueTrueKeyword_1_1_0()); }
		(
			{ before(grammarAccess.getXBooleanLiteralAccess().getIsTrueTrueKeyword_1_1_0()); }
			'true'
			{ after(grammarAccess.getXBooleanLiteralAccess().getIsTrueTrueKeyword_1_1_0()); }
		)
		{ after(grammarAccess.getXBooleanLiteralAccess().getIsTrueTrueKeyword_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XNumberLiteral__ValueAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXNumberLiteralAccess().getValueNumberParserRuleCall_1_0()); }
		ruleNumber
		{ after(grammarAccess.getXNumberLiteralAccess().getValueNumberParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XThrowExpression__ExpressionAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXThrowExpressionAccess().getExpressionXExpressionParserRuleCall_2_0()); }
		ruleXExpression
		{ after(grammarAccess.getXThrowExpressionAccess().getExpressionXExpressionParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__XReturnExpression__ExpressionAssignment_2
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getXReturnExpressionAccess().getExpressionXExpressionParserRuleCall_2_0()); }
		ruleXExpression
		{ after(grammarAccess.getXReturnExpressionAccess().getExpressionXExpressionParserRuleCall_2_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__TypeAssignment_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeJvmTypeCrossReference_0_0()); }
		(
			{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeJvmTypeQualifiedNameParserRuleCall_0_0_1()); }
			ruleQualifiedName
			{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeJvmTypeQualifiedNameParserRuleCall_0_0_1()); }
		)
		{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeJvmTypeCrossReference_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsJvmArgumentTypeReferenceParserRuleCall_1_1_0()); }
		ruleJvmArgumentTypeReference
		{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsJvmArgumentTypeReferenceParserRuleCall_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_2_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsJvmArgumentTypeReferenceParserRuleCall_1_2_1_0()); }
		ruleJvmArgumentTypeReference
		{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsJvmArgumentTypeReferenceParserRuleCall_1_2_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__TypeAssignment_1_4_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeJvmTypeCrossReference_1_4_1_0()); }
		(
			{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeJvmTypeValidIDParserRuleCall_1_4_1_0_1()); }
			ruleValidID
			{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeJvmTypeValidIDParserRuleCall_1_4_1_0_1()); }
		)
		{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getTypeJvmTypeCrossReference_1_4_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_4_2_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsJvmArgumentTypeReferenceParserRuleCall_1_4_2_1_0()); }
		ruleJvmArgumentTypeReference
		{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsJvmArgumentTypeReferenceParserRuleCall_1_4_2_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmParameterizedTypeReference__ArgumentsAssignment_1_4_2_2_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsJvmArgumentTypeReferenceParserRuleCall_1_4_2_2_1_0()); }
		ruleJvmArgumentTypeReference
		{ after(grammarAccess.getJvmParameterizedTypeReferenceAccess().getArgumentsJvmArgumentTypeReferenceParserRuleCall_1_4_2_2_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__ConstraintsAssignment_2_0_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsJvmUpperBoundParserRuleCall_2_0_0_0()); }
		ruleJvmUpperBound
		{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsJvmUpperBoundParserRuleCall_2_0_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__ConstraintsAssignment_2_0_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsJvmUpperBoundAndedParserRuleCall_2_0_1_0()); }
		ruleJvmUpperBoundAnded
		{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsJvmUpperBoundAndedParserRuleCall_2_0_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__ConstraintsAssignment_2_1_0
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsJvmLowerBoundParserRuleCall_2_1_0_0()); }
		ruleJvmLowerBound
		{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsJvmLowerBoundParserRuleCall_2_1_0_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmWildcardTypeReference__ConstraintsAssignment_2_1_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsJvmLowerBoundAndedParserRuleCall_2_1_1_0()); }
		ruleJvmLowerBoundAnded
		{ after(grammarAccess.getJvmWildcardTypeReferenceAccess().getConstraintsJvmLowerBoundAndedParserRuleCall_2_1_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmUpperBound__TypeReferenceAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmUpperBoundAccess().getTypeReferenceJvmTypeReferenceParserRuleCall_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJvmUpperBoundAccess().getTypeReferenceJvmTypeReferenceParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmUpperBoundAnded__TypeReferenceAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmUpperBoundAndedAccess().getTypeReferenceJvmTypeReferenceParserRuleCall_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJvmUpperBoundAndedAccess().getTypeReferenceJvmTypeReferenceParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmLowerBound__TypeReferenceAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmLowerBoundAccess().getTypeReferenceJvmTypeReferenceParserRuleCall_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJvmLowerBoundAccess().getTypeReferenceJvmTypeReferenceParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

rule__JvmLowerBoundAnded__TypeReferenceAssignment_1
	@init {
		int stackSize = keepStackSize();
	}
:
	(
		{ before(grammarAccess.getJvmLowerBoundAndedAccess().getTypeReferenceJvmTypeReferenceParserRuleCall_1_0()); }
		ruleJvmTypeReference
		{ after(grammarAccess.getJvmLowerBoundAndedAccess().getTypeReferenceJvmTypeReferenceParserRuleCall_1_0()); }
	)
;
finally {
	restoreStackSize(stackSize);
}

RULE_ML_COMMENT : 'MyVeryOwnComment!';

RULE_STRING : '"' ('\\' .|~(('\\'|'"')))* '"'?;

RULE_CHARACTER : '\'' ('\\' .|~(('\\'|'\'')))* '\''?;

RULE_HEX : ('0x'|'0X') ('0'..'9'|'a'..'f'|'A'..'F'|'_')+ ('#' (('b'|'B') ('i'|'I')|('l'|'L')))?;

RULE_INT : '0'..'9' ('0'..'9'|'_')*;

RULE_DECIMAL : RULE_INT (('e'|'E') ('+'|'-')? RULE_INT)? (('b'|'B') ('i'|'I'|'d'|'D')|('l'|'L'|'d'|'D'|'f'|'F'))?;

RULE_ID : '^'? ('a'..'z'|'A'..'Z'|'$'|'_') ('a'..'z'|'A'..'Z'|'$'|'_'|'0'..'9')*;

RULE_SL_COMMENT : '//' ~(('\n'|'\r'))* ('\r'? '\n')?;

RULE_WS : (' '|'\t'|'\r'|'\n')+;

RULE_ANY_OTHER : .;
